...
create or replace package debug_utils is
  ...
  procedure get_values(frame binary_integer := 0);
end;
/
...
create or replace package body debug_utils is
  ...
  procedure get_runtime_info is
    result binary_integer;
    local_runtime dbms_debug.runtime_info;
  begin
    result := dbms_debug.get_runtime_info(null, local_runtime);
    runtime := local_runtime;
    print_runtime(runtime);
    dbms_output.put_line('result = ' || result || ' ' || to_error(result));
  end;
  procedure get_values(frame binary_integer := 0) is
    result binary_integer;
    scalar_values varchar2(4000);
  begin
    result := dbms_debug.get_values(scalar_values, frame);
    dbms_output.put_line('result = ' || result || ' ' || to_error(result));
