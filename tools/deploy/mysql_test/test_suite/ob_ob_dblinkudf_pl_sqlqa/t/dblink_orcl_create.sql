...
create or replace function t1max return int is
  x int;
begin
...
insert into t1 values('luo', 3);$
insert into t1 values('bar', 2);$
insert into t1 values('zip', 5);$
insert into t1 values('zap', 1);$
commit;$
...
create or replace function bug5240 return int is
  x int;
  cursor c is select data from t1 where rownum = 1;
begin
  open c;
  fetch c into x;
  close c;
...
create table t1_11(f1_11 int);$
insert into t1_11 values(1);$
insert into t1_11 values(2);$
...
create table t1_14(col1 int);$
create table t2_14(col2 int);$
insert into t1_14 values(1);$
