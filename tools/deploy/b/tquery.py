...
where <pat> will be used to:
 ...
 - special file: 'stdin' indicate read from stdin, load into ':memory:' database
Sepcial Tables:
...
#matplotlib.rcParams['figure.figsize']=(64,48)
...
def readlines(path):
    f = isinstance(path, file) and path or open(path)
    content = f.readlines()
    f.close()
    return content
...
def mem_db(fd):
    return get_db(':memory:', t_=lambda :table_load(fd))
