...
begin
  ...
  prepare stmt from @sql_text;
  execute stmt;
  set @sql_text = concat("create tenant ", tenant_name, " primary_zone='", zone_name, "', resource_pool_list=('pool_", tenant_name, "'), locality = 'F@z1,F@z2,F@z3' set ob_compatibility_mode='oracle', ob_tcp_invited_nodes='%',  parallel_servers_target=100, ob_sql_work_area_percentage=50, secure_file_priv = '/';");
  prepare stmt from @sql_text;
  execute stmt;
  deallocate prepare stmt;
end //
...
call create_oracle_tenant('oracle_tenant');
select sleep(10);

alter system set _pushdown_storage_level=4 tenant=sys;
alter system set _pushdown_storage_level=4 tenant=all_user;
alter system set _pushdown_storage_level=4 tenant=all_meta;
