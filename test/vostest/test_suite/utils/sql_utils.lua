function get_mem_hold(sys_conn, addr)
    local rows, ok = sys_conn:query(string.format("select *, sum(hold) over() from (select mod_name, sum(hold) as hold from __all_virtual_memory_info where ctx_name not in ('MEMSTORE_CTX_ID', 'LIBEASY') and mod_name != 'KvstorCacheMb' group by mod_name)"))
