                                              ...
                                              int64_t &total_ctx_cnt)
{
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < raw_exprs.count(); i++) {
    ...
    if (OB_ISNULL(raw_expr = raw_exprs.at(i))
      ...
      // Special handling for expressions outside IS_EXPR_OP range but still need operator generation
      // e.g., ObLoadFileRawExpr with T_LOAD_FILE_EXPRESSION = 4916
      if (T_LOAD_FILE_EXPRESSION == rt_expr->type_) {
        ObLoadFileRawExpr *load_file_expr = static_cast<ObLoadFileRawExpr *>(raw_expr);
        if (OB_ISNULL(load_file_expr)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("load_file_expr is null", K(ret));
        } else {
          // Directly create ObExprLoadFile object and call cg_expr
          // because ObExprOperatorFetcher::add_expr_item won't set op_ for non-IS_EXPR_OP types
          ObExprLoadFile *load_file_op = NULL;
          void *ptr = NULL;
          if (OB_ISNULL(ptr = op_cg_ctx_.allocator_->alloc(sizeof(ObExprLoadFile)))) {
            ret = OB_ALLOCATE_MEMORY_FAILED;
            LOG_WARN("fail to alloc memory for ObExprLoadFile", K(ret));
          } else {
            load_file_op = new(ptr) ObExprLoadFile(*op_cg_ctx_.allocator_);
            if (OB_ISNULL(load_file_op)) {
              ...
              op->set_real_param_num(static_cast<int32_t>(load_file_expr->get_param_count()));
              op->set_row_dimension(ObExprOperator::NOT_ROW_DIMENSION);
              op->set_result_type(load_file_expr->get_result_type());
              if (op->need_rt_ctx()) {
                rt_expr->expr_ctx_id_ = total_ctx_cnt;
                total_ctx_cnt += 1;
              }
              if (OB_FAIL(op->cg_expr(op_cg_ctx_, *raw_expr, *rt_expr))) {
                LOG_WARN("fail to cg_expr for ObLoadFileRawExpr", K(ret));
      ...
      // do nothing for other non-IS_EXPR_OP expressions
