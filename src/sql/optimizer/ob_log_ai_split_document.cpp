 ...
 */
 ...
 int ObLogAiSplitDocument::generate_access_exprs()
 {
   int ret = OB_SUCCESS;
 ...
 int ObLogAiSplitDocument::allocate_expr_post(ObAllocExprContext &ctx)
 {
   int ret = OB_SUCCESS;
 ...
 int ObLogAiSplitDocument::get_op_exprs(ObIArray<ObRawExpr*> &all_exprs)
 {
   int ret = OB_SUCCESS;
   const ObDMLStmt *stmt = get_stmt();
   ...
   if (OB_FAIL(generate_access_exprs())) {
     ...
     // add other expr into all exprs
     if (OB_NOT_NULL(context_expr_) && OB_FAIL(all_exprs.push_back(context_expr_))) {
       LOG_WARN("push context expr to array failed", K(ret));
     } else if (OB_NOT_NULL(option_expr_) && OB_FAIL(all_exprs.push_back(option_expr_))) {
       LOG_WARN("push option expr to array failed", K(ret));
     } else { /*do nothing*/ }
   } 
 ...
 // todo@dazhi
 int ObLogAiSplitDocument::get_plan_item_info(PlanText &plan_text, 
                                        ObSqlPlanItem &plan_item)
 {
   int ret = OB_SUCCESS;
   if (OB_FAIL(ObLogicalOperator::get_plan_item_info(plan_text, plan_item))) {
     LOG_WARN("failed to get plan item info", K(ret));
   } else if (OB_ISNULL(get_context_expr())) {
     ret = OB_ERR_UNEXPECTED;
     LOG_WARN("unexpected null value expr", K(ret));
   } else {
     BEGIN_BUF_PRINT;
     EXPLAIN_PRINT_EXPR(get_context_expr(), type);
     EXPLAIN_PRINT_EXPR(get_option_expr(), type);
   ...
   if (OB_SUCC(ret)) {
     ...
     BUF_PRINT_STR("AI_SPLIT_DOCUMENT",
 ...
 uint64_t ObLogAiSplitDocument::hash(uint64_t seed) const
 {
