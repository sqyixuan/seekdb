  ...
  if (T_INDEX_COLUMN_LIST != node.type_ || node.num_child_ <= 0 ||
    ...
    for (int32_t i = 0; OB_SUCC(ret) && i < node.num_child_; ++i) {
      ...
      if (OB_ISNULL(sort_column_node) ||
        ...
        if (OB_SUCC(ret)) {
          const ObColumnSchemaV2 *column_schema = NULL;
          if (OB_NOT_NULL(column_schema = table_schema_->get_column_schema(sort_item.column_name_))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (NOT_SPECIFIED == index_scope_) {
        ...
        if (!global_) {
          if (nullptr != table_schema_) {
            if (OB_FAIL(get_suggest_index_scope(index_arg.tenant_id_, table_schema_->get_table_id(), index_arg, index_keyname_, global_))) {
      ...
      if (OB_FAIL(ret)) {
        ...
        if (index_keyname_ == MULTI_KEY || index_keyname_ == MULTI_UNIQUE_KEY) {
          ...
          if (global_) {
            ret = OB_NOT_SUPPORTED;
            LOG_WARN("global index, multivalue index not supported", K(ret));
        ...
        if (global_) {
          if (index_keyname_ == SPATIAL_KEY) {
            ...
            LOG_USER_ERROR(OB_NOT_SUPPORTED, "multivalue index");
          } else {
...
int ObAlterTableResolver::resolve_drop_primary(const ParseNode &action_node_list)
{
  ...
  if (OB_ISNULL(alter_table_stmt)) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("table_schema is null", K(ret));
  } else if (table_schema_->is_heap_organized_table()) {
    const ObString pk_name = "PRIMAY";
    ret = OB_NOT_SUPPORTED;
...
int ObAlterTableResolver::fill_high_bound_val_for_split_partition(const AlterTableSchema &alter_table_schema, ObPartition& split_part)
{
  ...
  OZ (ObMultiVersionSchemaService::get_instance().get_tenant_schema_guard(tenant_id, schema_guard));
  ...
  if (T_COLUMN_ALTER != node.type_ || OB_ISNULL(node.children_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (NULL == default_node ||
        ...
        } else if (ob_is_json_tc(alter_column_schema.get_data_type())) {
          ret = OB_ERR_BLOB_CANT_HAVE_DEFAULT;
          SQL_RESV_LOG(WARN, "BLOB/TEXT or JSON can't set default value!", K(ret));
        } else if (ob_is_geometry_tc(alter_column_schema.get_data_type())) {
          ret = OB_ERR_BLOB_CANT_HAVE_DEFAULT;
          SQL_RESV_LOG(WARN, "GEOMETRY can't set default value!", K(ret));
        }else if (ob_is_collection_sql_type(alter_column_schema.get_data_type())) {
          ret = OB_ERR_BLOB_CANT_HAVE_DEFAULT;
...
{
  int ret = OB_SUCCESS;
  if (ori_column.has_not_null_constraint()) {
    ...
    for (int i = 0; OB_SUCC(ret) && i < node.num_child_; ++i) {
      ...
      if (OB_SUCC(ret)) {
        ...
        if (OB_ISNULL(alter_table_stmt)) {
          ret = OB_ERR_UNEXPECTED;
          SQL_RESV_LOG(WARN, "alter table stmt should not be null", K(ret));
        } else if (OB_FAIL(check_column_in_foreign_key(
                              *table_schema_,
                              alter_column_schema.get_origin_column_name(),
                              true /* is_drop_column */))) {
          SQL_RESV_LOG(WARN, "failed to check_column_in_foreign_key", K(ret));
