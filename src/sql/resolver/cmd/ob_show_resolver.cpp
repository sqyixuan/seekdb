...
DEFINE_SHOW_CLAUSE_SET(SHOW_SYS_FULL_PROCESSLIST,
                       NULL,
                       "SELECT id AS `Id`, user as `User`, tenant as `Tenant`, host AS `Host`, db AS `db`, command AS `Command`, cast(time as SIGNED) AS `Time`, state AS `State`, info AS `Info`, sql_port AS `Port`, proxy_sessid AS `Proxy_sessid` FROM %s.%s",
...
DEFINE_SHOW_CLAUSE_SET(SHOW_TENANT,
                       NULL,
                       "select  `tenant_name` as `Current_tenant_name` from %s.%s  where 0 = %ld % 1",
                       NULL,
                       NULL);
...
DEFINE_SHOW_CLAUSE_SET(SHOW_CREATE_TENANT,
                       NULL,
                       "select  `tenant_name` as `Tenant`, `create_stmt` as `Create Tenant` from %s.%s  where 0 = %ld % 1",
                       NULL,
                       NULL);
