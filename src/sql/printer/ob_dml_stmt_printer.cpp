...
int ObDMLStmtPrinter::print_hint()
{
  ...
  if (OB_ISNULL(stmt_) || OB_ISNULL(stmt_->get_query_ctx()) || OB_ISNULL(buf_) || OB_ISNULL(pos_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      plan_text.type_ = EXPLAIN_UNINITIALIZED;  // just for print hint, ExplainType set as invalid type
      if (OB_FAIL(ret)) {
        // do nothing
...
{
  int ret = OB_SUCCESS;
  // Recursive CTE must have defined an alias list
  // Print the table of cte without printing the database name?
  //DATA_PRINTF(" %.*s)", LEN_AND_PTR(cte_table->table_name_));
  // cte in mysql when defined and used, does not allow usage like db.cte_name this method, when printing we
  // Cannot print database name
...
// Restore the complete definition of cte
int ObDMLStmtPrinter::print_with()
{
  ...
  if (OB_ISNULL(stmt_)) {
    ...
    DATA_PRINTF("WITH RECURSIVE ");
    // Restore definition, cte first put the cte generated by T_WITH_CLAUSE in this stmt, then put the cte passed from parent
