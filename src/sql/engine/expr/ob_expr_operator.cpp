  ...
  common::ObExprTypeCtx &type_ctx)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    for (int i = 0; OB_SUCC(ret) && i < param_num; ++i) {
      ...
      if (types[i].is_enum_set_with_subschema()) {
        ObObjMeta obj_meta;
...
#if defined(__ANDROID__)
             && !(lib::is_mysql_mode()
                  && (ob_is_null(type1.get_type()) || ob_is_null(type2.get_type())))
#endif
...
#if defined(__ANDROID__)
      ret = (lib::is_mysql_mode() && (type_ == T_OP_EQ || type_ == T_OP_NE))
                ? OB_INVALID_ARGUMENT
                                                   ...
                                                   common::ObExprTypeCtx &type_ctx)
{
  int ret = OB_SUCCESS;
  need_no_cast = false;
  const sql::ObSQLSessionInfo *my_session = NULL;
  CK (OB_NOT_NULL(my_session = type_ctx.get_session()));
                                             ...
                                             ObExprTypeCtx &type_ctx) const
{
  ...
  if (OB_SUCC(calc_cmp_type2(cmp_type, type1, type2, type_ctx, left_is_const,
                                    right_is_const))) {
...
int ObBitwiseExprOperator::cg_bitwise_expr(ObExprCGCtx &expr_cg_ctx, const ObRawExpr &raw_expr,
