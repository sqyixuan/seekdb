...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(omt::ObTenantNodeBalancer::get_instance().notify_create_tenant())) {
    LOG_WARN("fail to handle notify unit resource", KR(ret));
  }
  BOOTSTRAP_CHECK_SUCCESS();
  return ret;
}
...
int ObBootstrap::execute_bootstrap()
{
  ...
  // TODO@jingyu_cr: delete this function after obd ready
  if (FAILEDx(insert_all_tenant_table_())) {
    LOG_WARN("fail to insert __all_tenant table", KR(ret));
  }
...
int ObBootstrap::insert_all_tenant_table_()
{
  int ret = OB_SUCCESS;
  ObDMLSqlSplicer dml;
  char svr_ip[OB_MAX_SERVER_ADDR_SIZE] = "";
  char build_version[common::OB_SERVER_VERSION_LENGTH] = {'\0'};
  ObBuildVersion build_version_string;
  dml.reset();
  int64_t sql_port = 0;
  if (!GCTX.self_addr().ip_to_string(svr_ip, sizeof(svr_ip))) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret));
  } else if (OB_ISNULL(GCTX.config_) || OB_ISNULL(GCTX.sql_proxy_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), KP(GCTX.config_), KP(GCTX.sql_proxy_));
  ...
  } else if (FALSE_IT(sql_port = GCTX.config_->mysql_port)) {
  } else if (OB_FAIL(dml.add_pk_column(K(svr_ip)))
        || OB_FAIL(dml.add_pk_column("svr_port", GCTX.self_addr().get_port()))
        || OB_FAIL(dml.add_column("id", 1))
        || OB_FAIL(dml.add_column("zone", GCTX.config_->zone.str()))
        || OB_FAIL(dml.add_column("inner_port", sql_port))
        || OB_FAIL(dml.add_column("block_migrate_in_time", 0))
        || OB_FAIL(dml.add_column("with_partition", true))
        || OB_FAIL(dml.add_column("with_rootserver", true)
        || OB_FAIL(dml.add_column("start_service_time", ObTimeUtility::current_time()))
        || OB_FAIL(dml.add_column("status", "ACTIVE"))
        || OB_FAIL(dml.add_column("build_version", build_version_string))
        || OB_FAIL(dml.add_column("stop_time", 0))
        || OB_FAIL(dml.add_column("last_offline_time", 0)))) {
    ...
    if (OB_FAIL(ObRootUtils::get_rs_default_timeout_ctx(ctx))) {
      ...
      ObDMLExecHelper exec(*GCTX.sql_proxy_, OB_SYS_TENANT_ID);
      int64_t affected_rows = 0;
      if (OB_FAIL(exec.exec_insert(OB_ALL_SERVER_TNAME, dml, affected_rows))) {
        LOG_WARN("fail to exec update", KR(ret));
...
int ObBootstrap::create_sys_unit_config()
{
  ...
  } else if (OB_FAIL(init_sys_unit_config(config))) {
...
{
  int ret = OB_SUCCESS;
  // init __all_resource_pool
  ObDMLSqlSplicer pool_dml;
  if (OB_ISNULL(GCTX.config_) || OB_ISNULL(GCTX.sql_proxy_)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), KP(GCTX.config_), KP(GCTX.sql_proxy_));
  } else if (OB_FAIL(pool_dml.add_pk_column("resource_pool_id", 1))
          || OB_FAIL(pool_dml.add_column("name", "sys_pool"))
          || OB_FAIL(pool_dml.add_column("unit_count", 1))
          || OB_FAIL(pool_dml.add_column("unit_config_id", 1))
          || OB_FAIL(pool_dml.add_column("zone_list", GCTX.config_->zone.str()))
          || OB_FAIL(pool_dml.add_column("tenant_id", OB_SYS_TENANT_ID))
          || OB_FAIL(pool_dml.add_column("replica_type", 0))
          || OB_FAIL(pool_dml.add_gmt_modified())) {
    LOG_WARN("add column failed", K(ret));
  } else {
    ObDMLExecHelper exec(*GCTX.sql_proxy_, OB_SYS_TENANT_ID);
    int64_t affected_rows = 0;
    if (OB_FAIL(exec.exec_insert_update(OB_ALL_RESOURCE_POOL_TNAME, pool_dml, affected_rows))) {
  ...
  if (OB_FAIL(ret)) {
  } else if (false == GCTX.self_addr().ip_to_string(ip, sizeof(ip))) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("convert server ip to string failed", K(ret));
  } else if (OB_FAIL(dml.add_pk_column("unit_id", 1))
        || OB_FAIL(dml.add_column("resource_pool_id", 1))
        || OB_FAIL(dml.add_column("unit_group_id", 1))
        || OB_FAIL(dml.add_column("zone", GCTX.config_->zone.str()))
        || OB_FAIL(dml.add_column("svr_ip", ip))
        || OB_FAIL(dml.add_column("svr_port", GCTX.self_addr().get_port()))
        || OB_FAIL(dml.add_column(K(migrate_from_svr_ip)))
        || OB_FAIL(dml.add_column("migrate_from_svr_port", 0))
        || OB_FAIL(dml.add_gmt_modified())
        || OB_FAIL(dml.add_column("manual_migrate", false))
        || OB_FAIL(dml.add_column("status", "ACTIVE"))) {
      LOG_WARN("add column failed", K(ret));
  } else {
    ObDMLExecHelper exec(*GCTX.sql_proxy_, OB_SYS_TENANT_ID);
    ...
    common::ObISQLClient &sql_client)
{
  ...
  if (OB_ISNULL(GCTX.config_)) {
  ...
  } else if (OB_FAIL(zone_info.region_.info_.assign(DEFAULT_REGION_NAME))) {
    LOG_WARN("fail assign default region info", K(ret), K(zone_info));
  } else if (OB_FAIL(zone_info.status_.info_.assign(
             ObString(ObZoneStatus::get_status_str(ObZoneStatus::ACTIVE))))) {
    LOG_WARN("fail to assign zone status str", KR(ret));
  } else if (OB_FAIL(zone_info.storage_type_.info_.assign(
    ...
    zone_info.zone_ = GCTX.config_->zone.str();
  }
  if (OB_SUCC(ret)) {
...
{
  // TODO@jingyu.cr: remove this function after OBD ready
  int ret = OB_SUCCESS;
