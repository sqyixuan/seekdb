    ...
    const share::ObFreezeInfo &freeze_info)
{
  ...
  if (OB_UNLIKELY(!freeze_info.is_valid())) {
    ...
    LOG_INFO("success to set basic info", KR(ret), "compaction_scn", get_compaction_scn());
  }
  return ret;
}
...
int ObMajorMergeProgressChecker::check_table_merge_progress(
    ObSchemaGetterGuard &schema_guard,
    ObIArray<uint64_t> &unfinish_table_id_array)
{
  int ret = OB_SUCCESS;
  ...
  SMART_VAR(ObArray<share::ObTabletLSPair>, cur_tablet_ls_pair_array) {
    for ( ; OB_SUCC(ret) && !stop_ && idx < table_ids_.count(); ++idx) {
      ...
      if (OB_FAIL(ret)) {
      } else if (table_compaction_info.is_verified() || table_compaction_info.can_skip_verifying()) {
        // do nothing
    ...
    if (OB_SUCC(ret)) {
      table_ids_.batch_start_idx_ = idx + 1; // record next untouched tablet
    } else {
      table_ids_.batch_start_idx_ = idx; // record first failed table, need check in next loop
    }
  }
  return ret;
}
