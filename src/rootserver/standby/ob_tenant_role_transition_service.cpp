...
int ObTenantRoleTransitionService::failover_to_primary()
{
  ...
  if (OB_SUCC(ret) && !tenant_info.is_primary()) {
    // Removed recovery_until_scn check - recovery not supported in single tenant/single LS scenario
    if (tenant_info.is_normal_status()) {
      if (OB_FAIL(do_failover_to_primary_(tenant_info))) {
        ...
        // After do_failover_to_primary_ succeeds, tenant_info is updated to SWITCHING_TO_PRIMARY
        // Continue to switch access mode
      ...
      // Removed flashback related code - flashback not supported
...
int ObTenantRoleTransitionService::do_failover_to_primary_(share::ObAllTenantInfo &tenant_info)
{
  ...
  **/
  if (OB_FAIL(ret) || is_verify_) {
    // Removed clear_service_name_ for single tenant/single LS scenario
  } else {
    // Update tenant status (sync_scn removed, no longer stored in ObAllTenantInfo)
    ...
    if (OB_SUCC(ret) && OB_FAIL(ObAllTenantInfoProxy::load_tenant_info(
        false, tenant_info))) {
      LOG_WARN("failed to load tenant info", KR(ret));
    }
  }
  // Removed do_prepare_flashback_ call - flashback not supported
  return ret;
}

// Removed do_prepare_flashback_ implementation - flashback not supported in single tenant/single LS scenario
// Removed all flashback related functions - flashback not supported in single tenant/single LS scenario
// Removed clear_service_name_ and double_check_service_name_ implementations
// for single tenant/single LS scenario - ObServiceName functionality not needed
    ...
    if (OB_ISNULL(ls_service)) {
      ...
      if (OB_ISNULL(sys_ls)) {
        ...
        if (OB_ISNULL(log_handler)) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("log_handler is null", KR(ret));
        } else if (OB_FAIL(log_handler->get_max_decided_scn(cur_sync_scn))) {
          LOG_WARN("get_max_decided_scn failed", KR(ret));
        } else {
          share::SCN restore_source_next_scn = share::SCN::min_scn();
          if (check_sync_to_latest) {
            if (OB_FAIL(log_handler->get_end_scn(end_scn))) {
              LOG_WARN("get_end_scn failed", KR(ret));
            } else {
              is_sync_to_latest = (cur_sync_scn >= end_scn);
              // Set restore_source_next_scn to end_scn so that is_sync_to_latest() returns true
              // when cur_sync_scn >= end_scn
              restore_source_next_scn = end_scn;
            }
          } else {
            // If not checking sync to latest, set restore_source_next_scn to cur_sync_scn
            // so that is_sync_to_latest() returns true
            restore_source_next_scn = cur_sync_scn;
          }
          if (OB_SUCC(ret)) {
            checkpoint = obrpc::ObCheckpoint(share::SYS_LS, cur_sync_scn, restore_source_next_scn);
