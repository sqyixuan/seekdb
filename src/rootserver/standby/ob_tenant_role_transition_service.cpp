      ...
      if (OB_ISNULL(sys_ls)) {
        ...
        if (OB_ISNULL(log_handler)) {
          ...
          if (check_sync_to_latest) {
            // Call ObService::get_ls_sync_scn to get the primary's end_scn
            // via restore handler, instead of comparing against local end_scn
            obrpc::ObGetLSSyncScnArg arg;
            obrpc::ObGetLSSyncScnRes res;
            if (OB_ISNULL(GCTX.ob_service_)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("ob_service_ is null", KR(ret));
            } else if (OB_FAIL(arg.init(OB_SYS_TENANT_ID, share::SYS_LS, true /*check_sync_to_latest*/))) {
              ...
              restore_source_next_scn = res.get_cur_restore_source_next_scn();
            }
          } else {
