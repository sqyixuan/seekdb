                        ...
                        ObMultiVersionSchemaService *schema_service)
{
  ...
  if (FAILEDx(task_queue_.init(
  ...
  } else if (OB_FAIL(server_manager_.init(server_change_callback_, sql_proxy_,
    ...
    FLOG_WARN("init ddl_service_ failed", KR(ret));
          ...
          common_proxy_, sql_proxy_, *schema_service, zone_manager_))) {
    // init tenant ddl service
    FLOG_WARN("init tenant_ddl_service_ failed", KR(ret));
  } else if (OB_FAIL(unit_manager_.init(*GCTX.sql_proxy_, *config_, *GCTX.srv_rpc_proxy_, *GCTX.schema_service_, *this))) {
    // init unit manager
