...
#include "share/schema/ob_standby_schema_refresh_trigger.h"  // ObStandbySchemaRefreshTrigger
...
int ObService::start(bool embed_mode)
{
  ...
  if (!inited_) {
    ...
    // Standby cluster: skip bootstrap, create sys tenant if needed, setup log sync
    if (OB_FAIL(bootstrap_standby())) {
      LOG_ERROR("bootstrap standby failed", KR(ret));
    }
  } else if (need_bootstrap_) {
    // Primary cluster: normal bootstrap
...
void ObService::stop()
{
  ...
  if (!inited_) {
    ...
    if (GCTX.is_standby_cluster()) {
      FLOG_INFO("begin to stop schema refresh trigger");
      schema_refresh_trigger_.stop();
      FLOG_INFO("schema refresh trigger stopped");
...
void ObService::wait()
{
  ...
  if (!inited_) {
    ...
    if (GCTX.is_standby_cluster()) {
      FLOG_INFO("begin to wait schema refresh trigger");
      schema_refresh_trigger_.wait();
      FLOG_INFO("wait schema refresh trigger success");
...
int ObService::destroy()
{
  ...
  if (!inited_) {
    ...
    if (GCTX.is_standby_cluster()) {
      FLOG_INFO("begin to destroy schema refresh trigger");
      schema_refresh_trigger_.destroy();
      FLOG_INFO("schema refresh trigger destroyed");
...
int ObService::bootstrap_standby()
{
  ...
  // 3. Create sys tenant log stream if not exists (empty LS for standby)
  // 4. Setup log sync from primary

  if (!GCTX.is_standby_cluster()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_ERROR("bootstrap_standby called but not standby cluster", KR(ret));
  } else if (!GCTX.get_restore_source().is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_ERROR("restore_source is not set for standby cluster", KR(ret));
  } else {
    ...
    MTL_SWITCH(OB_SYS_TENANT_ID) {
      if (OB_FAIL(create_sys_ls())) {
        ...
        const common::ObAddr &primary_addr = GCTX.get_restore_source();
        ...
        if (!GCTX.is_standby_cluster()) {
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("setup_standby_log_sync called but not standby cluster", KR(ret));
    ...
    // Setup log sync from primary
    if (OB_FAIL(ret)) {
      // Error already logged
    } else if (OB_FAIL(setup_standby_log_sync(GCTX.get_restore_source()))) {
      LOG_ERROR("setup standby log sync failed", KR(ret));
    }

    // Start schema refresh trigger
    if (OB_FAIL(ret)) {
      // Error already logged
    } else if (OB_FAIL(start_schema_refresh())) {
      LOG_ERROR("start schema refresh failed", KR(ret));
    }
  }
...
int ObService::create_sys_ls()
{
  ...
  if (!GCTX.is_standby_cluster()) {
    ...
    if (OB_FAIL(ls_service->create_ls_for_restore())) {
      LOG_WARN("create ls failed", KR(ret));
    } else {
      log_service->update_replayable_point(SCN::max_scn());
    }
  }
...
int ObService::start_schema_refresh()
{
  int ret = OB_SUCCESS;

  if (!GCTX.is_standby_cluster()) {
    // Only start schema refresh for standby cluster
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("start_schema_refresh called but not standby cluster", KR(ret));
  } else if (OB_ISNULL(gctx_.schema_service_) || OB_ISNULL(gctx_.sql_proxy_)) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("schema_service or sql_proxy is null",
             KP(gctx_.schema_service_), KP(gctx_.sql_proxy_), KR(ret));
  } else if (OB_FAIL(schema_refresh_trigger_.start())) {
    LOG_WARN("start schema refresh trigger failed", KR(ret));
  } else {
    LOG_INFO("schema refresh trigger started successfully");
  }
                                          ...
                                          share::ObRestoreSourceServiceAttr &source_attr)
{
  ...
  // For debugging phase, build a simplified ObRestoreSourceServiceAttr
  // Format: ip_list=ip:port,USER=root@sys,TENANT_ID=1,CLUSTER_ID=1,COMPATIBILITY_MODE=MYSQL,IS_ENCRYPTED=false
  // For now, we use default values for user/password, which will need to be configured properly later
  ...
  if (!primary_addr.is_valid()) {
    ...
    if (OB_FAIL(source_attr.addr_.push_back(primary_addr))) {
      ...
      // Set default user info for debugging phase
      // TODO: These should be configurable via command line or config file
      const char *default_user = "root";
      const char *default_tenant = "sys";
      const uint64_t default_tenant_id = OB_SYS_TENANT_ID;
      const int64_t default_cluster_id = 1; // TODO: Get from config

      if (OB_FAIL(source_attr.set_service_user(default_user, default_tenant))) {
        LOG_WARN("failed to set service user", K(ret));
      } else if (OB_FAIL(source_attr.set_service_tenant_id(std::to_string(default_tenant_id).c_str()))) {
        LOG_WARN("failed to set service tenant id", K(ret));
      } else if (OB_FAIL(source_attr.set_service_cluster_id(std::to_string(default_cluster_id).c_str()))) {
        LOG_WARN("failed to set service cluster id", K(ret));
      } else if (OB_FAIL(source_attr.set_service_compatibility_mode("MYSQL"))) {
        LOG_WARN("failed to set compatibility mode", K(ret));
      } else if (OB_FAIL(source_attr.set_service_passwd_no_encrypt("root"))) {
        // Default password "root" for debugging phase
        // TODO: Make this configurable via command line or config file
        LOG_WARN("failed to set service password", K(ret));
...
int ObService::setup_standby_log_sync(const common::ObAddr &primary_addr)
{
  int ret = OB_SUCCESS;
  ...
  if (!GCTX.is_standby_cluster()) {
    ...
    // 1. Build ObRestoreSourceServiceAttr from primary_addr
    share::ObRestoreSourceServiceAttr source_attr;
    if (OB_FAIL(build_restore_source_attr(primary_addr, source_attr))) {
      LOG_WARN("fail to build restore source attr", K(ret), K(primary_addr));
    } else {
      // 2. Switch to sys tenant context before accessing MTL services
      // For standby, it always fetches logs to infinite point (no recovery_until_scn limit)
      MTL_SWITCH(OB_SYS_TENANT_ID) {
        ...
        if (OB_ISNULL(log_service)) {
          ...
          if (OB_ISNULL(restore_service)) {
            ...
            logservice::ObLogRestoreNetDriver *net_driver = restore_service->get_net_driver();
            if (OB_ISNULL(net_driver)) {
              ret = OB_ERR_UNEXPECTED;
              LOG_WARN("net_driver is null", KR(ret));
            } else if (OB_FAIL(net_driver->set_source(source_attr))) {
              // Set source without blocking (non-blocking)
              // The restore_service background thread will call do_schedule() later
              LOG_WARN("set_source failed", KR(ret), K(source_attr));
            } else {
              // // Wake up the background thread to process the source immediately
              // restore_service->signal();
              // LOG_INFO("log restore source set successfully, background thread will schedule",
              //          K(source_attr), K(primary_addr));
            }
          }
        }
      }
  ...
  if (OB_SUCC(ret)) {
    ...
    // scn get order, sync_scn from leader before archive_scn
      ...
      LOG_WARN("failed to get ls cur_sync_scn", KR(ret), K(ls_id), KPC(ls));
               ...
               && FALSE_IT(cur_restore_source_max_scn = cur_sync_scn)) {
