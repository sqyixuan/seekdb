...
int ObHSeriesAdapter::put(ObTableExecCtx &ctx, const ObITableEntity &cell)
{
  ...
  SMART_VAR(ObTableCtx, tb_ctx, allocator_)
  {
    ...
    } else if (OB_FAIL(convert_normal_to_series(cell, series_cell))) {
      LOG_WARN("fail to convert normal to series", K(ret), K(cell));
    } else if (OB_FAIL(ObTableApiService::insert(tb_ctx, series_cell, result))) {        // lob can not use put
...
int ObHSeriesAdapter::multi_put(ObTableExecCtx &ctx, const ObIArray<ObITableEntity *> &cells)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    SMART_VAR(ObTableCtx, tb_ctx, allocator_)
    {
      ...
      } else if (OB_FAIL(convert_normal_to_series(cells, series_cells, real_tablet_ids))) {
  ...
  if (OB_FAIL(ret)) {
    ...
    for (KTAggMap::const_iterator i = kt_agg_map_.begin(); OB_SUCC(ret) && i != kt_agg_map_.end(); ++i) {
      ...
      if (OB_ISNULL(series_cell = OB_NEWx(ObTableEntity, (&allocator_)))) {
        ...
        if (OB_FAIL(ret)) {
        } else if (OB_FAIL(construct_series_value(allocator_, json_obj, value_obj))) {
