 ...
 */
 ...
 {
 public:
   ObOldMergeTableQueryResultIterator(const ObTableQuery& query, table::ObTableQueryResult &one_result);
   ObOldMergeTableQueryResultIterator(const ObTableQuery& query, table::ObTableQueryIterableResult &one_result);
   TO_STRING_KV(KP_(serlize_result), KP_(iterable_result), K_(binary_heap));
   virtual ~ObOldMergeTableQueryResultIterator()
   {
