...
int ObServer::init(const ObServerOptions &opts, const ObPLogWriterCfg &log_cfg)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    } else if (OB_FAIL(parse_server_role_and_restore_source(opts))) {
      LOG_ERROR("parse server role and restore source failed", KR(ret));
...
int ObServer::start(bool embed_mode)
{
  ...
  LOG_DBA_INFO_V2(OB_SERVER_START_BEGIN,
    ...
    if (config_.enable_rpc_service) {
      grpc_server_.register_service(&storage_grpc_service_impl_);
      grpc_server_.register_service(&log_service_grpc_impl_);
      if (FAILEDx(grpc_server_.start(config_.rpc_port))) {
        LOG_ERROR("fail to start grpc server", K(ret));
      } else {
        FLOG_INFO("success to start grpc server");
      }
    }
...
int ObServer::check_if_schema_ready()
{
  ...
  while (!stop_ && !schema_ready) {
    ret = OB_SUCCESS;
    if (OB_FAIL(schema_service_.get_baseline_schema_version(OB_SYS_TENANT_ID, true/*auto_update*/, baseline_schema_version))) {
    ...
    } else if (OB_FAIL(schema_service_.get_tenant_refreshed_schema_version(OB_SYS_TENANT_ID, current_schema_version))) {
...
int ObServer::init_config(const ObServerOptions &opts)
{
  ...
  const char *meta_db_path = "./etc/meta.db";
  if (OB_FAIL(meta_db_pool_.init(meta_db_path))) {
...
int ObServer::parse_server_role_and_restore_source(const ObServerOptions &opts)
{
  int ret = OB_SUCCESS;

  // Parse server_role
  if (opts.server_role_.empty()) {
    // Default to PRIMARY
    gctx_.server_role_ = common::PRIMARY_CLUSTER;
    LOG_INFO("server_role not specified, default to PRIMARY");
  } else {
    common::ObString role_str(opts.server_role_.length(), opts.server_role_.ptr());
    if (0 == role_str.case_compare("PRIMARY")) {
      gctx_.server_role_ = common::PRIMARY_CLUSTER;
    } else if (0 == role_str.case_compare("PHYSICAL_STANDBY")) {
      gctx_.server_role_ = common::STANDBY_CLUSTER;
    } else {
      ret = OB_INVALID_ARGUMENT;
      LOG_ERROR("invalid server_role", K(opts.server_role_));
    }
  }

  // Parse restore_source (required for standby)
  if (OB_SUCC(ret)) {
    if (common::STANDBY_CLUSTER == gctx_.server_role_) {
      if (opts.restore_source_.empty()) {
        ret = OB_INVALID_ARGUMENT;
        LOG_ERROR("restore_source is required when server_role=PHYSICAL_STANDBY");
      } else {
        // Parse restore_source as "ip:port"
        common::ObString source_str(opts.restore_source_.length(), opts.restore_source_.ptr());
        if (OB_FAIL(gctx_.restore_source_.parse_from_string(source_str))) {
          LOG_ERROR("failed to parse restore_source", K(opts.restore_source_), K(ret));
        } else {
          LOG_INFO("parsed restore_source", K(gctx_.restore_source_));
        }
      }
    } else {
      // PRIMARY doesn't need restore_source
      gctx_.restore_source_.reset();
    }
  }

  LOG_INFO("server role and restore source parsed",
           "server_role", gctx_.server_role_ == common::PRIMARY_CLUSTER ? "PRIMARY" : "STANDBY",
           K(gctx_.restore_source_));
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(OB_PX_TARGET_MGR.init(self_addr_))) {
