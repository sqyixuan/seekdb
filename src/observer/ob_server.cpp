...
int ObServer::start(bool embed_mode)
{
  ...
  if (is_arbitration_mode()) {
#ifdef OB_BUILD_ARBITRATION
    if (OB_FAIL(start_server_in_arb_mode())) {
...
int ObServer::init_pre_setting()
{
  ...
  if (OB_SUCC(ret)) {
    const int64_t stack_size = std::max(static_cast<int64_t>(1L << 19), static_cast<int64_t>(GCONF.stack_size));
