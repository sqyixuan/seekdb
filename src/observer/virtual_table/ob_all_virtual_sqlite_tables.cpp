...
int ObAllVirtualTabletMetaTable::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, tablet_id, svr_ip, svr_port, gmt_create, gmt_modified, compaction_scn, data_size, required_size, report_scn, status "
...
int ObAllVirtualTabletReplicaChecksum::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, tablet_id, svr_ip, svr_port, compaction_scn, row_count, data_checksum, column_checksums, b_column_checksums, data_checksum_type, co_base_snapshot_version "
