...
int ObAllVirtualDeadlockEventHistory::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, event_id, svr_ip, svr_port, detector_id, report_time, cycle_idx, cycle_size, role, priority_level, priority, create_time, start_delay, module, visitor, object, extra_name1, extra_value1, extra_name2, extra_value2, extra_name3, extra_value3 "
...
int ObAllVirtualDeadlockEventHistory::get_next_row_from_sqlite()
{
  ...
  if (OB_ISNULL(stmt_)) {
    ...
    ret = guard_->step_query(stmt_, reader);
    if (OB_SUCC(ret)) {
      tenant_id_ = reader.get_int64();
      event_id_ = reader.get_int64();
      int svr_ip_len = 0;
      const char *svr_ip_str = reader.get_text(&svr_ip_len);
      if (OB_ISNULL(svr_ip_str)) {
        svr_ip_.reset();
      } else {
        svr_ip_.assign_ptr(svr_ip_str, svr_ip_len);
      }
      svr_port_ = reader.get_int64();
...
int ObAllVirtualTabletMetaTable::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, tablet_id, ls_id, svr_ip, svr_port, gmt_create, gmt_modified, compaction_scn, data_size, required_size, report_scn, status "
...
int ObAllVirtualTabletReplicaChecksum::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, tablet_id, svr_ip, svr_port, ls_id, compaction_scn, row_count, data_checksum, column_checksums, b_column_checksums, data_checksum_type, co_base_snapshot_version "
...
int ObAllVirtualZoneMergeInfo::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, zone, all_merged_scn, broadcast_scn, frozen_scn, is_merging, last_merged_time, last_merged_scn, merge_start_time, merge_status "
...
int ObAllVirtualZoneMergeInfo::get_next_row_from_sqlite()
{
  ...
  if (OB_ISNULL(stmt_)) {
    ...
    if (OB_SUCC(ret)) {
      tenant_id_ = reader.get_int64();
      int zone_len = 0;
      const char *zone_str = reader.get_text(&zone_len);
      if (OB_ISNULL(zone_str)) {
        zone_.reset();
      } else {
        zone_.assign_ptr(zone_str, zone_len);
      }
...
int ObAllVirtualMergeInfo::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, cluster, frozen_scn, global_broadcast_scn, is_merge_error, last_merged_scn, merge_status, error_type, suspend_merging, merge_start_time, last_merged_time "
...
int ObAllVirtualColumnChecksumErrorInfo::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, frozen_scn, index_type, data_table_id, index_table_id, data_tablet_id, index_tablet_id, column_id, data_column_checksum, index_column_checksum "
...
int ObAllVirtualReservedSnapshot::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, snapshot_type, svr_ip, svr_port, create_time, snapshot_version, status "
...
int ObAllVirtualServerEventHistory::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT gmt_create, event_type, svr_ip, svr_port, module, event, name1, value1, name2, value2, name3, value3, name4, value4, name5, value5, name6, value6, extra_info "
...
int ObAllVirtualTenantEventHistory::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, gmt_create, svr_ip, svr_port, module, event, name1, value1, name2, value2, name3, value3, name4, value4, name5, value5, name6, value6, extra_info, trace_id, cost_time, ret_code, error_msg "
...
int ObAllVirtualRootserviceJob::get_next_row_from_sqlite()
{
  ...
  if (OB_ISNULL(stmt_)) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(ret)) {
      } else if (OB_ISNULL(job_type_str)) {
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("invalid argument", KR(ret), KP(job_type_str));
      } else {
      ...
      if (OB_FAIL(ret)) {
      } else if (OB_ISNULL(job_status_str)) {
        ret = OB_INVALID_ARGUMENT;
        LOG_WARN("invalid argument", KR(ret), KP(job_status_str));
      } else {
