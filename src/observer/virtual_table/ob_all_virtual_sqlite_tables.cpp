...
int ObAllVirtualZoneMergeInfo::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, all_merged_scn, broadcast_scn, frozen_scn, is_merging, last_merged_time, last_merged_scn, merge_start_time, merge_status "
...
int ObAllVirtualMergeInfo::inner_open()
{
  ...
  if (!guard) {
    ...
    guard_ = std::move(guard);
    const char *select_sql =
      "SELECT tenant_id, frozen_scn, global_broadcast_scn, is_merge_error, last_merged_scn, merge_status, error_type, suspend_merging, merge_start_time, last_merged_time "
