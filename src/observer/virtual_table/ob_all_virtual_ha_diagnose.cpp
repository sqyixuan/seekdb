  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < count; i++) {
    ...
    switch (col_id) {
      ...
      case REPLAY_DIAGNOSE_INFO:
        ...
        break;
      case GC_STATE:
        cur_row_.cells_[i].set_varchar(ObString::make_string("<NOT AVAILABLE>"));
        cur_row_.cells_[i].set_collation_type(ObCharset::get_default_collation(
                                              ObCharset::get_default_charset()));
        break;
      case GC_START_TS:
      ...
      case RESTORE_HANDLER_ROLE:
        cur_row_.cells_[i].set_varchar(ObString::make_string(""));
        cur_row_.cells_[i].set_collation_type(ObCharset::get_default_collation(
                                                ObCharset::get_default_charset()));
        break;
      case RESTORE_HANDLER_PROPOSAL_ID:
        cur_row_.cells_[i].set_int(palf::INVALID_PROPOSAL_ID);
        break;
