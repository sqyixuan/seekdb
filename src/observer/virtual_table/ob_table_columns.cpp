...
#endif
    // construct sql
    ObSqlString view_select_sql;
    if (OB_FAIL(view_select_sql.append_fmt(is_oracle_mode
                                        ? "select * from \"%.*s\".\"%.*s\""
                                        : "select * from `%.*s`.`%.*s`",
                                      db_name.length(),
                                      db_name.ptr(),
      ...
      if (OB_FAIL(parser.parse(view_select_sql.ptr(), parse_result))) {
        LOG_WARN("parse view definition failed", K(view_select_sql), K(ret));
      } else {
