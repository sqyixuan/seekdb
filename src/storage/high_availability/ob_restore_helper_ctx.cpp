...
{
  return RESTORE_HELPER_CTX_LS_VIEW;
}

bool ObRestoreHelperLSViewCtx::is_valid() const
{
  return RESTORE_HELPER_CTX_LS_VIEW == get_type();
}
...
void ObRestoreHelperLSViewCtx::reset()
{
  ...
  if (OB_NOT_NULL(tablet_info_reader_)) {
    delete tablet_info_reader_;
    tablet_info_reader_ = NULL;
  }
...
{
  return RESTORE_HELPER_CTX_SSTABLE_INFO;
}

bool ObRestoreHelperSSTableInfoCtx::is_valid() const
{
  return RESTORE_HELPER_CTX_SSTABLE_INFO == get_type() &&
...
void ObRestoreHelperSSTableInfoCtx::reset()
{
  ...
  if (OB_NOT_NULL(sstable_info_reader_)) {
    delete sstable_info_reader_;
    sstable_info_reader_ = NULL;
  }
...
{
  return RESTORE_HELPER_CTX_SSTABLE_MACRO_RANGE;
}
...
{
  if (OB_NOT_NULL(macro_info_reader_)) {
    delete macro_info_reader_;
    macro_info_reader_ = NULL;
  }
...
{
  return RESTORE_HELPER_CTX_MACRO_BLOCK;
}
...
{
  if (OB_NOT_NULL(macro_block_reader_)) {
    delete macro_block_reader_;
    macro_block_reader_ = NULL;
  }
    ...
    ObIRestoreHelperCtx *&ctx)
{
  ...
  void *buf = nullptr;
  ctx = nullptr;
  if (OB_UNLIKELY(ctx_type <= RESTORE_HELPER_CTX_NONE || ctx_type >= RESTORE_HELPER_CTX_MAX)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid ctx type", K(ret), "ctx_type", static_cast<int64_t>(ctx_type));
  } else {
    switch (ctx_type) {
      case RESTORE_HELPER_CTX_LS_VIEW: {
      ...
      case RESTORE_HELPER_CTX_SSTABLE_INFO: {
      ...
      case RESTORE_HELPER_CTX_SSTABLE_MACRO_RANGE: {
      ...
      case RESTORE_HELPER_CTX_MACRO_BLOCK: {
