...
void ObRestoreHelperLSViewCtx::reset()
{
  ...
  if (OB_NOT_NULL(tablet_info_reader_)) {
    if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(tablet_info_reader_, status))) {
      LOG_ERROR("failed to close reader", K(ret), K(status.error_code()), "error_message", status.error_message().c_str());
    } else {
      ret = grpc_client_->translate_error(status);
      if (OB_FAIL(ret)) {
...
void ObRestoreHelperSSTableInfoCtx::reset()
{
  ...
  if (OB_NOT_NULL(sstable_info_reader_)) {
    if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(sstable_info_reader_, status))) {
      LOG_ERROR("failed to close reader", K(ret), K(status.error_code()), "error_message", status.error_message().c_str());
    } else {
      ret = grpc_client_->translate_error(status);
      if (OB_FAIL(ret)) {
...
void ObRestoreHelperSSTableMacroRangeCtx::reset()
{
  ...
  if (OB_NOT_NULL(macro_info_reader_)) {
    if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(macro_info_reader_, status))) {
      LOG_ERROR("failed to close reader", K(ret), K(status.error_code()), "error_message", status.error_message().c_str());
    } else {
      ret = grpc_client_->translate_error(status);
      if (OB_FAIL(ret)) {
...
void ObRestoreHelperMacroBlockCtx::reset()
{
  ...
  if (OB_NOT_NULL(macro_block_reader_)) {
    if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(macro_block_reader_, status))) {
      LOG_ERROR("failed to close reader", K(ret), K(status.error_code()), "error_message", status.error_message().c_str());
    } else {
      ret = grpc_client_->translate_error(status);
      if (OB_FAIL(ret)) {
