...
int ObCompleteRestoreDagNet::clear_dag_net_ctx()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_TMP_FAIL(update_restore_status_(ls))) {
      LOG_WARN("failed to finalize update restore status", K(ret), KPC(&ctx_));
    } 
    // if (OB_TMP_FAIL(report_ls_meta_table_(ls))) {
    //   LOG_WARN("failed to finalize report ls meta table", K(ret), KPC(&ctx_));
    // }
...
int ObCompleteRestoreDagNet::update_restore_status_(ObLS *ls)
{
  ...
  static const int64_t UPDATE_RESTORE_STATUS_INTERVAL_MS = 100 * 1000; // 100ms
  ObTenantDagScheduler *scheduler = nullptr;
  int32_t result = OB_SUCCESS;
  ...
  if (IS_NOT_INIT) {
    ...
    while (!is_finish) {
      ...
      if (ls->is_stopped()) {
        ...
        } else if (OB_FAIL(get_next_complete_status_(ls, current_restore_status, next_restore_status))) {
        ...
        if (OB_FAIL(ret)) {
          ...
          LOG_WARN("failed to set restore status", K(ret),
              K(current_restore_status), K(next_restore_status), K(result), KPC(&ctx_), KPC(ls));
        } else {
          is_finish = true;
        }
      }
      ...
      if (OB_FAIL(ret)) {
        ob_usleep(UPDATE_RESTORE_STATUS_INTERVAL_MS);
      }
    }
  }
...
int ObCompleteRestoreDagNet::deal_with_cancel()
{
  ...
  if (IS_NOT_INIT) {
    ret = OB_NOT_INIT;
    LOG_WARN("complete restore dag net do not init", K(ret));
  } else if (OB_FAIL(reinterpret_cast<ObIHADagNetCtx *>(&ctx_)->set_result(result, need_retry))) {
    LOG_WARN("failed to set complete restore ctx result", K(ret), KPC(&ctx_));
  }
  return ret;
}
    ...
    ObRestoreStatus &next_complete_status)
{
  ...
  if (OB_ISNULL(ls)) {
    ...
    ObRestoreStatus::Status current_status = current_complete_status;
    if (ctx_.is_failed()) {
      next_complete_status = ObRestoreStatus::Status::RESTORE_FAILED;
      LOG_WARN("restore task failed, transition to RESTORE_FAILED", K(current_status), KPC(&ctx_));
    } else {
...
int ObCompleteRestoreDagNet::report_ls_meta_table_(ObLS *ls)
{
  int ret = OB_SUCCESS;
  //int ret = OB_NOT_SUPPORTED;
  //TODO(xingzhi): add restore status
  return ret;
}

int ObCompleteRestoreDagNet::report_result_()
{
  int ret = OB_SUCCESS;
  //int ret = OB_NOT_SUPPORTED;
  // TODO(xingzhi): report result
  return ret;
}
...
int ObWaitDataReadyRestoreTask::process()
{
  ...
  // } else if (OB_FAIL(update_ls_to_standby_())) {
  //   LOG_WARN("failed to update ls to standby serving state", K(ret), KPC(ctx_));
  }
...
int ObWaitDataReadyRestoreTask::update_ls_to_standby_()
{
  int ret = OB_SUCCESS;
  ObLS *ls = nullptr;
  // Set restore status to NONE to indicate restore is complete
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_WARN("failed to set restore status to NONE", K(ret), KPC(ls));
  } else {
    // Enable read and set LS to work state for standby serving
    ls->enable_to_read();
    if (OB_FAIL(ls->set_start_work_state())) {
      LOG_WARN("failed to set start work state", K(ret), KPC(ls));
    } else {
      LOG_INFO("successfully updated ls to standby serving state",
