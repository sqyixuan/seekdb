...
bool ObRestoreDagNetInitParam::is_valid() const
{
  return task_.is_valid() && OB_NOT_NULL(handler_)
                          && OB_NOT_NULL(bandwidth_throttle_) && OB_NOT_NULL(sql_proxy_);
}
  ...
  if (OB_NOT_NULL(helper_) || OB_ISNULL(ctx_) || !ctx_->is_valid()) {
    ...
    if (ObRestoreTaskType::STANDBY_RESTORE_TASK == task_type) {
      ...
      if (OB_ISNULL(buf = ctx_->allocator_.alloc(sizeof(ObStandbyRestoreHelper)))) {
        ...
        if (OB_FAIL(standby_helper->init(ctx_->task_.src_info_, ctx_->task_.task_id_, sql_proxy_, bandwidth_throttle_))) {
...
{
  return is_inited_ && nullptr != ctx_ && ctx_->is_valid() && OB_NOT_NULL(bandwidth_throttle_)
                    && OB_NOT_NULL(sql_proxy_) && OB_NOT_NULL(helper_) && OB_NOT_NULL(handler_);
}
...
int ObRestoreDagNet::fill_dag_net_key(char *buf, const int64_t buf_len) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid args", K(ret), KP(buf), K(buf_len));
  } else if (OB_UNLIKELY(0 > ctx_->task_.task_id_.to_string(task_id_str, MAX_TRACE_ID_LENGTH))) {
...
bool ObStartRestoreDag::operator == (const share::ObIDag &other) const
{
  ...
  if (this == &other) {
    ...
    if (OB_NOT_NULL(ctx) || OB_NOT_NULL(other_ctx)) {
      if (ctx->task_.task_id_ != other_ctx->task_.task_id_) {
        is_same = false;
      }
    }
  }
...
int ObStartRestoreTask::process()
{
  ...
  if (IS_NOT_INIT) {
    ...
    //do nothing
  } else if (OB_FAIL(check_disk_space_enough_())) {
...
int ObStartRestoreTask::check_disk_space_enough_()
{
  int ret = OB_SUCCESS;
...
int ObStartRestoreTask::inner_build_ls_()
{
  ...
  if (IS_NOT_INIT) {
    ...
    } else if (share::ObLSRestoreStatus::NONE != restore_status) {
      ret = OB_ERR_UNEXPECTED;
...
int ObStartRestoreTask::fetch_next_tablet_info_(obrpc::ObCopyTabletInfo &tablet_info)
{
  ...
  } else if (OB_FAIL(helper_->fectch_next_tablet_info(tablet_info))) {
    LOG_WARN("failed to fetch tablet meta", K(ret), K(tablet_info));
    ...
    ObTabletType tablet_type)
{
  ...
  } else if (!tablet_id.is_valid() || OB_ISNULL(dag_net) || OB_ISNULL(tablet_group_ctx)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("tablet restore dag init get invalid argument", K(ret), K(tablet_id), KP(dag_net), KP(tablet_group_ctx));
  } else if (ObDagNetType::DAG_NET_TYPE_RESTORE != dag_net->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("dag net type is unexpected", K(ret), KPC(dag_net));
  } else if (FALSE_IT(restore_dag_net = static_cast<ObRestoreDagNet*>(dag_net))) {
...
int ObTabletRestoreTask::init(ObCopyTabletCtx &ctx)
{
  ...
  if (IS_INIT) {
    ...
    sql_proxy_ = restore_dag_net->get_sql_proxy();
    proto_helper = restore_dag_net->get_helper();
    copy_tablet_ctx_ = &ctx;
    if (OB_ISNULL(ctx_) || OB_ISNULL(bandwidth_throttle_) || OB_ISNULL(sql_proxy_) || OB_ISNULL(proto_helper)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ctx, bandwidth throttle, sql proxy or restore helper should not be NULL", 
                  K(ret), KP(ctx_), KP(bandwidth_throttle_), KP(sql_proxy_), KP(proto_helper));
    } else if (OB_FAIL(proto_helper->copy_for_task(allocator_, helper_))) {
      LOG_WARN("failed to copy task helper", K(ret), KP(proto_helper));
    } else if (OB_ISNULL(helper_) || !helper_->is_valid()) {
      ret = OB_ERR_UNEXPECTED;
    ...
    ObTabletFinishRestoreTask *&tablet_finish_restore_task)
{
  ...
  } else if (OB_ISNULL(dag = this->get_dag()) || ObDagNetType::DAG_NET_TYPE_RESTORE != dag->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    ...
    ObITask *child_task)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(src_info), K(copy_table_key), KP(parent_task), KP(child_task));
  } else if (OB_ISNULL(dag = this->get_dag())) {
    ret = OB_ERR_UNEXPECTED;
    ...
    LOG_WARN("failed to get ls", K(ret), KPC(ctx_));
  //TODO(xingzhi): refactor copy_sstable_info_mgr_ with helper 
    ...
    if (OB_FAIL(ctx_->ha_table_info_mgr_.get_table_info(copy_tablet_ctx_->tablet_id_, copy_table_key, init_param.sstable_param_))) {
      ...
      //TODO(xingzhi): refactor ha_table_info_mgr_ with helper
...
{
  int ret = OB_SUCCESS;
  ObStorageHASrcInfo src_info;//TODO(xingzhi): refactor with helper
  ...
  if (IS_NOT_INIT) {
    ...
    src_info.src_addr_ = ctx_->task_.src_info_;
    src_info.cluster_id_ = 0;
    for (int64_t i = 0; OB_SUCC(ret) && i < copy_table_key_array_.count(); ++i) {
      ...
      if (!copy_table_key.is_valid()) {
        ...
        if (OB_FAIL(check_need_copy_sstable_(copy_table_key, need_copy))) {
        ...
        } else if (OB_FAIL(generate_physical_copy_task_(src_info,
          ...
          LOG_INFO("succeed to generate sstable copy task", "src", src_info.src_addr_, K(copy_table_key));
        }
      }
    }
  }
...
int ObDataTabletsRestoreTask::process()
{
  ...
  if (IS_NOT_INIT) {
    ...
    while (OB_SUCC(ret)) {
      if (OB_FAIL(generate_tablet_group_dag_())) {
        if (OB_ITER_END == ret) {
          ret = OB_SUCCESS;
          break;
        } else {
    ...
    ObHATabletGroupCtx *tablet_group_ctx)
{
  ...
  ObIDagNet *dag_net = nullptr;
  ObRestoreDagNet *restore_dag_net = nullptr;
  ObArray<ObTabletID> tmp_tablet_id_array;
  share::ObLSID sys_ls_id(share::ObLSID::SYS_LS_ID);
  ...
  if (IS_INIT) {
    ...
    ctx_ = restore_dag_net->get_ctx();
    bandwidth_throttle_ = restore_dag_net->get_bandwidth_throttle();
    helper_ = restore_dag_net->get_helper();
    finish_dag_ = finish_dag;
    tablet_group_ctx_ = tablet_group_ctx;

    if (OB_ISNULL(ctx_) || OB_ISNULL(bandwidth_throttle_) || OB_ISNULL(helper_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ctx or bandwidth throttle or helper should not be NULL", K(ret), KP(ctx_), KP(bandwidth_throttle_), KP(helper_));
    } else if (OB_FAIL(ObStorageHADagUtils::get_ls(sys_ls_id, ls_handle_))) {
      LOG_WARN("failed to get ls", K(ret), KPC(ctx_));
    } else if (OB_FAIL(ObStorageHAUtils::append_tablet_list(tablet_id_array, tmp_tablet_id_array))) {
      LOG_WARN("failed to append tablet list", K(ret), K(tablet_id_array), KPC(ctx_));
    } else {
      ObLS *ls = nullptr;
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < tmp_tablet_id_array.count(); ++i) {
        ObTabletHandle tablet_handle;
        if (OB_FAIL(ls->ha_get_tablet(tmp_tablet_id_array.at(i), tablet_handle))) {
          if (OB_TABLET_NOT_EXIST == ret) {
            ret = OB_SUCCESS;
            LOG_INFO("tablet not exist, skip it", K(tmp_tablet_id_array.at(i)));
          } else {
            LOG_WARN("failed to get tablet", K(ret), K(tmp_tablet_id_array.at(i)));
          }
      ...
      if (FAILEDx(helper_->init_for_build_tablets_sstable_info(ls, tablet_handle_array))) {
        LOG_WARN("failed to init for build tablets sstable info", K(ret), K(tablet_id_array), KPC(ctx_));
      } else {
        is_inited_ = true;
...
int ObRestoreFinishTask::init()
{
  ...
  if (IS_INIT) {
    ...
    dag_net_ = dag_net;
    is_inited_ = true;
    LOG_INFO("succeed init restore finish task", "ls_id", share::ObLSID::SYS_LS_ID,
        "dag_id", *ObCurTraceId::get_trace_id(), "dag_net_id", ctx_->task_.task_id_);
  }
  return ret;
}
  ...
  ObLS *ls)
{
  ...
  } else if (OB_FAIL(helper->init_for_build_tablets_sstable_info(ls, tablet_handle_array))) {
  ...
  if (OB_ISNULL(helper) || OB_ISNULL(dag_net) || OB_ISNULL(ls) || tablet_id_array.empty()) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < tablet_id_array.count(); ++i) {
      ...
      if (dag_net->is_cancel()) {
        ret = OB_CANCELED;
        LOG_WARN("task is cancelled", K(ret));
      } else if (OB_FAIL(helper->fetch_tablet_meta(tablet_id, tablet_info))) {
