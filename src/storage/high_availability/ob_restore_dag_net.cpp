  ...
  if (OB_NOT_NULL(helper_) || OB_ISNULL(ctx_) || !ctx_->is_valid()) {
    ...
    if (ObRestoreTaskType::STANDBY_RESTORE_TASK == task_type) {
      ...
      if (OB_ISNULL(buf = ctx_->allocator_.alloc(sizeof(ObStandbyRestoreHelper)))) {
        ...
        if (OB_FAIL(standby_helper->init(ctx_->task_.src_info_, ls_service->get_storage_rpc_proxy(),
                                            ls_service->get_storage_rpc(), sql_proxy_, bandwidth_throttle_))) {
...
int ObStartRestoreTask::check_disk_space_enough_()
{
  ...
  } else if (OB_FAIL(helper_->check_disk_space())) {
    LOG_WARN("failed to check disk space enough", K(ret));
  }
  return ret;
}
    ...
    ObHATabletGroupCtx *tablet_group_ctx)
{
  ...
  if (IS_INIT) {
    ...
    } else if (OB_FAIL(helper_->init_for_build_tablets_sstable_info(tmp_tablet_id_array))) {
  ...
  ObLS *ls)
{
  ...
  } else if (OB_FAIL(helper->init_for_build_tablets_sstable_info(tablet_id_array))) {
