  ...
  if (OB_NOT_NULL(helper_) || OB_ISNULL(ctx_) || !ctx_->is_valid()) {
    ...
    if (ObRestoreTaskType::STANDBY_RESTORE_TASK == task_type) {
      ...
      if (OB_ISNULL(buf = ctx_->allocator_.alloc(sizeof(ObStandbyRestoreHelper)))) {
        ...
        if (OB_FAIL(standby_helper->init(ctx_->task_.src_info_, ctx_->task_.task_id_))) {
...
{
  return is_inited_ && nullptr != ctx_ && ctx_->is_valid() && OB_NOT_NULL(helper_) && OB_NOT_NULL(handler_);
}
...
int ObStartRestoreTask::process()
{
  ...
  if (IS_NOT_INIT) {
    ...
    LOG_INFO("xinzghi test:start restore task is failed", K(ret), KPC(ctx_));
    //do nothing
...
int ObStartRestoreTask::update_ls_()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(ls->update_ls_meta(false/*update_restore_status*/, ctx_->src_ls_meta_package_.ls_meta_))) {
      ...
      if (OB_FAIL(ls->advance_base_info(ctx_->src_ls_meta_package_.palf_meta_, false/*is_rebuild*/))) {
...
int ObStartRestoreTask::inner_build_ls_()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(helper_->init_for_ls_view())) {
      LOG_WARN("failed to init ls view", K(ret), KPC(ctx_));
    } else {
      if (OB_FAIL(helper_->fetch_ls_meta(ls_meta_package))) {
        LOG_WARN("failed to fetch ls meta", K(ret), KPC(ctx_));
      } else {
        storage::ObRestoreStatus restore_status;
        ...
        } else if (storage::ObRestoreStatus::NONE != restore_status) {
          ret = OB_ERR_UNEXPECTED;
...
int ObTabletRestoreTask::process()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(generate_restore_tasks_())) {
...
int ObDataTabletsRestoreTask::process()
{
  ...
  } else if (OB_FAIL(helper_->trigger_log_sync())) {
    LOG_WARN("failed to trigger log sync", KR(ret), KPC(ctx_), KPC(helper_));
...
int ObDataTabletsRestoreTask::set_restore_status_()
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ls should not be NULL", K(ret), KPC(ctx_));
  } else if (OB_FAIL(ls->set_restore_status(ObRestoreStatus(ObRestoreStatus::RESTORE_DATA_TABLETS)))) {
    LOG_WARN("failed to set restore status", KR(ret), KPC(ctx_));
  }
  return ret;
}
