...
int ObStandbyRestoreHelper::trigger_log_sync()
{
  int ret = OB_SUCCESS;
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    ...
    if (OB_ISNULL(log_service)) {
      ...
      if (OB_ISNULL(restore_service)) {
        ...
        LOG_INFO("standby log sync triggered", K(src_), K(task_id_));
      }
    }
  }
  return ret;
    ...
    const share::ObTaskId &task_id)
{
  ...
  if (is_valid()) {
    ...
    LOG_WARN("invalid argument", K(ret), K(src), K(task_id));
  } else {
    src_ = src;
    task_id_ = task_id;
    is_inited_ = true;
...
int ObStandbyRestoreHelper::fetch_tablet_meta(obrpc::ObCopyTabletInfo &tablet_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ObRestoreHelperTabletInfoCtx *tablet_info_ctx = static_cast<ObRestoreHelperTabletInfoCtx *>(ctx_);
    if (OB_ISNULL(tablet_info_ctx->grpc_client_) || OB_ISNULL(tablet_info_ctx->tablet_info_reader_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("tablet info stream is not initialized", K(ret), KP(tablet_info_ctx->grpc_client_), KP(tablet_info_ctx->tablet_info_reader_));
    } else {
      storageservice::FetchTabletInfoRes response;
      if (!tablet_info_ctx->tablet_info_reader_->Read(&response)) {
        grpc::Status status = tablet_info_ctx->tablet_info_reader_->Finish();
        int grpc_ret = tablet_info_ctx->grpc_client_->translate_error(status);
        if (OB_FAIL(grpc_ret)) {
          LOG_WARN("tablet info stream finished with error", K(ret));
        } else {
          ret = OB_ITER_END;
    ...
    storage::ObMacroBlockReuseMgr *macro_block_reuse_mgr)
{
  ...
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("standby restore helper not init", K(ret), KPC(this));
  //TODO(xingzhi): change code
  } else if (!copy_table_key.is_valid() || !macro_range_info.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(copy_table_key), K(macro_range_info));
    ...
    if (OB_FAIL(arg.copy_macro_range_info_.assign(macro_range_info))) {
      ...
      if (OB_FAIL(macro_block_ctx->data_buffer_.ensure_space(common::OB_DEFAULT_MACRO_BLOCK_SIZE))) {
