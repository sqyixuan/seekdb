...
int ObStandbyRestoreHelper::fetch_next_tablet_info(obrpc::ObCopyTabletInfo &tablet_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (!ls_view_ctx->ls_meta_fetched_) {
    ...
    } else if (OB_ISNULL(ls_view_ctx->grpc_client_) || !ls_view_ctx->tablet_info_reader_) {
      ret = OB_ERR_UNEXPECTED;
      ...
      if (!ls_view_ctx->tablet_info_reader_->Read(&response)) {
        if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(ls_view_ctx->tablet_info_reader_, ls_view_ctx->grpc_client_))) {
