  ...
  if (IS_NOT_INIT) {
    ...
    if (!ls_view_ctx->ls_meta_fetched_) {
      ...
      if (!ls_view_ctx->tablet_info_reader_->Read(&response)) {
        grpc::Status status;
        if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(ls_view_ctx->tablet_info_reader_, status))) {
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_ISNULL(tablet_info_ctx->grpc_client_) || OB_ISNULL(tablet_info_ctx->tablet_info_reader_)) {
      ...
      if (!tablet_info_ctx->tablet_info_reader_->Read(&response)) {
        grpc::Status status;
        if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(tablet_info_ctx->tablet_info_reader_, status))) {
          LOG_WARN("failed to close reader", K(ret));
        } else {
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(sstable_info_ctx->grpc_client_) || OB_ISNULL(sstable_info_ctx->sstable_info_reader_)) {
      ...
      if (!sstable_info_ctx->sstable_info_reader_->Read(&response)) {
        grpc::Status status;
        if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(sstable_info_ctx->sstable_info_reader_, status))) {
          LOG_WARN("failed to close reader", K(ret));
        } else {
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(macro_range_ctx->grpc_client_) || OB_ISNULL(macro_range_ctx->macro_info_reader_)) {
      ...
      if (!macro_range_ctx->macro_info_reader_->Read(&response)) {
        grpc::Status status;
        if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(macro_range_ctx->macro_info_reader_, status))) {
          LOG_WARN("failed to close reader", K(ret));
        } else {
    ...
    obrpc::ObCopyMacroBlockHeader &header)
{
  ...
  if (OB_ISNULL(macro_block_ctx) || OB_ISNULL(macro_block_ctx->macro_block_reader_)) {
    ...
    if (!macro_block_ctx->macro_block_reader_->Read(&response)) {
      grpc::Status status;
      if (OB_FAIL(ObRestoreHelperCtxUtil::close_reader(macro_block_ctx->macro_block_reader_, status))) {
        LOG_WARN("failed to close reader", K(ret));
      } else {
