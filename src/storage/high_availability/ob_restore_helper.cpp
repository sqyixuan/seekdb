...
{
  int64_t rpc_timeout = RPC_TIMEOUT_US;
...
static int64_t calc_ls_view_stream_timeout_us(const int64_t tablet_count)
{
  ...
  timeout_us = std::max(timeout_us, get_rpc_timeout());
  return timeout_us;
}
...
int ObStandbyRestoreHelper::check_restore_precondition()
{
  ...
  if (!is_valid()) {
    ...
    if (OB_FAIL(grpc_client.init(src_, get_rpc_timeout()))) {
...
int ObStandbyRestoreHelper::get_ls_view_rpc_timeout_(int64_t &rpc_timeout_us)
{
  ...
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("standby restore helper not init", K(ret), KPC(this));
  } else if (OB_FAIL(grpc_client.init(src_, get_rpc_timeout()))) {
...
int ObStandbyRestoreHelper::init_for_fetch_tablet_meta(const common::ObIArray<common::ObTabletID> &tablet_id_array)
{
  ...
  if (!is_valid()) {
    ...
    } else if (OB_FAIL(storage::ObStorageGrpcClient::init_tablet_info_stream(src_, get_rpc_timeout(), 
  ...
  if (!is_valid()) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(storage::ObStorageGrpcClient::init_tablet_sstable_info_stream(
              src_, get_rpc_timeout(), arg, ctx_allocator_, *sstable_info_ctx))) {
...
int ObStandbyRestoreHelper::init_for_sstable_macro_range(const common::ObIArray<storage::ObITable::TableKey> &copy_table_key_array)
{
  ...
  if (!is_valid()) {
    ...
    } else if (OB_FAIL(storage::ObStorageGrpcClient::init_sstable_macro_info_stream(src_, get_rpc_timeout(), 
    ...
    storage::ObMacroBlockReuseMgr *macro_block_reuse_mgr)
{
  ...
  if (!is_valid()) {
    ...
    } else if (OB_FAIL(storage::ObStorageGrpcClient::init_macro_block_stream(src_, get_rpc_timeout(), 
