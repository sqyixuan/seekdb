...
namespace restore
{
static const int64_t LS_VIEW_GRPC_RPC_TIMEOUT = 60L * 1000L * 1000L; // 60 seconds
    ...
    common::ObInOutBandwidthThrottle *bandwidth_throttle)
{
  ...
  } else if (!src.is_valid() || !task_id.is_valid() || OB_ISNULL(sql_proxy) || OB_ISNULL(bandwidth_throttle)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(src), K(task_id), KP(sql_proxy), KP(bandwidth_throttle));
  } else {
    src_ = src;
    task_id_ = task_id;
...
int ObStandbyRestoreHelper::copy_for_task(common::ObIAllocator &allocator, ObIRestoreHelper *&helper) const
{
  ...
  if (!is_valid()) {
    ...
    if (OB_ISNULL(buf = allocator.alloc(sizeof(ObStandbyRestoreHelper)))) {
      ...
      task_helper->sql_proxy_ = sql_proxy_;
...
int ObStandbyRestoreHelper::check_restore_precondition()
{
  ...
  if (!is_valid()) {
    ...
    //TODO(xingzhi): get the sum of all tablet sizes
...
int ObStandbyRestoreHelper::init_for_ls_view()
{
  ...
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("standby restore helper not init", K(ret), KPC(this));
  } else if (OB_FAIL(create_ctx_(RESTORE_HELPER_CTX_LS_VIEW))) {
...
int ObStandbyRestoreHelper::fetch_ls_meta(ObLSMetaPackage &ls_meta)
{
  ...
  if (!is_valid()) {
  ...
  } else if (RESTORE_HELPER_CTX_LS_VIEW != ctx_->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    ...
    if (ls_view_ctx->ls_meta_fetched_) {
      ...
      // Single-replica: no args needed
      oceanbase::storage::ObLSViewStreamInitResult init_result;
      if (OB_FAIL(oceanbase::storage::ObStorageGrpcClient::init_ls_view_stream(
              src_, LS_VIEW_GRPC_RPC_TIMEOUT, ctx_allocator_, init_result))) {
        LOG_WARN("failed to init ls view stream", K(ret), K_(src));
      } else {
        ls_meta = init_result.ls_meta_;
        ls_view_ctx->tablet_id_list_ = init_result.tablet_id_list_;
        ls_view_ctx->grpc_client_ = init_result.grpc_client_;
        ls_view_ctx->tablet_info_context_ = init_result.tablet_info_context_;
        ls_view_ctx->tablet_info_reader_ = init_result.tablet_info_reader_;
...
int ObStandbyRestoreHelper::fectch_next_tablet_info(obrpc::ObCopyTabletInfo &tablet_info)
{
  int ret = OB_SUCCESS;
  tablet_info.reset();
  if (IS_NOT_INIT) {
    ...
    } else if (OB_ISNULL(ls_view_ctx->grpc_client_) || OB_ISNULL(ls_view_ctx->tablet_info_reader_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("ls view stream is not initialized", K(ret), KP(ls_view_ctx->grpc_client_), KP(ls_view_ctx->tablet_info_reader_));
    } else {
      storageservice::FetchTabletInfoRes response;
      if (!ls_view_ctx->tablet_info_reader_->Read(&response)) {
        grpc::Status status = ls_view_ctx->tablet_info_reader_->Finish();
        int grpc_ret = ls_view_ctx->grpc_client_->translate_error(status);
        if (OB_FAIL(grpc_ret)) {
          LOG_WARN("tablet info stream finished with error", K(ret));
        } else {
          ret = OB_ITER_END;
        }
...
int ObStandbyRestoreHelper::fetch_tablet_meta(const common::ObTabletID &tablet_id, ObCopyTabletInfo &tablet_info)
{
  ...
  if (!is_valid()) {
    ...
    LOG_WARN("fetch_tablet_meta is not supported yet", K(ret), K(tablet_id));
  }
  return ret;
}
    ...
    const common::ObIArray<ObTabletHandle> &tablet_handle_array)
{
  ...
  if (!is_valid()) {
  ...
  } else if (OB_ISNULL(ls) || tablet_handle_array.empty()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), KP(ls), K(tablet_handle_array));
  } else if (OB_FAIL(create_ctx_(RESTORE_HELPER_CTX_SSTABLE_INFO))) {
    ...
    if (OB_SUCC(ret)) {
      storage::ObSSTableInfoStreamInitResult stream_result;
      if (OB_FAIL(storage::ObStorageGrpcClient::init_tablet_sstable_info_stream(
              src_, RPC_TIMEOUT_US, arg, ctx_allocator_, stream_result))) {
        LOG_WARN("failed to init tablet sstable info stream", K(ret), K(arg), K_(src));
      } else {
        sstable_info_ctx->grpc_client_ = stream_result.grpc_client_;
        sstable_info_ctx->sstable_info_context_ = stream_result.sstable_info_context_;
        sstable_info_ctx->sstable_info_reader_ = stream_result.sstable_info_reader_;
...
int ObStandbyRestoreHelper::fetch_next_tablet_sstable_header(obrpc::ObCopyTabletSSTableHeader &copy_header)
{
  ...
  } else if (RESTORE_HELPER_CTX_SSTABLE_INFO != ctx_->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ctx type mismatch", K(ret), "cur_ctx_type", ctx_->get_type(), KPC(this));
  } else {
    ObRestoreHelperSSTableInfoCtx *sstable_info_ctx = static_cast<ObRestoreHelperSSTableInfoCtx *>(ctx_);
    if (OB_ISNULL(sstable_info_ctx->grpc_client_) || OB_ISNULL(sstable_info_ctx->sstable_info_reader_)) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("sstable info stream is not initialized", K(ret), KP(sstable_info_ctx->grpc_client_), KP(sstable_info_ctx->sstable_info_reader_));
      ...
      if (!sstable_info_ctx->sstable_info_reader_->Read(&response)) {
        grpc::Status status = sstable_info_ctx->sstable_info_reader_->Finish();
        int grpc_ret = sstable_info_ctx->grpc_client_->translate_error(status);
        if (OB_FAIL(grpc_ret)) {
          LOG_WARN("tablet sstable info stream finished with error", K(ret));
        } else {
          ret = OB_ITER_END;
        }
    ...
    if (OB_ISNULL(sstable_info_ctx->grpc_client_) || OB_ISNULL(sstable_info_ctx->sstable_info_reader_)) {
      ...
      if (!sstable_info_ctx->sstable_info_reader_->Read(&response)) {
        ...
        if (OB_FAIL(grpc_ret)) {
          ...
          ret = OB_ERR_UNEXPECTED;
          LOG_WARN("unexpected end of stream when expecting sstable meta", K(ret), "pending_count", sstable_info_ctx->pending_sstable_count_);
        }
...
int ObStandbyRestoreHelper::init_for_sstable_macro_range(const common::ObIArray<storage::ObITable::TableKey> &copy_table_key_array)
{
  ...
  if (!is_valid()) {
  ...
  } else if (OB_FAIL(create_ctx_(RESTORE_HELPER_CTX_SSTABLE_MACRO_RANGE))) {
    ...
    storage::ObSSTableMacroInfoStreamInitResult stream_result;
    obrpc::ObCopySSTableMacroRangeInfoArg arg;
    arg.tenant_id_ = OB_SYS_TENANT_ID;  
    arg.ls_id_ = share::ObLSID(share::ObLSID::SYS_LS_ID); 
    ...
    if (OB_FAIL(arg.copy_table_key_array_.assign(copy_table_key_array))) {
      LOG_WARN("failed to assign copy table key array", K(ret), "key_cnt", copy_table_key_array.count());
                                                                                ...
                                                                                arg, ctx_allocator_, stream_result))) {
      LOG_WARN("failed to init sstable macro info stream", K(ret), K(arg), K_(src));
    } else {
       macro_range_ctx->grpc_client_ = stream_result.grpc_client_;
       macro_range_ctx->macro_info_context_ = stream_result.macro_info_context_;
       macro_range_ctx->macro_info_reader_ = stream_result.macro_info_reader_;
    }
...
int ObStandbyRestoreHelper::fetch_next_sstable_macro_range_info(storage::ObCopySSTableMacroRangeInfo &sstable_macro_range_info)
{
  ...
  if (!is_valid()) {
  ...
  } else if (RESTORE_HELPER_CTX_SSTABLE_MACRO_RANGE != ctx_->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ctx type mismatch", K(ret), "cur_ctx_type", ctx_->get_type(), KPC(this));
  } else {
    ObRestoreHelperSSTableMacroRangeCtx *macro_range_ctx = static_cast<ObRestoreHelperSSTableMacroRangeCtx *>(ctx_);
    ...
    if (OB_ISNULL(macro_range_ctx->grpc_client_) || OB_ISNULL(macro_range_ctx->macro_info_reader_)) {
      ret = OB_ERR_UNEXPECTED;
      ...
      if (!macro_range_ctx->macro_info_reader_->Read(&response)) {
        grpc::Status status = macro_range_ctx->macro_info_reader_->Finish();
        int grpc_ret = macro_range_ctx->grpc_client_->translate_error(status);
        if (OB_FAIL(grpc_ret)) {
          LOG_WARN("tablet sstable macro info stream finished with error", K(ret));
        } else {
          ret = OB_ITER_END;
        }
    ...
    storage::ObMacroBlockReuseMgr *macro_block_reuse_mgr)
{
  ...
  if (!is_valid()) {
    ...
    LOG_WARN("standby restore helper not init", K(ret), KPC(this));
                  ...
                  || data_version < 0 || OB_ISNULL(macro_block_reuse_mgr)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(copy_table_key), K(macro_range_info), K(data_version), KP(macro_block_reuse_mgr));
  } else if (OB_FAIL(create_ctx_(RESTORE_HELPER_CTX_MACRO_BLOCK))) {
    ...
    storage::ObMacroBlockStreamInitResult stream_result;
    obrpc::ObCopyMacroBlockRangeArg arg;
    arg.tenant_id_ = OB_SYS_TENANT_ID;
    arg.ls_id_ = share::ObLSID(share::ObLSID::SYS_LS_ID);
    arg.table_key_ = copy_table_key;
    arg.copy_macro_range_info_ = macro_range_info;
    ...
    if (OB_FAIL(storage::ObStorageGrpcClient::init_macro_block_stream(src_, RPC_TIMEOUT_US, 
                                                                        arg, ctx_allocator_, stream_result))) {
      LOG_WARN("failed to init macro block stream", K(ret), K(arg), K_(src));
    } else {
      macro_block_ctx->grpc_client_ = stream_result.grpc_client_;
      macro_block_ctx->macro_block_context_ = stream_result.macro_block_context_;
      macro_block_ctx->macro_block_reader_ = stream_result.macro_block_reader_;
    }
...
int ObStandbyRestoreHelper::fetch_next_macro_block(storage::ObICopyMacroBlockReader::CopyMacroBlockReadData &read_data)
{
  ...
  if (!is_valid()) {
  ...
  } else if (RESTORE_HELPER_CTX_MACRO_BLOCK != ctx_->get_type()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("ctx type mismatch", K(ret), "cur_ctx_type", ctx_->get_type(), KPC(this));
  } else {
    ObRestoreHelperMacroBlockCtx *macro_block_ctx = static_cast<ObRestoreHelperMacroBlockCtx *>(ctx_);
    ...
    if (OB_ISNULL(macro_block_ctx->grpc_client_) || OB_ISNULL(macro_block_ctx->macro_block_reader_)) {
      ret = OB_ERR_UNEXPECTED;
    ...
    } else if (header.data_type_ == obrpc::ObCopyMacroBlockDataType::MACRO_DATA) {
    ...
    } else if (header.data_type_ == obrpc::ObCopyMacroBlockDataType::MACRO_META_ROW) {
    ...
    obrpc::ObCopyMacroBlockHeader &header)
{
  ...
  if (OB_ISNULL(macro_block_ctx) || OB_ISNULL(macro_block_ctx->macro_block_reader_)) {
    ret = OB_INVALID_ARGUMENT;
    ...
    if (!macro_block_ctx->macro_block_reader_->Read(&response)) {
      grpc::Status status = macro_block_ctx->macro_block_reader_->Finish();
      int grpc_ret = macro_block_ctx->grpc_client_->translate_error(status);
      if (OB_FAIL(grpc_ret)) {
        LOG_WARN("macro block stream finished with error", K(ret));
      } else {
        ret = OB_ITER_END;
      }
    } else if (OB_FAIL(obgrpc::deserialize_proto_to_ob(response, header))) {
    ...
    blocksstable::ObBufferReader &data_reader)
{
  ...
  if (OB_ISNULL(macro_block_ctx) || OB_ISNULL(macro_block_ctx->macro_block_reader_)) {
    ...
    common::ObArenaAllocator allocator("MacroBlockData");
    char *data_buf = nullptr;
    
    if (OB_ISNULL(data_buf = static_cast<char*>(allocator.alloc(occupy_size)))) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
      LOG_WARN("failed to alloc memory for macro block data", K(ret), K(occupy_size));
    } else {
      storageservice::FetchMacroBlockRes response;
      if (!macro_block_ctx->macro_block_reader_->Read(&response)) {
        grpc::Status status = macro_block_ctx->macro_block_reader_->Finish();
        ...
        // Copy raw binary data from bytes field (protobuf already deserialized the message,
        // but the bytes field content is raw binary, no additional deserialization needed)
        MEMCPY(data_buf, response.buf().data(), occupy_size);
        data_reader.assign(data_buf, occupy_size, occupy_size);
      }
    }
  }
    ...
    blocksstable::ObBufferReader &buffer)
{
  ...
  if (!is_valid()) {
    ret = OB_NOT_INIT;
    LOG_WARN("standby restore helper not init", K(ret), KPC(this));
  } else if (!macro_id.logic_block_id_.is_valid()) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", K(ret), K(macro_id));
  } else {
    ret = OB_NOT_SUPPORTED;
    LOG_WARN("fetch_macro_block is not supported yet", K(ret), K(macro_id));
  }
  return ret;
}
