...
public:
  ...
  INHERIT_TO_STRING_KV("ObIHADagNetCtx", ObIHADagNetCtx, K_(tenant_id), K_(local_clog_checkpoint_scn), 
                          K_(task), K_(ha_table_info_mgr), K_(src_ls_meta_package), 
                          K_(start_ts), K_(finish_ts), K_(check_tablet_info_cost_time));
...
public:
  ...
  ObRestoreDagNetCtx *get_ctx() { return ctx_; }
  ...
  ObIRestoreHelper *get_helper() { return helper_; }
...
public:
  ...
  VIRTUAL_TO_STRING_KV(K("ObInitialRestoreTask"), KP(this), KPC_(ctx), KPC_(helper));
private:
  int generate_restore_dags_();
...
private:
  ...
  int check_before_ls_restore_(const ObLSMeta &ls_meta);
  int build_ls_();
...
public:
  ...
  int init();
  virtual int process() override;
  VIRTUAL_TO_STRING_KV(K("ObRestoreFinishTask"), KP(this), KPC_(ctx), KPC_(helper));
private:
