...
int ObRestoreHandler::init(ObLS *ls)
{
  int ret = OB_SUCCESS;
  if (IS_INIT) {
    ret = OB_INIT_TWICE;
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("restore handler init get invalid argument", KR(ret), KP(ls));
  } else {
    ls_ = ls;
...
int ObRestoreHandler::add_ls_restore_task(const ObRestoreTask &task)
{
  ...
  } else if (OB_FAIL(ls_->set_restore_status(ObRestoreStatus(ObRestoreStatus::RESTORE_DOING)))) {
...
int ObRestoreHandler::cancel_task(const share::ObTaskId &task_id, bool &is_exist)
{
  int ret = OB_SUCCESS;
  ObTenantDagScheduler *scheduler = nullptr;
  is_exist = false;
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(check_task_exist_with_nolock_(task_id, is_exist))) {
      LOG_WARN("fail to check task exist", KR(ret), K(task_id));
    } else if (!is_exist) {
      LOG_INFO("task is not exist in restore task", K(task_id));
    } else if (OB_ISNULL(scheduler = MTL(ObTenantDagScheduler*))) {
      ret = OB_ERR_UNEXPECTED;
      LOG_ERROR("failed to get ObTenantDagScheduler from MTL", KR(ret));
    // If task not exist, cancel_dag_net return OB_SUCCESS
    } else if (OB_FAIL(scheduler->cancel_dag_net(task_id))) {
      LOG_WARN("failed to cancel dag net", KR(ret), K(task_id));
    } else {
      is_cancel_ = true;
    }
  }
...
int ObRestoreHandler::do_build_status_()
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_FAIL(get_restore_task_(task))) {
      ...
      if (OB_FAIL(generate_build_dag_net_())) {
