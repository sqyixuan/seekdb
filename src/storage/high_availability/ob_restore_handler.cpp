    ...
    common::ObMySQLProxy *sql_proxy)
{
  ...
  } else if (OB_ISNULL(ls) || OB_ISNULL(bandwidth_throttle) || OB_ISNULL(sql_proxy)) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("restore handler init get invalid argument", KR(ret), KP(ls), KP(bandwidth_throttle), KP(sql_proxy));
  } else {
    ls_ = ls;
