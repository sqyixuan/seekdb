...
int ObPartTransCtx::handle_timeout(const int64_t delay)
{
  ...
  if (OB_SUCC(lock_.lock(5000000 /*5 seconds*/))) {
    ...
    if (IS_NOT_INIT) {
      ...
      if (!is_follower_() && part_trans_action_ == ObPartTransAction::COMMIT && is_local_tx_()) {
        if (tx_expired) {
...
int ObPartTransCtx::kill(const KillTransArg &arg, ObTxCommitCallback *&cb_list)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (arg.graceful_) {
      if (FALSE_IT(notify_scheduler_tx_killed = !is_follower_() && part_trans_action_ == ObPartTransAction::START)) {
                           ...
                           const int64_t &request_id)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (exec_info_.participants_.count() <= 0) {
      ...
      TRANS_LOG(ERROR, "2pc is not supported", K(ret), KPC(this));
    }
  }
  if (OB_SUCC(ret)) {
    commit_cb_.enable();
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < cb_arg_array.count(); i++) {
    ...
    if (ObTxLogType::TX_REDO_LOG == log_type) {
      ...
      if (ObTxLogType::TX_PREPARE_LOG == log_type) {
        ret = OB_NOT_SUPPORTED;
        TRANS_LOG(WARN, "2pc is not supported", K(ret), K(log_type), KPC(this));
        ...
        if (OB_SUCC(ret)) {
          if (is_local_tx_() || sub_state_.is_force_abort()) {
            ...
            } else if (OB_FAIL(notify_data_source_(type, log_ts, false,
                                                   mds_cache_.get_final_notify_array(), false/*willing_to_commit*/))) {
              TRANS_LOG(WARN, "notify data source failed", KR(ret), K(*this));
            }
          }
        }
...
int ObPartTransCtx::try_submit_next_log_(const bool for_freeze)
{
  ...
  if (ObPartTransAction::COMMIT == part_trans_action_ && !need_force_abort_()
      && !sub_state_.is_state_log_submitted() && !sub_state_.is_state_log_submitting()
      && !sub_state_.is_gts_waiting() && exec_info_.state_ < ObTxState::PREPARE) {
    if (is_follower_()) {
      ret = OB_NOT_MASTER;
    } else {
      if (is_local_tx_()) {
...
int ObPartTransCtx::submit_commit_log_()
{
  ...
  if (OB_SUCC(ret)) {
    if (is_local_tx_() &&
      ...
      ret = OB_NOT_SUPPORTED;
      TRANS_LOG(WARN, "2pc is not supported", K(ret), KPC(this));
    }
  }
...
int ObPartTransCtx::submit_abort_log_()
{
  ...
  if (OB_SUCC(ret)) {
    if ((exec_info_.multi_data_source_.count() > 0 || mds_cache_.count() > 0)) {
      if (OB_FAIL(try_alloc_retain_ctx_func_())) {
        TRANS_LOG(WARN, "alloc retain ctx func for mds trans failed", K(ret), K(mds_cache_),
                  KPC(this));
      }
    }
  ...
  if (OB_FAIL(ret)) {
    ...
    TRANS_LOG(WARN, "decide abort log barrier type failed", K(ret), K(abort_log_barrier_type),
              KPC(this));
  } else if (OB_FAIL(init_log_block_(log_block))) {
    TRANS_LOG(WARN, "init log block failed", KR(ret), K(*this));
                                          ...
                                          const int64_t retry_timeout_us)
{
  ...
  if (is_contain_stat_log(log_block.get_cb_arg_array()) && FALSE_IT(is_2pc_state_log = true)) {
...
{
  return cluster_version_accurate_;
}
