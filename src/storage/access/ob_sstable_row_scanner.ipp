...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::inner_get_next_row_with_row_id(const ObDatumRow *&store_row, ObCSRowId &row_id)
{
  int ret = OB_SUCCESS;
...
template<>
inline bool ObSSTableRowScanner<ObCOPrefetcher>::can_blockscan() const
{
...
template<>
inline bool ObSSTableRowScanner<ObCOPrefetcher>::can_batch_scan() const
{
  return can_blockscan() &&
         !block_row_store_->is_disabled() &&
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::get_blockscan_start(
    ObCSRowId &start,
    int32_t &range_idx,
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::forward_blockscan(
    ObCSRowId &end,
    BlockScanState &block_scan_state,
...
inline int ObSSTableRowScanner<PrefetcheType>::try_skip_deleted_row(ObCSRowId &co_current)
{
  int ret = OB_SUCCESS;
  ObCOPrefetcher* co_prefetcher = reinterpret_cast<ObCOPrefetcher*>(&prefetcher_);
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::update_start_rowid_for_column_store()
{
  int ret = OB_SUCCESS;
  ObCOPrefetcher* co_prefetcher = reinterpret_cast<ObCOPrefetcher*>(&prefetcher_);
  co_prefetcher->set_range_scan_finish();
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::prepare_micro_scanner_for_column_store(ObSSTableReadHandle& read_handle)
{
  int ret = OB_SUCCESS;
  ObCOPrefetcher* co_prefetcher = reinterpret_cast<ObCOPrefetcher*>(&prefetcher_);
  if (nullptr == micro_scanner_) {
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::detect_border_rowid_for_column_store()
{
  int ret = OB_SUCCESS;
  ObCOPrefetcher* co_prefetcher = reinterpret_cast<ObCOPrefetcher*>(&prefetcher_);
  if (OB_FAIL(prefetcher_.prefetch())) {
...
template<typename PrefetchType>
inline int ObSSTableRowScanner<PrefetchType>::try_refreshing_blockscan_checker_for_column_store(
