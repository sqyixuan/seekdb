    ...
    grpc::ServerWriter<FetchMacroBlockRes>* writer)
{
  ...
  if (OB_FAIL(deserialize_proto_to_ob(*request, arg))) {
    ...
    MTL_SWITCH(arg.tenant_id_) {
      ...
      common::ObArenaAllocator header_allocator("MacroBlockHead");
      while (OB_SUCC(ret)) {
        if (context->IsCancelled()) {
          ret = OB_CANCELED;
          LOG_WARN("client cancelled the request", K(ret));
        ...
        if (OB_FAIL(producer->get_next_macro_block(data, header))) {
          ...
          } else if (FALSE_IT(data_response.set_buf(data.data(), data.capacity()))) {
          } else if (FALSE_IT(data_response.set_size(data.capacity()))) {
          } else if (data.capacity() != header.occupy_size_) {
            ret = OB_ERR_UNEXPECTED;
