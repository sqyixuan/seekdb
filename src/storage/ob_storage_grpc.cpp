    ...
    restore::ObRestoreHelperSSTableInfoCtx &sstable_info_ctx)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    sstable_info_ctx.sstable_info_reader_ = nullptr;
  }
    ...
    restore::ObRestoreHelperSSTableMacroRangeCtx &macro_range_ctx)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    macro_range_ctx.macro_info_reader_ = nullptr;
  }
    ...
    restore::ObRestoreHelperMacroBlockCtx &macro_block_ctx)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    macro_block_ctx.macro_block_reader_ = nullptr;
  }
    ...
    restore::ObRestoreHelperTabletInfoCtx &tablet_info_ctx)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    tablet_info_ctx.tablet_info_reader_ = nullptr;
  }
    ...
    restore::ObRestoreHelperLSViewCtx &ls_view_ctx)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    } else if (OB_FAIL(ls_view_ctx.tablet_id_list_.assign(copy_ls_info_result.tablet_id_array_))) {
      LOG_WARN("failed to assign tablet id list", K(ret), "tablet_cnt", copy_ls_info_result.tablet_id_array_.count());
    } else {
      ls_meta = copy_ls_info_result.ls_meta_package_;
    }
  }
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_ISNULL(ctx_buf = allocator.alloc(sizeof(grpc::ClientContext)))) {
      ...
      ls_view_ctx.tablet_info_context_ = new (ctx_buf) grpc::ClientContext();
      obrpc::ObRestoreCopyTabletInfoArg tablet_info_arg;
      if (OB_FAIL(tablet_info_arg.tablet_id_list_.assign(ls_view_ctx.tablet_id_list_))) {
        LOG_WARN("failed to assign tablet id list to arg", K(ret), "tablet_cnt", ls_view_ctx.tablet_id_list_.count());
      } else {
        ...
        if (OB_FAIL(tablet_info_arg_compat.tablet_id_list_.assign(tablet_info_arg.tablet_id_list_))) {
          LOG_WARN("failed to assign tablet id list", K(ret));
                ...
                tablet_info_arg_compat, *ls_view_ctx.tablet_info_context_, ls_view_ctx.tablet_info_reader_))) {
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_NOT_NULL(ls_view_ctx.tablet_info_context_)) {
      ls_view_ctx.tablet_info_context_->~ClientContext();
      allocator.free(ls_view_ctx.tablet_info_context_);
      ls_view_ctx.tablet_info_context_ = nullptr;
    }
    ls_view_ctx.grpc_client_ = nullptr;
    ls_view_ctx.tablet_info_reader_ = nullptr;
  }
