...
struct ObStandbyCopyMacroBlockRangeArg final
{
  OB_UNIS_VERSION(1);
public:
  ObStandbyCopyMacroBlockRangeArg();
  ~ObStandbyCopyMacroBlockRangeArg() {}

  bool is_valid() const;
  TO_STRING_KV(K_(table_key), K_(data_version), K_(backfill_tx_scn), K_(copy_macro_range_info), "standby_single_replica", true);
  ...
  storage::ObCopyMacroRangeInfo copy_macro_range_info_;
  ObSArray<ObCopyMacroBlockInfo> copy_macro_block_infos_;
  DISALLOW_COPY_AND_ASSIGN(ObStandbyCopyMacroBlockRangeArg);
};
...
// Simplified version for single-replica scenario (no tenant_id/ls_id needed)
struct ObStandbyCopyTabletInfoArg
{
  OB_UNIS_VERSION(1);
public:
  ObStandbyCopyTabletInfoArg();
  virtual ~ObStandbyCopyTabletInfoArg() {}

  bool is_valid() const { return true; }
  TO_STRING_KV(K_(tablet_id_list), "standby_single_replica", true);
  common::ObSArray<common::ObTabletID> tablet_id_list_;
};
...
struct ObCheckRestorePreconditionArg
{
  OB_UNIS_VERSION(1);
public:
  ObCheckRestorePreconditionArg();
  virtual ~ObCheckRestorePreconditionArg() {}
...
struct ObCheckRestorePreconditionResult
{
  OB_UNIS_VERSION(1);
public:
...
// Simplified version for single-replica scenario (no tenant_id/ls_id needed)
struct ObStandbyCopySSTableMacroRangeInfoArg final
{
  OB_UNIS_VERSION(1);
public:
  ObStandbyCopySSTableMacroRangeInfoArg();
  ~ObStandbyCopySSTableMacroRangeInfoArg();
  bool is_valid() const;
  int assign(const ObStandbyCopySSTableMacroRangeInfoArg &arg);

  TO_STRING_KV(K_(tablet_id), K_(copy_table_key_array), K_(macro_range_max_marco_count), "standby_single_replica", true);
  common::ObTabletID tablet_id_;
  common::ObSArray<ObITable::TableKey> copy_table_key_array_;
  int64_t macro_range_max_marco_count_;
  DISALLOW_COPY_AND_ASSIGN(ObStandbyCopySSTableMacroRangeInfoArg);
};
