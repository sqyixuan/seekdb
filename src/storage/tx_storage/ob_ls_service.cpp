...
int ObLSService::create_ls_()
{
  ...
  const ObTenantRole primary_role(ObTenantRole::PRIMARY_TENANT);
  common::ObMemberList member_list;
  common::GlobalLearnerList learner_list;
  const ObMigrationStatus migration_status(ObMigrationStatus::OB_MIGRATION_STATUS_NONE);
  const share::ObLSRestoreStatus restore_status(ObLSRestoreStatus::NONE);
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_UNLIKELY(!is_running_)) {
      ...
      } else if (OB_BREAK_FAIL(ls->create_ls(primary_role/*tenant_role*/,
                                             palf_base_info,
                                             REPLICA_TYPE_FULL/*replica_type*/))) {
        LOG_WARN("enable ls palf failed", K(ret), K(ls_meta));
      } else if (FALSE_IT(state = ObLSCreateState::CREATE_STATE_PALF_ENABLED)) {
      } else if (OB_BREAK_FAIL(ls->create_ls_inner_tablet(compat_mode,
                                                          create_scn))) {
