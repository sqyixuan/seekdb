...
int ObLSService::create_ls(const ObTenantRole &tenant_role)
{
  ...
  if (OB_FAIL(create_ls_(tenant_role))) {
...
int ObLSService::create_ls_for_restore()
{
  ...
  const ObRestoreStatus restore_status(ObRestoreStatus::Status::RESTORE_DOING); 
  const SCN create_scn = SCN::min_scn(); 
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_UNLIKELY(!is_running_)) {
      ...
      // Use RESTORE_TENANT_ROLE for standby restore (key difference from normal create)
      } else if (OB_BREAK_FAIL(ls->create_ls(RESTORE_TENANT_ROLE/*tenant_role*/,
                                             palf_base_info,
      ...
      // Skip create_ls_inner_tablet for restore - inner tablets will be copied from source
      ...
      } else if (OB_BREAK_FAIL(post_create_ls_(ObLSCreateType::RESTORE/*create_type*/, ls))) {
      ...
      } else if (OB_FAIL(ls->set_initial_member_list(member_list, 1/*paxos_replica_num*/, learner_list))) {
  ...
  if (OB_LS_EXIST == ret) {
    // If LS already exists, consider it success for idempotency
    ret = OB_SUCCESS;
    LOG_INFO("ls already exists, reuse it");
  }
  FLOG_INFO("create_ls_for_restore finish", K(ret));
  return ret;
}
...
int ObLSService::create_ls_(const ObTenantRole &tenant_role)
{
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  int64_t abs_timeout_ts = INT64_MAX;
  ...
  palf::PalfBaseInfo palf_base_info;
  palf_base_info.generate_by_default();
  palf_base_info.prev_log_info_.scn_ = SCN::base_scn();
  const storage::ObMajorMVMergeInfo major_mv_merge_info;
  common::ObMemberList member_list;
  common::GlobalLearnerList learner_list;
  const ObMigrationStatus migration_status(ObMigrationStatus::OB_MIGRATION_STATUS_NONE);
  const ObRestoreStatus restore_status(ObRestoreStatus::Status::NONE);
  ...
  if (IS_NOT_INIT) {
    ...
    if (OB_UNLIKELY(!is_running_)) {
      ...
      } else if (OB_BREAK_FAIL(ls->create_ls(tenant_role/*tenant_role*/,
                                             palf_base_info,
                                             REPLICA_TYPE_FULL/*replica_type*/))) {
        LOG_WARN("enable ls palf failed", K(ret), K(ls_meta));
      } else if (FALSE_IT(state = ObLSCreateState::CREATE_STATE_PALF_ENABLED)) {
      ...
      } else if (OB_BREAK_FAIL(post_create_ls_(ObLSCreateType::NORMAL/*create_type*/, ls))) {
        LOG_WARN("post create ls failed", K(ret), K(ls_meta));
      } else if (OB_FAIL(member_list.add_member(ObMember(GCTX.self_addr(), 1/*timestamp*/)))) {
        LOG_WARN("failed to add member", KR(ret));
