...
int ObLSRestoreHandler::online()
{
  ...
  } else if (!new_status.is_restore_doing() && !new_status.is_restore_failed()) {
    set_is_online(true);
  } else {
...
int ObLSRestoreHandler::get_ls_restore_status(share::ObLSRestoreStatus &status) const
{
  int ret = OB_SUCCESS;
  lib::ObMutexGuard guard(mtx_);
  if (nullptr == state_handler_) {
    status = share::ObLSRestoreStatus(share::ObLSRestoreStatus::Status::NONE);
  } else {
