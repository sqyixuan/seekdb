    ...
    ObPLExecCtx &ctx, sql::ParamStore &params, common::ObObj &result)
{
  ...
  if (OB_FAIL(GET_MIN_DATA_VERSION(ctx.exec_ctx_->get_my_session()->get_effective_tenant_id(), data_version))) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (OB_FAIL(get_wr_begin_and_end_time(ash_report_params))) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_FAIL(get_wr_num_samples(ash_report_params, wr_num_samples))) {
    ...
    const AshReportParams &ash_report_params, ObSqlString &sql_string)
{
  ...
  const char* ash_view_ptr = ASH_VIEW_SQL_4352;
    ...
    const AshReportParams &ash_report_params, ObSqlString &sql_string)
{
  ...
  char port_buf[time_buf_len];
  if (OB_FAIL(usec_to_string(ash_report_params.tz_info, ash_report_params.wr_begin_time,
  ...
  } else if (OB_FAIL(sql_string.append_fmt(WR_VIEW_SQL_4352, 
                 lib::is_oracle_mode()
...
int ObDbmsWorkloadRepository::append_fmt_ash_wr_view_sql(const AshReportParams &ash_report_params, ObSqlString &sql_string) {
  int ret = OB_SUCCESS;
  if (OB_FAIL(sql_string.append("SELECT * FROM ("))) {
        ...
        while (OB_SUCC(ret)) {
          if (OB_FAIL(result->next())) {
            ...
            if (OB_SUCC(ret) && 1 == program_rank) {
              ...
              } else if(OB_FAIL(GCTX.cgroup_ctrl_->get_group_path(group_path, 128))) {
          ...
          if (OB_FAIL(result->next())) {
            ...
            if (OB_SUCC(ret)) {
              ...
              if (OB_ISNULL(GCTX.cgroup_ctrl_)) {
                ...
                LOG_WARN("get group path failed", K(ret));
              } else {
                group_path[127] = '\0';
              }
              if (OB_SUCCESS != ret) {
