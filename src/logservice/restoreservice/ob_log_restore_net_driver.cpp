...
int ObLogRestoreNetDriver::set_source(share::ObRestoreSourceServiceAttr &source)
{
  RLockGuard guard(lock_);
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ...
    LOG_INFO("source set successfully, background thread will schedule later", K(source));
  }
  return ret;
}

bool ObLogRestoreNetDriver::has_source() const
{
  RLockGuard guard(lock_);
  return inited_ && source_.is_valid();
}

int ObLogRestoreNetDriver::get_saved_source(share::ObRestoreSourceServiceAttr &source) const
{
  RLockGuard guard(lock_);
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
    ret = OB_NOT_INIT;
    LOG_WARN("net driver not init", K(inited_));
  } else if (OB_UNLIKELY(!source_.is_valid())) {
    ret = OB_ENTRY_NOT_EXIST;
    LOG_WARN("saved source not valid", K(source_));
  } else if (OB_FAIL(source.assign(source_))) {
    LOG_WARN("assign source failed", K(source_));
  }
  return ret;
}

int ObLogRestoreNetDriver::do_schedule_with_saved_source()
{
  RLockGuard guard(lock_);
  int ret = OB_SUCCESS;
  if (OB_UNLIKELY(!inited_)) {
...
int ObLogRestoreNetDriver::refresh_fetcher_if_needed_(const share::ObRestoreSourceServiceAttr &source)
{
  ...
  //    Check a fetcher if is stale based on the source addr list,
  //    which means the log_restore_source changed.
  if (OB_SUCC(ret)) {
    if (NULL != fetcher_ && is_fetcher_stale_(source)) {
  ...
  // 2. Init or refresh proxy based on server list
  // Note: Only ip_list is supported now, user/password are not available
  if (OB_SUCC(ret)) {
  ...
  // 3. Init fetcher
  // Note: Use default cluster_id and tenant_id since only ip_list is available
  if (OB_SUCC(ret) && NULL == fetcher_) {
    const int64_t default_cluster_id = GCONF.cluster_id;
    const uint64_t default_tenant_id = OB_SYS_TENANT_ID; // Use sys tenant as default
    if (OB_FAIL(init_fetcher_if_needed_(default_cluster_id, default_tenant_id))) {
...
int ObLogRestoreNetDriver::refresh_proxy_(const share::ObRestoreSourceServiceAttr &source)
{
  ...
  if (proxy_.is_inited()) {
    if (OB_FAIL(proxy_.refresh_conn(source.addr_, default_user, default_passwd, db_name))) {
      LOG_WARN("refresh_conn failed", K(source));
    }
  } else if (OB_FAIL(proxy_.init(MTL_ID(), source.addr_, default_user, default_passwd, db_name))) {
...
bool ObLogRestoreNetDriver::is_fetcher_stale_(const share::ObRestoreSourceServiceAttr &source)
{
  ...
  if (NULL == fetcher_) {
    ...
    // Compare addr list to check if source changed
    // Note: Only ip_list is supported now, cluster_id and tenant_id are not available
    if (source_.addr_.count() != source.addr_.count()) {
      bret = true;
    } else {
      for (int64_t i = 0; i < source_.addr_.count(); ++i) {
        bool found = false;
        for (int64_t j = 0; j < source.addr_.count(); ++j) {
          if (source_.addr_.at(i) == source.addr_.at(j)) {
            found = true;
            break;
          }
        }
