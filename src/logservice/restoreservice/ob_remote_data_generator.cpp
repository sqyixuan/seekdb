...
//
// Only support backup case, no need to handle non-atomic archive write situation
...
// Temporarily only support single fetch, continuous fetch of large data segments is controlled by upper layer
...
// To accelerate locating start file, rely on LSN -> file_id rule
...
int LocationDataGenerator::fetch_log_from_location_(char *&buf, int64_t &buf_size)
{
  ...
  if (OB_FAIL(get_precise_file_and_offset_(dest_id, round_id, piece_id,
    ...
    // Not the first time reading file, no need to parse file header again
...
// Because file_id is calculated from LSN, this file may be in current piece, or in previous one or multiple pieces, need to relocate
