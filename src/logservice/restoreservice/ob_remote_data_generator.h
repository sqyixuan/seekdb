...
struct RemoteDataBuffer
{
  ...
  int next(LogEntryType &entry, LSN &lsn, const char *&buf, int64_t &buf_size)
  {
    ...
    if (is_empty()) {
      ...
      // Current returned entry corresponding buffer and length
...
{
public:
  // TODO Consider physical restore log stream concurrent log fetching consumption, should use 64M and integer multiples, temporarily not considered
