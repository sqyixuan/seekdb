...
int ObRemoteLocationAdaptor::init(const uint64_t tenant_id, ObLSService *ls_svr, ObLogRestoreNetDriver *net_driver)
{
  int ret = OB_SUCCESS;
...
int ObRemoteLocationAdaptor::get_source_(share::ObLogRestoreSourceItem &item, bool &source_exist)
{
  int ret = OB_SUCCESS;
    source_exist = false;
    // If no source from table, check if net_driver_ has a saved source (set via set_source())
    // This handles the case where __all_log_restore_source table doesn't exist
    if (OB_NOT_NULL(net_driver_) && net_driver_->has_source()) {
      share::ObRestoreSourceServiceAttr service_attr;
      if (OB_FAIL(net_driver_->get_saved_source(service_attr))) {
        LOG_WARN("get_saved_source from net_driver failed", K(ret));
      } else {
        // Convert ObRestoreSourceServiceAttr to ObLogRestoreSourceItem
        char value_str[1024];
        if (OB_FAIL(service_attr.gen_service_attr_str(value_str, sizeof(value_str)))) {
          LOG_WARN("gen_service_attr_str failed", K(ret), K(service_attr));
        } else {
          // Construct ObLogRestoreSourceItem using constructor
          // For standby, until_scn_ is max (infinite point)
          share::SCN until_scn = share::SCN::max_scn();
          common::ObString value(strlen(value_str), value_str);
          share::ObLogRestoreSourceItem temp_item(
              service_attr.user_.tenant_id_,
              1,  // id: use 1 as default since we only have one source
              share::ObLogRestoreSourceType::SERVICE,
              value,
              until_scn);
          // Use deep_copy to copy to item (since operator= is disabled)
