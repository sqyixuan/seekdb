...
// PG tasks must be strictly guaranteed to increment, by the caller to ensure
// There cannot be a situation where 10th log is inserted before 8th log
int ObArchiveSender::submit_send_task(ObArchiveSendTask *task)
{
  ...
  // Submit send_task encounters archive stop, release task, return success
...
// Only tasks that need to be retried return error codes
...
// 1. No data has ever been archived out, can archive immediately -> ls_archive_task has no piece record
// 2. Unchunked piece, can be archived immediately -> ls_archive_task piece is the same as the current task piece
// 3. Need to wait for persistence -> persist_mgr piece is different from the current task piece, and LSN is not continuous
// 4. Need to compensate piece -> persist_mgr piece and current task piece are different, and LSN is continuous and piece_id difference is greater than 1
//    NOTE: Specify next_piece_id compensation, ensure all consecutive empty piece have the opportunity to compensate
    ...
    ObLSArchiveTask &ls_archive_task)
{
  ...
  // 7. Update log stream archive task archive file info
  } else {
