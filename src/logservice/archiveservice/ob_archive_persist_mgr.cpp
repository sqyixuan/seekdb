    ...
    const bool is_madatory, const ObLSArchivePersistInfo &info)
{
  ...
  if (OB_UNLIKELY(! info.is_valid())) {
    ...
    if (OB_FAIL(map_.alloc_value(value))) {
      ...
      // insert_and_get must be used in pairs with revert
      map_.revert(value);
      value = NULL;
    }
...
int ObArchivePersistMgr::do_persist_(const ObLSID &id, const bool exist, ObLSArchivePersistInfo &info)
{
  ...
  if (need_wipe_suspend_status_(state_) && OB_FAIL(do_wipe_suspend_status_(id))) {
    ...
    // 1. When the tenant archive status is BEGINNING / DOING, persist the archive progress if there is a log stream archive progress
    bool record_exist = false;
    ...
    if (! exist) {
      ...
      // Need to update archive progress scenario
      // 1. There is no persisted archive progress
      // 2. The persisted archive progress is invalid
      // 3. The persisted archive progress piece is smaller than the current value
      // 4. piece is the same, persisted archive progress piece is smaller than the current value
      // 5. Current archive stream is interrupted, while the persistent archive progress is not in an interrupted state
    ...
    // 2. Archive status is in STOPPING state, then set the archive status of all pieces of this log stream to STOP
