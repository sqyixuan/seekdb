...
void ObArchiveService::do_check_switch_archive_()
{
  ...
  if (OB_UNLIKELY(! inited_)) {
    ...
    // Start a new round of archiving
    ...
    ArchiveRoundOp &op)
{
  ...
  if (local_state.is_stopping()) {
    ...
    ARCHIVE_LOG(INFO, "need_force_stop", K(local_key), K(local_state), K(attr), K_(archive_round_mgr));
      ...
      && ! local_round_lag) {
    // Mark interrupt with caution, only set local to interrupt when the current round and in doing state, and tenant interrupt is encountered
