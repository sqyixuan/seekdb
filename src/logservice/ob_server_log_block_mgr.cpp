...
{
  int ret = OB_SUCCESS;
  int64_t log_disk_size = get_total_size_guarded_by_lock_();
  int64_t expected_log_disk_size = 0;
  int64_t unused_log_disk_percentage = 0;
  int64_t total_log_disk_size = 0;
  if (OB_FAIL(observer::ObServerUtils::get_log_disk_info_in_config(expected_log_disk_size,
  ...
  } else if (expected_log_disk_size < log_disk_size) {
                                                    ...
                                                    const block_id_t block_id)
{
  ...
  if (OB_FAIL(block_id_to_string(block_id, block_path, OB_MAX_FILE_NAME_LENGTH))) {
    ...
    ret = convert_sys_errno();
    CLOG_LOG(ERROR, "::ftruncate failed (macOS fallocate replacement)", K(ret), KPC(this), K(dir_fd), K(block_id),
             K(errno));
#else
