    ...
    common::ObAddr &svr)
{
  ...
  if (IS_NOT_INIT) {
    ...
    // For single machine, single tenant, single LS: 
    // First call returns restore_source, second call returns OB_ITER_END
    const common::ObAddr &restore_source = GCTX.get_restore_source();
    if (!restore_source.is_valid()) {
      ret = OB_ITER_END;
      LOG_WARN("restore_source is not valid, return OB_ITER_END", KR(ret));
    ...
    ObIArray<common::ObAddr> &svr_array)
{
  ...
  if (IS_NOT_INIT) {
    ...
    svr_array.reset();
    // For single machine, single tenant, single LS: hardcode to return restore_source
    const common::ObAddr &restore_source = GCTX.get_restore_source();
    if (!restore_source.is_valid()) {
      ret = OB_ERR_UNEXPECTED;
      LOG_WARN("restore_source is not valid", KR(ret), K(restore_source));
    ...
    common::ObAddr &leader)
{
  ...
  if (IS_NOT_INIT) {
    ...
    // For single machine, single tenant, single LS: hardcode to return restore_source
    leader = GCTX.get_restore_source();
    if (!leader.is_valid()) {
      ret = OB_NOT_MASTER;
      LOG_WARN("restore_source is not valid", KR(ret), K(leader));
    } else {
    ...
    const common::ObAddr &cur_svr)
{
  ...
  const common::ObAddr &restore_source = GCTX.get_restore_source();
  bool bool_ret = (restore_source.is_valid() && cur_svr != restore_source);
    ...
    int64_t &avail_svr_count) const
{
  ...
  if (IS_NOT_INIT) {
    ...
    // For single machine, single tenant, single LS: hardcode to return 1
    avail_svr_count = GCTX.get_restore_source().is_valid() ? 1 : 0;
