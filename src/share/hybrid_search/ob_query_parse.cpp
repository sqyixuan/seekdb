  ...
  if (OB_SUCC(ret)) {
    ...
    for (uint64_t i = 0; OB_SUCC(ret) && i < count; i++) {
      ...
      if (OB_FAIL(j_node->get_key(i, key))) {
        ...
        // do nothing
      } else if (key.case_compare("semantic_query") == 0) {
        is_semantic_query = true;
        if (OB_FAIL(parse_semantic_query(*req_node, query, knn))) {
          LOG_WARN("fail to parse semantic expr", K(ret));
        }
      } else {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (is_hybrid) {
        ...
        } else if (OB_FAIL(construct_hybrid_query(query, knn, query_req, is_semantic_query))) {
...
int ObESQueryParser::init_default_params(ObIJsonBase &req_node)
{
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_SEARCH_NOT_FOUND == ret) {
      ret = OB_SUCCESS;
      ObIJsonBase *semantic_node = NULL;
      if (OB_FAIL(req_node.get_object_value("semantic_query", semantic_node))) {
        if (OB_SEARCH_NOT_FOUND != ret) {
          LOG_WARN("failed to get semantic query", K(ret));
        }
      } else if (semantic_node == NULL || semantic_node->json_type() != ObJsonNodeType::J_OBJECT) {
        ret = OB_ERR_INVALID_TYPE_FOR_ARGUMENT;
        LOG_WARN("invalid semantic query", K(ret));
      } else if (OB_FAIL(semantic_node->get_object_value("k", size_node))) {
        if (OB_SEARCH_NOT_FOUND != ret) {
          LOG_WARN("failed to get semantic query k", K(ret));
        }
      } else if (OB_FAIL(parse_const(*size_node, default_size_, true))) {
        LOG_WARN("fail to parse semantic query k", K(ret));
      } else {
...
int ObESQueryParser::set_output_columns(ObQueryReqFromJson &query_res, bool is_hybrid, bool include_inner_column/* true */, bool is_semantic_query/* false */)
{
  int ret = OB_SUCCESS;
  query_res.output_all_columns_ = false;
  if (!query_res.is_score_item_exist() && !is_semantic_query) {
    ObReqColumnExpr *score_col = NULL;
...
int ObESQueryParser::construct_hybrid_query(ObQueryReqFromJson *fts, ObQueryReqFromJson *knn, ObQueryReqFromJson *&hybrid, bool is_semantic_query/*false*/)
{
  int ret = OB_SUCCESS;
  ObString rowkey_hint("opt_param('hidden_column_visible', 'true')");
  ...
  if (OB_FAIL(get_base_table_query(fts, base_table_fts_req))) {
    ...
    if (!out_cols_->empty()) {
      if (OB_FAIL(set_output_columns(*knn, false, false, is_semantic_query))) {
        LOG_WARN("fail to set output columns", K(ret));
  ...
  if (OB_FAIL(ret)) {
    ...
    } else if (OB_FAIL(construct_rank_query(empty_str, knn_score, vs_rank_alias, knn, is_semantic_query))) {
      LOG_WARN("fail to construct keyword rank query", K(ret));
...
int ObESQueryParser::construct_rank_query(ObString &sub_query_name, ObReqExpr *order_expr, ObString &rank_alias, ObQueryReqFromJson *&query_req, bool is_semantic_query)
{
  int ret = OB_SUCCESS;
  ...
  if (OB_FAIL(wrap_sub_query(sub_query_name, query_req))) {
    ...
    LOG_WARN("fail to construct order by item", K(ret));
  } else if (!is_semantic_query && OB_FAIL(ObReqWindowFunExpr::construct_window_fun_expr(alloc_, order_info, expr_name, rank_alias, rank_expr))) {
    LOG_WARN("fail to construct window fun expr", K(ret));
  } else if (is_semantic_query && OB_FAIL(ObReqWindowFunExpr::construct_window_fun_expr(alloc_, "ROW_NUMBER", rank_alias, rank_expr))) {
    LOG_WARN("fail to construct window fun expr", K(ret));
...
int ObESQueryParser::parse_query_string_fields(ObIJsonBase &req_node, ObEsQueryInfo &query_info, bool semantic_query)
{
  int ret = OB_SUCCESS;
...
int ObESQueryParser::parse_semantic_query(ObIJsonBase &req_node,
                                          ObQueryReqFromJson *&query_req,
                                          ObQueryReqFromJson *&knn_req)
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(parse_semantic_query_string(req_node, query_req))) {
    LOG_WARN("fail to parse query string expr", K(ret));
  } else if (OB_FAIL(parse_semantic_knn(req_node, knn_req))) {
    LOG_WARN("fail to parse knn expr", K(ret));
  } 
  return ret;
}


int ObESQueryParser::parse_semantic_knn(ObIJsonBase &req_node, ObQueryReqFromJson *&query_req)
{
  int ret = OB_SUCCESS;
  RequiredParamsSet required_params;
  const char *params_name[] = {"field", "k", "query"};
  if (req_node.json_type() != ObJsonNodeType::J_OBJECT) {
    ret = OB_ERR_INVALID_TYPE_FOR_ARGUMENT;
    LOG_WARN("unexpectd json type", K(ret), K(req_node.json_type()));
  } else if (OB_ISNULL(query_req = OB_NEWx(ObQueryReqFromJson, &alloc_))) {
  ...
  for (uint64_t i = 0; OB_SUCC(ret) && i < req_node.element_count(); i++) {
    ...
    if (OB_FAIL(req_node.get_key(i, key))) {
      ...
      if (OB_FAIL(parse_const(*sub_node, query_str, false, true))) {
        LOG_WARN("fail to parse query string", K(ret), K(i));
      } else if (OB_FAIL(required_params.erase_refactored("query"))) {
        LOG_WARN("fail to erase set", K(ret));
      }
    } else {
      ret = OB_NOT_SUPPORTED;
  ...
  if (OB_FAIL(ret)) {
    ...
    if (OB_FAIL(construct_semantic_ip_expr(vec_field, query_str, case_when_expr, add_expr, order_by_4ip))) {
      LOG_WARN("fail to construct semantic ip expr", K(ret));
    } else if (OB_FAIL(construct_order_by_item(order_by_4ip, true, order_info))) {
      LOG_WARN("fail to construct order by item", K(ret));
    // } else if (OB_FAIL(query_req->select_items_.push_back(add_expr))) {
    //   LOG_WARN("fail to push query item", K(ret));
    }
  } else if (OB_FAIL(set_semantic_distance_score_expr(alg_type, norm_const, dist_vec, add_expr, div_expr))) {
    LOG_WARN("fail to set semantic distance expr", K(ret));
  } else if (OB_FAIL(dist_vec->params.push_back(vec_field))) {
    LOG_WARN("fail to push vector distance expr param", K(ret));
  } else if (OB_FAIL(dist_vec->params.push_back(query_str))) {
    LOG_WARN("fail to push vector distance expr param", K(ret));
  } else if (OB_FAIL(construct_order_by_item(dist_vec, true, order_info))) {
    LOG_WARN("fail to construct order by item", K(ret));
  // } else if (OB_FAIL(query_req->select_items_.push_back(dist_vec))) {
  ...
  if (OB_SUCC(ret)) {
    ...
    if (OB_FAIL(ObReqConstExpr::construct_const_numeric_expr(round_const, alloc_, 8.0, ObIntType))) {
      ...
      LOG_WARN("fail to push query order item", K(ret));
    // } else if (OB_FAIL(query_req->add_score_item(alloc_, normalize_expr))) {
    //   LOG_WARN("fail to push query item", K(ret));
    }
  }
...
int ObESQueryParser::set_semantic_distance_score_expr(const ObVectorIndexDistAlgorithm alg_type,
                                                      ObReqConstExpr *&norm_const, ObReqExpr *&dist_vec,
                                                      ObReqOpExpr *&add_expr, ObReqOpExpr *&score_expr)
{
  int ret = OB_SUCCESS;
  switch (alg_type) {
    case ObVectorIndexDistAlgorithm::VIDA_L2 : {
      // l2_distance : score = 1 / (1 + l2_distance)
      if (OB_FAIL(ObReqExpr::construct_expr(dist_vec, alloc_, N_SEMANTIC_DISTANCE, ""))) {
        LOG_WARN("fail to create distance expr", K(ret));
      } else if (OB_FAIL(ObReqConstExpr::construct_const_numeric_expr(norm_const, alloc_, 1.0, ObIntType))) {
        LOG_WARN("fail to create const expr", K(ret));
      } else {
...
int ObESQueryParser::construct_semantic_ip_expr(ObReqColumnExpr *vec_field, ObReqConstExpr *query_str, ObReqCaseWhenExpr *&case_when/* score */,
                                       ObReqOpExpr *&minus_expr/* distance */, ObReqExpr *&order_by_vec)
{
  int ret = OB_SUCCESS;
  ObReqOpExpr *negative_expr = NULL;
  ...
  ObReqOpExpr *cmp_expr = NULL;
  ObReqOpExpr *add_expr = NULL;
  if (OB_FAIL(ObReqExpr::construct_expr(order_by_vec, alloc_, N_SEMANTIC_DISTANCE, vec_field, query_str))) {
    LOG_WARN("fail to create distance expr", K(ret));
  } else if (OB_FAIL(ObReqConstExpr::construct_const_numeric_expr(one_const, alloc_, 1.0, ObIntType))) {
    LOG_WARN("fail to create one const expr", K(ret));
  } else if (OB_FAIL(ObReqConstExpr::construct_const_numeric_expr(zero_const, alloc_, 0.0, ObIntType))) {
...
int ObESQueryParser::parse_semantic_query_string(ObIJsonBase &req_node, ObQueryReqFromJson *&query_req)
{
  int ret = OB_SUCCESS;
  if (OB_ISNULL(query_req = OB_NEWx(ObQueryReqFromJson, &alloc_))) {
    ret = OB_ALLOCATE_MEMORY_FAILED;
  ...
  ObEsQueryInfo query_info(query_req, QUERY_ITEM_QUERY, is_basic_query_);
  if (OB_SUCC(ret) && OB_FAIL(parse_semantic_query_string(req_node, query_info))) {
    LOG_WARN("unexpectd json type", K(ret));
  }
...
int ObESQueryParser::parse_semantic_query_string(ObIJsonBase &req_node, ObEsQueryInfo &query_info)
{
  int ret = OB_SUCCESS;
  uint32_t count = 0;
  if (query_info.query_item_ == QUERY_ITEM_UNKNOWN) {
  ...
  if (req_node.json_type() != ObJsonNodeType::J_OBJECT) {
    ...
    if (OB_SUCC(ret)) {
      if (OB_SUCC(parse_query_string_fields(req_node, query_info, true))) {
        parsed_keys++;
      } else {
        LOG_WARN("fail to parse query_string fields", K(ret));
      }
    ...
    if (OB_SUCC(ret)) {
      ObIJsonBase *k_node = nullptr;
      if (OB_FAIL(req_node.get_object_value("k", k_node))) {
        if (ret == OB_SEARCH_NOT_FOUND) {
          ret = OB_SUCCESS;
        } else {
          LOG_WARN("fail to get query field", K(ret));
