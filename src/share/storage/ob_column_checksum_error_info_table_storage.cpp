    ...
    ObIArray<ObColumnChecksumErrorInfo> &error_infos)
{
  ...
  if (!is_inited()) {
    ...
    const char *select_sql =
      "SELECT tenant_id, frozen_scn, index_type, data_table_id, index_table_id, "
      "       data_tablet_id, index_tablet_id, column_id, data_column_checksum, index_column_checksum "
      "FROM __all_column_checksum_error_info "
      "WHERE tenant_id = ? AND frozen_scn = ? AND index_type = ? "
    ...
    auto row_processor = [&](ObSQLiteRowReader &reader) -> int {
      ...
      error_info.tenant_id_ = tenant_id_val;
    ...
    int64_t limit)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt("DELETE FROM __all_column_checksum_error_info "
                                ...
                                "  WHERE tenant_id = %lu AND frozen_scn < %lu "
                                "  ORDER BY frozen_scn, tenant_id "
                                "  LIMIT %ld"
                                ")",
                                tenant_id, frozen_scn_before.get_val_for_inner_table_field(), limit))) {
