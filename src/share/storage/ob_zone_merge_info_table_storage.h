...
public:
  ...
  // Get zone merge info
  int get(const uint64_t tenant_id, const common::ObZone &zone, ObZoneMergeInfo &zone_merge_info);

  // Get all zone merge infos for a tenant
  int get_all(const uint64_t tenant_id, ObIArray<ObZoneMergeInfo> &zone_merge_infos);

  // Remove zone merge info by tenant_id and zone
  int remove(const uint64_t tenant_id, const common::ObZone &zone);
