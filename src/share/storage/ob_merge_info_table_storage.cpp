...
int ObMergeInfoTableStorage::insert_or_update(const ObGlobalMergeInfo &global_merge_info)
{
  ...
  if (!is_inited()) {
    ...
    const char *upsert_sql =
      ...
      "(tenant_id, cluster, frozen_scn, global_broadcast_scn, is_merge_error, "
    ...
    auto binder = [&](ObSQLiteBinder &b) -> int {
      b.bind_int64(global_merge_info.tenant_id_);
      b.bind_int64(global_merge_info.cluster_.get_value());
...
int ObMergeInfoTableStorage::get(const uint64_t tenant_id, ObGlobalMergeInfo &global_merge_info)
{
  ...
  if (!is_inited()) {
    ...
    const char *select_sql =
      "SELECT tenant_id, cluster, frozen_scn, global_broadcast_scn, is_merge_error, "
      "       last_merged_scn, merge_status, error_type, suspend_merging, "
      "       merge_start_time, last_merged_time "
    ...
    auto row_processor = [&](ObSQLiteRowReader &reader) -> int {
      ...
      int64_t cluster = reader.get_int64();
      ...
      global_merge_info.tenant_id_ = tenant_id_val;
      global_merge_info.cluster_.set_val(cluster, false);
