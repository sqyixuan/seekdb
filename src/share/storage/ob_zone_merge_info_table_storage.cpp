...
int ObZoneMergeInfoTableStorage::insert_or_update(const ObZoneMergeInfo &zone_merge_info)
{
  ...
  if (!is_inited()) {
    ...
    const char *upsert_sql =
      ...
      "(tenant_id, zone, all_merged_scn, broadcast_scn, frozen_scn, "
    ...
    auto binder = [&](ObSQLiteBinder &b) -> int {
      b.bind_int64(zone_merge_info.tenant_id_);
      b.bind_text(zone_merge_info.zone_.ptr(), zone_merge_info.zone_.size());
...
int ObZoneMergeInfoTableStorage::get(const uint64_t tenant_id, const common::ObZone &zone, ObZoneMergeInfo &zone_merge_info)
{
  int ret = OB_SUCCESS;
  if (!is_inited()) {
    ret = OB_NOT_INIT;
    ...
    // Construct array with single zone element to use as query condition
    ObSEArray<ObZoneMergeInfo, 1> infos;
    ObZoneMergeInfo filter_info;
    filter_info.zone_ = zone;
    if (OB_FAIL(infos.push_back(filter_info))) {
      LOG_WARN("failed to push back filter info", K(ret));
    } else if (OB_FAIL(get_all(tenant_id, infos))) {
...
{
  int ret = OB_SUCCESS;
  // Save input zones as query conditions before reset
  ObSEArray<common::ObZone, 8> filter_zones;
  const bool has_filter = (zone_merge_infos.count() > 0);
  if (has_filter) {
    for (int64_t i = 0; OB_SUCC(ret) && i < zone_merge_infos.count(); ++i) {
      if (OB_FAIL(filter_zones.push_back(zone_merge_infos.at(i).zone_))) {
        LOG_WARN("failed to push back filter zone", K(ret));
      }
    }
  }
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        "SELECT tenant_id, zone, all_merged_scn, broadcast_scn, frozen_scn, "
        "       is_merging, last_merged_time, last_merged_scn, merge_start_time, merge_status "
        "FROM __all_zone_merge_info "
        "WHERE tenant_id = %lu",
        tenant_id))) {
      LOG_WARN("failed to append sql", K(ret));
    } else if (has_filter && filter_zones.count() > 0) {
      // Add zone filter condition
      if (OB_FAIL(sql.append(" AND zone IN ("))) {
        LOG_WARN("failed to append sql", K(ret));
      } else {
        for (int64_t i = 0; OB_SUCC(ret) && i < filter_zones.count(); ++i) {
          if (OB_FAIL(sql.append_fmt("%s'%s'", i == 0 ? "" : ", ", filter_zones.at(i).ptr()))) {
            LOG_WARN("failed to append zone", K(ret));
          }
        }
    ...
    if (OB_SUCC(ret) && OB_FAIL(sql.append(" ORDER BY zone;"))) {
      LOG_WARN("failed to append sql", K(ret));
    }
    ...
    if (OB_SUCC(ret)) {
      auto row_processor = [&](ObSQLiteRowReader &reader) -> int {
        ...
        int64_t tenant_id_val = reader.get_int64();
        int zone_len = 0;
        ...
        zone_merge_info.tenant_id_ = tenant_id_val;
        // Use ObString constructor with length to avoid strlen() issue with binary data
        if (OB_NOT_NULL(zone_str) && zone_len > 0) {
          common::ObString zone_obstr(zone_len, zone_str);
          zone_merge_info.zone_ = common::ObZone(zone_obstr);
        } else {
          zone_merge_info.zone_.reset();
        }
...
int ObZoneMergeInfoTableStorage::remove(const uint64_t tenant_id, const common::ObZone &zone)
{
  int ret = OB_SUCCESS;
  if (!is_inited()) {
    ret = OB_NOT_INIT;
    ...
    const char *delete_sql =
      "DELETE FROM __all_zone_merge_info "
      "WHERE tenant_id = ? AND zone = ?;";

    auto binder = [&](ObSQLiteBinder &b) -> int {
      b.bind_int64(tenant_id);
      b.bind_text(zone.ptr(), zone.size());
      return OB_SUCCESS;
    };
