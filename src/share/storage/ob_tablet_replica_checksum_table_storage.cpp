    ...
    const ObIArray<ObTabletReplicaChecksumItem> &items)
{
  ...
  if (!is_inited()) {
    ...
    const char *upsert_sql =
      ...
      "(tenant_id, tablet_id, svr_ip, svr_port, ls_id, compaction_scn, "
      " row_count, data_checksum, column_checksums, b_column_checksums, "
      " data_checksum_type, co_base_snapshot_version) "
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
      "ON CONFLICT(tenant_id, tablet_id, svr_ip, svr_port, ls_id) DO UPDATE SET "
      ...
      if (OB_FAIL(guard->begin_transaction())) {
        ...
        if (OB_FAIL(guard->prepare_execute(upsert_sql, stmt))) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < items.count(); ++i) {
            ...
            if (OB_UNLIKELY(!item.server_.ip_to_string(ip, sizeof(ip)))) {
              ret = OB_INVALID_ARGUMENT;
              LOG_WARN("ip to string failed", K(ret), K(item.server_));
            } else {
              ...
              auto binder = [&](ObSQLiteBinder &b) -> int {
                b.bind_int64(item.tenant_id_);
                b.bind_int64(item.tablet_id_.id());
                b.bind_text(ip, strlen(ip));
                b.bind_int(item.server_.get_port());
                b.bind_int64(item.ls_id_.id());
        ...
        if (OB_FAIL(guard->prepare_execute(delete_sql, stmt))) {
          ...
          for (int64_t i = 0; OB_SUCC(ret) && i < replicas.count(); ++i) {
            ...
            if (replica.primary_keys_are_valid() && tenant_id == replica.get_tenant_id()) {
              char ip[OB_MAX_SERVER_ADDR_SIZE] = "";
              if (OB_UNLIKELY(!replica.get_server().ip_to_string(ip, sizeof(ip)))) {
                ret = OB_INVALID_ARGUMENT;
                LOG_WARN("ip to string failed", K(ret), K(replica.get_server()));
              } else {
                auto binder = [&](ObSQLiteBinder &b) -> int {
    ...
    const bool include_larger_than)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        "SELECT tenant_id, tablet_id, svr_ip, svr_port, ls_id, compaction_scn, "
        "       row_count, data_checksum, column_checksums, b_column_checksums, "
        "       data_checksum_type, co_base_snapshot_version "
    ...
    if (OB_SUCC(ret)) {
      ...
      auto row_processor = [&](ObSQLiteRowReader &reader) -> int {
        ...
        int64_t ls_id_val = reader.get_int64();
        ...
        if (OB_ISNULL(ip_str)) {
          ...
          item.tenant_id_ = tenant_id_val;
          item.tablet_id_ = ObTabletID(tablet_id_val);
          item.ls_id_ = ObLSID(ls_id_val);
          item.server_ = server;
    ...
    bool &has_error)
{
  ...
  if (!is_inited()) {
    ...
    ObSqlString sql;
    if (OB_FAIL(sql.append("SELECT tenant_id, tablet_id FROM ("))) {
      LOG_WARN("failed to append sql", K(ret));
    } else if (OB_FAIL(sql.append("SELECT tenant_id, tablet_id, row_count, data_checksum, b_column_checksums, compaction_scn FROM __all_tablet_replica_checksum WHERE tenant_id = "))) {
      LOG_WARN("failed to append sql", K(ret));
    } else if (OB_FAIL(sql.append_fmt("%lu AND tablet_id IN (", tenant_id))) {
      LOG_WARN("failed to append sql", K(ret));
    } else {
      // Build IN clause
