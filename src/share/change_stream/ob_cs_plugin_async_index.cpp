...
ObCSAsyncIndexProcessor::ObCSAsyncIndexProcessor(ObCSExecCtx &ctx)
  : ctx_(ctx), vec_index_cache_created_(false)
{
}
...
{
  if (vec_index_cache_created_) {
    vec_index_cache_.destroy();
    vec_index_cache_created_ = false;
  }
}
...
{
  LOG_INFO("ob_cs_plugin_start_process", K(rows.count()));
  int ret = common::OB_SUCCESS;
  if (rows.count() == 0) {
    // nothing to process
  } else if (!vec_index_cache_created_) {
    if (OB_FAIL(vec_index_cache_.create(64, "CSAsyncIdxCa"))) {
      LOG_WARN("create vec_index_cache failed", K(ret));
    } else {
      vec_index_cache_created_ = true;
    }
  }

  if (OB_SUCC(ret) && rows.count() > 0) {
    if (OB_FAIL(init_schema_guard_())) {
  ...
  if (OB_SUCC(ret) && rows.count() > 0) {
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < rows.count(); ++i) {
      ...
      if (OB_FAIL(resolve_table_id_from_tablet_id_(row.tablet_id_, data_table_id))) {
        ...
        ret = common::OB_ERR_UNEXPECTED;
        LOG_WARN("resolved table_id is invalid", K(ret), K(row.tablet_id_), K(row));
      } else {
    ...
    bool &skip)
{
  ...
  if (skip) {
    ...
    LOG_WARN("INSERT/UPDATE row has no new_row data", K(ret), K(row));
             ...
             && OB_FAIL(extract_vector_data_(row.new_row_, vec_info, vec_data, vec_len))) {
    LOG_WARN("fail to extract vector data from new_row", K(ret), K(row), K(vec_info));
  } else if (OB_FAIL(scn.convert_for_tx(ctx_.refresh_scn_))) {
    LOG_WARN("fail to convert refresh_scn to SCN", K(ret), K(ctx_.refresh_scn_));
  } else {
    vid = row.heap_pk_;
    ...
    LOG_INFO("built async index event", K(event), K(vec_info),
             "scn_val", scn.get_val_for_tx(), "vid", vid,
             "type", event_type, "vec_len", vec_len,
             "vec_data", (void*)vec_data);
  }
  return ret;
}
    ...
    sql::ObDASInsCtDef *&ins_ctdef)
{
  ...
  if (OB_ISNULL(insert_op)) {
    ...
    if (OB_FAIL(schema_guard_.get_table_schema(
      ...
      ins_ctdef->schema_version_ = ctx_.schema_version_;
      ...
      if (OB_FAIL(ins_ctdef->column_ids_.reserve(column_count)) ||
        ...
        for (int64_t i = 0; OB_SUCC(ret) && i < sorted_columns.count(); ++i) {
          ...
          if (OB_ISNULL(column_schema)) {
            ...
            LOG_INFO("Added column to ins_ctdef",
                    "col_id", column_schema->get_column_id(),
                    "col_name", column_schema->get_column_name_str(),
                    "rowkey_pos", column_schema->get_rowkey_position(),
                    "index_in_array", ins_ctdef->column_ids_.count() - 1);
          }
        }
      }
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < events.count(); ++i) {
      ...
      if (OB_FAIL(datum_row.init(allocator, col_count))) {
        ...
        if (scn_idx >= 0 && scn_idx < col_count) {
          datum_row.storage_datums_[scn_idx].set_uint(event.scn_.get_val_for_tx());
        }
      ...
      if (OB_FAIL(ret)) {
        ...
        if (OB_FAIL(shadow_row.shadow_copy(datum_row))) {
          ...
          if (OB_FAIL(insert_buffer.add_row(shadow_row, &stored_row))) {
            ...
            LOG_INFO("Added event to insert_buffer for 4th table", K(i),
                     "scn", event.scn_.get_val_for_tx(), "vid", event.vid_,
                     "type", event.type_, K(scn_idx), K(vid_idx), K(type_idx), K(vector_idx));
          }
        }
      }
    }
    ...
    const ObCSVecIndexInfo &target_vec_info)
{
  ...
  LOG_INFO("insert_vector_index_log_batch_ start",
            "event_cnt", events.count(), K(target_vec_info),
            "first_event", events.count() > 0 ? events.at(0) : ObASyncIndexEvent());
    ...
    if (OB_FAIL(build_das_ins_ctdef_(das_allocator, target_vec_info, insert_op, ins_ctdef))) {
      ...
      if (OB_FAIL(insert_op->open_op())) {
        ...
        if (!is_domain_index && affected_rows != expected_rows) {
          ...
          LOG_INFO("insert_vector_index_log_batch succeeded",
                  K(affected_rows), K(expected_rows), K(target_vec_info.index_id_table_id_));
        }
      }
    }
  }
          ...
          if (OB_ISNULL(vids = static_cast<int64_t *>(tmp_allocator.alloc(sizeof(int64_t) * insert_count)))) {
            ...
            if (OB_SUCC(ret) && insert_count > 0) {
              ...
              if (OB_FAIL(obvectorutil::add_index(index_handler,
                ...
                LOG_INFO("Successfully added vectors to vsag index",
                          K(insert_count), K(dim), K(ls_id), K(vec_info.index_id_table_id_));
              }
            }
          }
        }
...
int ObCSPluginAsyncIndex::init()
{
  ...
  if (is_inited_) {
    ...
    is_inited_ = true;
    LOG_INFO("ObCSPluginAsyncIndex inited");
  }
  return ret;
}
