...
/// Link node for queue: holds a pointer to ObCSExecSubTask (ownership transferred on push).
struct ObCSSubTaskLinkNode : public common::LinkTask
{
  ObCSExecSubTask *sub_task_;
  ObCSSubTaskLinkNode() : sub_task_(nullptr) {}
  explicit ObCSSubTaskLinkNode(ObCSExecSubTask *st) : sub_task_(st) {}
};
...
public:
  ...
  /// Push a subtask (allocates link node; ownership of sub_task transfers to executor).
