 ...
 * Plugins implement map (per-subtask) and reduce (batch commit, advance scn).
 */
...
// ---------------------------------------------------------------------------
// Plugin type and abstract interface (map / reduce)
// ---------------------------------------------------------------------------
...
/// Change Stream framework interacts with external logic via plugins: register / map / reduce.
class ObCSPlugin
{
public:
  ...
  virtual int map(common::ObIArray<ObCSRow> &rows, ObCSExecCtx &ctx) = 0;
