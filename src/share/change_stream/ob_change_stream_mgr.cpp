...
int ObChangeStreamMgr::mtl_init(ObChangeStreamMgr *&mgr)
{
  ...
  if (OB_ISNULL(mgr)) {
    ...
    LOG_INFO("ObChangeStreamMgr mtl_init success", "tenant_id", MTL_ID());
  }
  return ret;
}
...
int ObChangeStreamMgr::init()
{
  ...
  } else if (OB_FAIL(worker_.init())) {
    LOG_WARN("ObChangeStreamMgr: worker init failed", K(ret));
  } else {
    is_inited_ = true;
    LOG_INFO("ObChangeStreamMgr init success (Fetcher/Dispatcher/Worker)");
  }
  return ret;
}
