...
int ObCSFetcher::init_consumption_position_()
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_ISNULL(GCTX.schema_service_)) {
      ...
      current_scn_.set_min();
      LOG_WARN("CSFetcher: min_dep_lsn is 0, schema_version 0");
    } else {
      ...
      if (OB_FAIL(iter_.next())) {
        if (OB_ITER_END == ret) {
          ret = OB_SUCCESS;
          current_scn_.set_min();
          LOG_WARN("CSFetcher: no logs at min_dep_lsn, schema_version 0");
        } else {
        ...
        if (timestamp_us <= 0) {
          LOG_WARN("CSFetcher: scn convert_to_ts invalid, schema_version 0");
        } else {
        ...
        if (OB_SUCC(ret) && OB_FAIL(logservice::seek_log_iterator(ls_id_, palf::LSN(persisted_min_dep_lsn), iter_))) {
...
void ObCSFetcher::run1()
{
  ...
  // Retry init_consumption_position_ until success or stop.
