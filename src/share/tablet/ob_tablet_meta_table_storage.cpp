...
int ObTabletReplicaConstructor::operator()(share::ObSQLiteRowReader &reader, ObTabletReplica &replica)
{
  ...
  if (OB_ISNULL(ip_str)) {
    ...
    LOG_WARN("fail to init replica", K(ret),
        K(tenant_id), K(tablet_id), K(server), K(data_size), K(required_size));
  }
  ...
  if (!is_inited()) {
    ...
    if (OB_UNLIKELY(!addr.ip_to_string(ip, sizeof(ip)))) {
      ...
      const char *select_sql =
        "SELECT tenant_id, tablet_id, svr_ip, svr_port, "
    ...
    ObTabletInfo &tablet_info)
{
  ...
  if (!is_inited()) {
    ...
    if (!guard) {
      ...
      if (OB_FAIL(tablet_info.init(tenant_id, tablet_id, ObLSID(ObLSID::SYS_LS_ID), replicas))) {
    ...
    ObIArray<ObTabletInfo> &tablet_infos)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        ...
        "       compaction_scn, data_size, required_size, report_scn, status "
        "FROM __all_tablet_meta_table "
        "WHERE tenant_id = %lu AND tablet_id IN (",
        tenant_id))) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(sql.append(") ORDER BY tenant_id, tablet_id, svr_ip, svr_port;"))) {
          LOG_WARN("failed to append sql", K(ret));
        }
      }
    }
    ...
    ObIArray<ObTabletInfo> &tablet_infos)
{
  ...
  if (!is_inited()) {
    ...
    const char *select_sql =
      ...
      "FROM __all_tablet_meta_table "
      "WHERE tenant_id = ? AND tablet_id > ? "
      "ORDER BY tablet_id, svr_ip, svr_port "
      "LIMIT ?;";
    ...
    const ObIArray<ObTabletReplica> &replicas)
{
  ...
  if (OB_ISNULL(conn)) {
    ...
    const char *upsert_sql =
      ...
      "(gmt_create, gmt_modified, tenant_id, tablet_id, svr_ip, svr_port, "
      " compaction_scn, data_size, required_size, report_scn, status) "
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    if (!guard) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < tablet_ls_pairs.count(); ++i) {
        ...
        const char *update_sql =
          ...
          "SET status = ? "
          "WHERE tenant_id = ? AND tablet_id = ? AND compaction_scn = ?;";
    ...
    int64_t &affected_rows)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        ...
        "WHERE tenant_id = %lu AND tablet_id IN (",
        major_frozen_scn, major_frozen_scn, tenant_id))) {
      LOG_WARN("failed to append sql", K(ret));
    } else {
