...
int ObTabletReplicaConstructor::operator()(share::ObSQLiteRowReader &reader, ObTabletReplica &replica)
{
  ...
  int64_t tenant_id = reader.get_int64();
  int64_t tablet_id = reader.get_int64();
  int64_t ls_id = reader.get_int64();
  const char *ip_str = reader.get_text();
  int32_t port = reader.get_int();
  ...
  } else if (OB_UNLIKELY(!server.set_ip_addr(ip_str, port))) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("invalid server address", K(ret), K(ip_str), K(port));
    ...
    ObTabletReplica &tablet_replica)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_UNLIKELY(!addr.ip_to_string(ip, sizeof(ip)))) {
      ret = OB_INVALID_ARGUMENT;
      LOG_WARN("convert server ip to string failed", K(ret), K(addr));
    } else {
      const char *select_sql =
        "SELECT tenant_id, tablet_id, ls_id, svr_ip, svr_port, "
        "       compaction_scn, data_size, required_size, report_scn, status "
        "FROM __all_tablet_meta_table "
        "WHERE tenant_id = ? AND tablet_id = ? AND ls_id = ? "
        "  AND svr_ip = ? AND svr_port = ?;";

      auto binder = [&](share::ObSQLiteBinder &b) -> int {
        b.bind_int64(tenant_id);
        b.bind_int64(tablet_id.id());
        b.bind_int64(ls_id.id());
        b.bind_text(ip, strlen(ip));
        b.bind_int(addr.get_port());
        return OB_SUCCESS;
      };
    ...
    ObTabletInfo &tablet_info)
{
  ...
  if (!is_inited()) {
    ...
    if (!guard) {
      ...
      if (OB_FAIL(tablet_info.init(tenant_id, tablet_id, ls_id, replicas))) {
    ...
    ObIArray<ObTabletInfo> &tablet_infos)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        ...
        "       compaction_scn, data_size, required_size, report_scn, status "
        "FROM __all_tablet_meta_table "
        "WHERE tenant_id = %lu AND (tablet_id, ls_id) IN (",
        tenant_id))) {
      ...
      if (OB_SUCC(ret)) {
        if (OB_FAIL(sql.append(") ORDER BY tenant_id, tablet_id, ls_id, svr_ip, svr_port;"))) {
          LOG_WARN("failed to append sql", K(ret));
        }
      }
    }
    ...
    ObIArray<ObTabletInfo> &tablet_infos)
{
  ...
  if (!is_inited()) {
    ...
    const char *select_sql =
      ...
      "       compaction_scn, data_size, required_size, report_scn, status "
      "FROM __all_tablet_meta_table "
      "WHERE tenant_id = ? AND tablet_id > ? "
      "ORDER BY tablet_id, ls_id, svr_ip, svr_port "
      "LIMIT ?;";
    ...
    const ObIArray<ObTabletReplica> &replicas)
{
  ...
  if (OB_ISNULL(conn)) {
    ...
    const char *upsert_sql =
      ...
      "(gmt_create, gmt_modified, tenant_id, tablet_id, ls_id, svr_ip, svr_port, "
      " compaction_scn, data_size, required_size, report_scn, status) "
      "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
    ...
    if (OB_FAIL(conn->prepare_execute(upsert_sql, stmt))) {
      ...
      while (OB_SUCC(ret) && current_index < replicas.count()) {
        ...
        if (replica.is_valid() && tenant_id == replica.get_tenant_id()) {
          auto binder = [&](share::ObSQLiteBinder &b) -> int {
            char ip[OB_MAX_SERVER_ADDR_SIZE] = {'\0'};
            if (replica.get_server().is_valid()) {
              replica.get_server().ip_to_string(ip, sizeof(ip));
            }
            b.bind_int64(current_time); // gmt_create
            b.bind_int64(current_time); // gmt_modified
            b.bind_int64(replica.get_tenant_id());
            b.bind_int64(replica.get_tablet_id().id());
            b.bind_int64(replica.get_ls_id().id());
            b.bind_text(ip, strlen(ip));
            b.bind_int(replica.get_server().get_port());
    ...
    const ObIArray<ObTabletReplica> &replicas)
{
  ...
  if (OB_ISNULL(conn)) {
    ...
    if (OB_FAIL(conn->prepare_execute(delete_sql, stmt))) {
      ...
      while (OB_SUCC(ret) && current_index < replicas.count()) {
        const ObTabletReplica &replica = replicas.at(current_index++);
        if (replica.primary_keys_are_valid() && tenant_id == replica.get_tenant_id()) {
          auto binder = [&](share::ObSQLiteBinder &b) -> int {
            char ip[OB_MAX_SERVER_ADDR_SIZE] = {'\0'};
            if (replica.get_server().is_valid()) {
              replica.get_server().ip_to_string(ip, sizeof(ip));
    ...
    if (!guard) {
      ...
      for (int64_t i = 0; OB_SUCC(ret) && i < tablet_ls_pairs.count(); ++i) {
        ...
        const char *update_sql =
          ...
          "SET status = ? "
          "WHERE tenant_id = ? AND tablet_id = ? AND ls_id = ? AND compaction_scn = ?;";

        auto binder = [&](share::ObSQLiteBinder &b) -> int {
          b.bind_int64(status);
          b.bind_int64(tenant_id);
          b.bind_int64(pair.get_tablet_id().id());
          b.bind_int64(pair.get_ls_id().id());
    ...
    int64_t &affected_rows)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        ...
        "WHERE tenant_id = %lu AND ls_id = %ld AND tablet_id IN (",
        major_frozen_scn, major_frozen_scn, tenant_id, ls_id.id()))) {
      LOG_WARN("failed to append sql", K(ret));
    } else {
    ...
    int64_t &affected_rows)
{
  ...
  if (!is_inited()) {
    ...
    const char *update_sql =
      "UPDATE __all_tablet_meta_table "
      "SET report_scn = ? "
      "WHERE tenant_id = ? AND tablet_id >= ? AND tablet_id <= ? "
    ...
    int64_t &affected_rows)
{
  ...
  if (!is_inited()) {
    ...
    const char *update_sql =
      "UPDATE __all_tablet_meta_table "
      "SET status = ? "
      "WHERE tenant_id = ? AND tablet_id >= ? AND tablet_id <= ? AND status = ?;";
    ...
    ObIArray<ObTabletInfo> &tablet_infos)
{
  ...
  if (!is_inited()) {
    ...
    if (OB_FAIL(sql.append_fmt(
        ...
        "       compaction_scn, data_size, required_size, report_scn, status "
        "FROM __all_tablet_meta_table "
        "WHERE tenant_id = %lu AND tablet_id > %ld AND tablet_id <= %ld",
        tenant_id, start_tablet_id.id(), end_tablet_id.id()))) {
      LOG_WARN("failed to append sql", K(ret));
