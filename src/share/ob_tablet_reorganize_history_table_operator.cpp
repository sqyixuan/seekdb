  ...
  if (tenant_id == common::OB_INVALID_TENANT_ID || !ls_id.is_valid()) {
    ...
    SMART_VAR(ObMySQLProxy::MySQLResult, res) {
      ...
      if (OB_FAIL(sql_string.assign_fmt("SELECT src_tablet_id, dest_tablet_id FROM %s WHERE "
              " tenant_id = %lu", OB_ALL_TABLET_REORGANIZE_HISTORY_TNAME, zero_tenant_id))) {
    ...
    ObIArray<ReorganizeTabletPair> &tablet_pairs)
{
  ...
  if (tenant_id == common::OB_INVALID_TENANT_ID || !ls_id.is_valid()) {
    ...
    SMART_VAR(ObMySQLProxy::MySQLResult, res) {
      ...
      const uint64_t zero_tenant_id = 0; 
      if (OB_FAIL(sql_string.assign_fmt("SELECT src_tablet_id, dest_tablet_id FROM %s WHERE "
              " tenant_id = %lu and src_tablet_id = %ld", OB_ALL_TABLET_REORGANIZE_HISTORY_TNAME, zero_tenant_id, tablet_id.id()))) {
        LOG_WARN("assign sql string failed", K(ret), K(tenant_id));
      } else if (OB_FAIL(sql_proxy.read(res, tenant_id, sql_string.ptr()))) {
        LOG_WARN("read tablet ids from all tablet reorganize history table failed", K(ret), K(sql_string));
      } else if (OB_ISNULL(result = res.get_result())) {
    ...
    const ObIArray<ObTabletID> &dest_tablet_ids)
{
  ...
  if (OB_UNLIKELY(dest_tablet_ids.count() == 0)) {
    ...
    if (OB_FAIL(insert_sql.assign_fmt("INSERT INTO %s (tenant_id, src_tablet_id, dest_tablet_id, "
    ...
    for (int64_t i = 0; OB_SUCC(ret) && i < dest_tablet_ids.count(); ++i) {
      ...
      if (OB_FAIL(insert_sql.append_fmt("(%lu, %ld, %ld, %ld, usec_to_time(%lu), usec_to_time(%lu))",
          ObSchemaUtils::get_extract_tenant_id(complete_record.tenant_id_, complete_record.tenant_id_), complete_record.src_tablet_id_.id(),
