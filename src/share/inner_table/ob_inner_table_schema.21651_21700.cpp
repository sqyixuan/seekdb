...
int ObInnerTableSchema::gv_ob_sql_ccl_status_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  ...
  table_schema.set_table_id(OB_GV_OB_SQL_CCL_STATUS_TID);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_GV_OB_SQL_CCL_STATUS_TNAME))) {
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(           SELECT           TENANT_ID as CON_ID,           SVR_IP,           SVR_PORT,           CCL_RULE_ID,           FORMAT_SQLID,           CURRENT_CONCURRENCY,           MAX_CONCURRENCY         FROM oceanbase.__all_virtual_ccl_status )__"))) {
...
int ObInnerTableSchema::v_ob_sql_ccl_status_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(           SELECT           TENANT_ID as CON_ID,           SVR_IP,           SVR_PORT,           CCL_RULE_ID,           FORMAT_SQLID,           CURRENT_CONCURRENCY,           MAX_CONCURRENCY         FROM oceanbase.__all_virtual_ccl_status         WHERE SVR_IP=HOST_IP() AND SVR_PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_mview_running_jobs_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_CDB_MVIEW_RUNNING_JOBS_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_CDB_MVIEW_RUNNING_JOBS_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
...
int ObInnerTableSchema::cdb_ob_dynamic_partition_tables_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_CDB_OB_DYNAMIC_PARTITION_TABLES_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_CDB_OB_DYNAMIC_PARTITION_TABLES_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     A.TENANT_ID AS TENANT_ID,     C.DATABASE_NAME AS DATABASE_NAME,     A.TABLE_NAME AS TABLE_NAME,     A.TABLE_ID AS TABLE_ID,     B.HIGH_BOUND_VAL AS MAX_HIGH_BOUND_VAL,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 1), '=', -1) AS ENABLE,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 2), '=', -1) AS TIME_UNIT,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 3), '=', -1) AS PRECREATE_TIME,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 4), '=', -1) AS EXPIRE_TIME,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 5), '=', -1) AS TIME_ZONE,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 6), '=', -1) AS BIGINT_PRECISION   FROM     oceanbase.__all_virtual_table A   JOIN     oceanbase.__all_virtual_database C   ON     A.TENANT_ID = C.TENANT_ID   AND     A.DATABASE_ID = C.DATABASE_ID   JOIN   (     SELECT       TENANT_ID,       TABLE_ID,       PART_IDX,       HIGH_BOUND_VAL,       ROW_NUMBER() OVER (         PARTITION BY TENANT_ID, TABLE_ID         ORDER BY PART_IDX DESC       ) AS rn     FROM       oceanbase.__all_virtual_part   ) B   ON     A.TENANT_ID = B.TENANT_ID   AND     A.TABLE_ID = B.TABLE_ID   AND     B.rn = 1   WHERE     A.DYNAMIC_PARTITION_POLICY != ''     AND C.DATABASE_NAME != '__recyclebin'     AND C.IN_RECYCLEBIN = 0; )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_dynamic_partition_tables_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     TENANT_ID,     TENANT_SCHEMA_VERSION,     DATABASE_NAME,     TABLE_NAME,     TABLE_ID,     MAX_HIGH_BOUND_VAL,     ENABLE,     TIME_UNIT,     PRECREATE_TIME,     EXPIRE_TIME,     TIME_ZONE,     BIGINT_PRECISION   FROM oceanbase.__all_virtual_dynamic_partition_table; )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_vector_memory_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_GV_OB_VECTOR_MEMORY_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_GV_OB_VECTOR_MEMORY_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
...
int ObInnerTableSchema::dba_ob_ai_model_endpoints_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       ENDPOINT_ID,       ENDPOINT_NAME,       AI_MODEL_NAME,       SCOPE,       URL,       ACCESS_KEY,       PROVIDER,       REQUEST_MODEL_NAME,       PARAMETERS,       REQUEST_TRANSFORM_FN,       RESPONSE_TRANSFORM_FN     FROM oceanbase.__all_virtual_ai_model_endpoint WHERE tenant_id = effective_tenant_id() AND ENDPOINT_ID != -1;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_ai_models_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_CDB_OB_AI_MODELS_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_CDB_OB_AI_MODELS_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       TENANT_ID,       MODEL_ID,       NAME,       case type         when 1 then 'DENSE_EMBEDDING'         when 2 then 'SPARSE_EMBEDDING'         when 3 then 'COMPLETION'         when 4 then 'RERANK'         else 'INVALID'       END AS TYPE,       MODEL_NAME     FROM oceanbase.__all_virtual_ai_model;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_ai_model_endpoints_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_CDB_OB_AI_MODEL_ENDPOINTS_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_CDB_OB_AI_MODEL_ENDPOINTS_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       TENANT_ID,       ENDPOINT_ID,       ENDPOINT_NAME,       AI_MODEL_NAME,       SCOPE,       URL,       ACCESS_KEY,       PROVIDER,       REQUEST_MODEL_NAME,       PARAMETERS,       REQUEST_TRANSFORM_FN,       RESPONSE_TRANSFORM_FN     FROM oceanbase.__all_virtual_ai_model_endpoint     WHERE ENDPOINT_ID != -1;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_hnsw_index_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_GV_OB_HNSW_INDEX_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_GV_OB_HNSW_INDEX_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT       A.SVR_IP,       A.SVR_PORT,       A.TENANT_ID,       A.DATA_TABLE_ID,       A.DATA_TABLET_ID,       T.TABLE_NAME,       SUBSTR(T1.TABLE_NAME, 7 + INSTR(SUBSTR(T1.TABLE_NAME, 7), '_')) AS INDEX_NAME,       (CASE M.ROLE WHEN 1 THEN 'LEADER' ELSE 'FOLLOWER' END) AS ROLE,       JSON_VALUE(A.STATISTICS, '$.param') AS INDEX_PARAM,       JSON_VALUE(A.STATISTICS, '$.incr_mem_used' RETURNING SIGNED) + JSON_VALUE(A.STATISTICS, '$.snap_mem_used' RETURNING SIGNED) AS MEMORY_USED,       JSON_VALUE(A.STATISTICS, '$.incr_mem_hold' RETURNING SIGNED) + JSON_VALUE(A.STATISTICS, '$.snap_mem_hold' RETURNING SIGNED) AS MEMORY_HOLD,       JSON_VALUE(A.STATISTICS, '$.incr_mem_hold' RETURNING SIGNED) AS INCR_MEM_HOLD,       JSON_VALUE(A.STATISTICS, '$.snap_mem_hold' RETURNING SIGNED) AS SNAP_MEM_HOLD,       JSON_VALUE(A.SYNC_INFO, '$.incr_cnt' RETURNING SIGNED) AS INCR_INDEX_CNT,       JSON_VALUE(A.SYNC_INFO, '$.snap_cnt' RETURNING SIGNED) AS SNAP_INDEX_CNT,       JSON_VALUE(A.STATISTICS, '$.incr_data_scn' RETURNING UNSIGNED) AS INCR_DATA_SCN,       JSON_VALUE(A.STATISTICS, '$.snap_data_scn' RETURNING UNSIGNED) AS SNAP_DATA_SCN,       (CASE JSON_VALUE(A.SYNC_INFO, '$.last_succ_time' RETURNING SIGNED) WHEN 0 THEN NULL ELSE usec_to_time(JSON_VALUE(A.SYNC_INFO, '$.last_succ_time' RETURNING SIGNED)) END) as LAST_SUCC_SYNC_TIME,       (CASE JSON_VALUE(A.SYNC_INFO, '$.last_fail_time' RETURNING SIGNED) WHEN 0 THEN NULL ELSE usec_to_time(JSON_VALUE(A.SYNC_INFO, '$.last_fail_time' RETURNING SIGNED)) END) as LAST_FAILED_SYNC_TIME,       JSON_VALUE(A.SYNC_INFO, '$.last_fail_code' RETURNING SIGNED) as LAST_FAILED_CODE   FROM       oceanbase.__all_virtual_vector_index_info A   LEFT JOIN OCEANBASE.__ALL_VIRTUAL_LS_META_TABLE M       ON A.TENANT_ID = M.TENANT_ID AND A.LS_ID = M.LS_ID AND A.SVR_IP = M.SVR_IP AND A.SVR_PORT = M.SVR_PORT   LEFT JOIN oceanbase.__all_virtual_table T       ON A.TENANT_ID = T.TENANT_ID       AND A.DATA_TABLE_ID = T.TABLE_ID   LEFT JOIN oceanbase.__all_virtual_table T1       ON A.TENANT_ID = T1.TENANT_ID       AND A.inc_index_table_id = T1.TABLE_ID   WHERE       A.INDEX_TYPE IN (0,1,5,6); )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_hnsw_index_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_V_OB_HNSW_INDEX_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_V_OB_HNSW_INDEX_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     SVR_IP,     SVR_PORT,     TENANT_ID,     DATA_TABLE_ID,     DATA_TABLET_ID,     TABLE_NAME,     INDEX_NAME,     ROLE,     INDEX_PARAM,     MEMORY_USED,     MEMORY_HOLD,     INCR_MEM_HOLD,     SNAP_MEM_HOLD,     INCR_INDEX_CNT,     SNAP_INDEX_CNT,     INCR_DATA_SCN,     SNAP_DATA_SCN,     LAST_SUCC_SYNC_TIME,     LAST_FAILED_SYNC_TIME,     LAST_FAILED_CODE FROM     OCEANBASE.GV$OB_HNSW_INDEX_INFO WHERE         SVR_IP=HOST_IP()     AND         SVR_PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_ivf_index_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_GV_OB_IVF_INDEX_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_GV_OB_IVF_INDEX_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     A.SVR_IP,     A.SVR_PORT,     A.TENANT_ID,     T.TABLE_ID AS DATA_TABLE_ID,     T.TABLE_NAME AS TABLE_NAME,     SUBSTR(T1.TABLE_NAME, 7 + INSTR(SUBSTR(T1.TABLE_NAME, 7), '_')) AS INDEX_NAME,     A.ROWKEY_VID_TABLE_ID AS INDEX_TABLE_ID,     A.ROWKEY_VID_TABLET_ID AS INDEX_TABLET_ID,     JSON_VALUE(A.STATISTICS, '$.param') AS INDEX_PARAM,     (CASE JT.CACHE_TYPE       WHEN 0 THEN 'IVF_CENTROID_CACHE'       WHEN 1 THEN 'IVF_PQ_CENTROID_CACHE'       WHEN 2 THEN 'IVF_PQ_PRECOMPUTE_TABLE_CACHE'       ELSE NULL END     ) AS CACHE_TYPE,     (case JSON_EXTRACT(A.STATISTICS, '$.is_inited')       WHEN 0 THEN 'FALSE'       WHEN 1 THEN 'TRUE'       ELSE NULL END     ) AS IS_INIT,     CAST(JT.MEMORY_USED AS SIGNED) AS MEMORY_USED,     CAST(JT.MEMORY_HOLD AS SIGNED) AS MEMORY_HOLD,     JSON_VALUE(JT.CACHE_INFO, '$') AS CACHE_INFO   FROM     oceanbase.__all_virtual_vector_index_info A   LEFT JOIN oceanbase.__all_virtual_table T1       ON A.TENANT_ID = T1.TENANT_ID       AND A.ROWKEY_VID_TABLE_ID = T1.TABLE_ID   LEFT JOIN oceanbase.__all_virtual_table T       ON A.TENANT_ID = T.TENANT_ID       AND SUBSTR(T1.TABLE_NAME, 7, INSTR(SUBSTR(T1.TABLE_NAME, 7), '_') - 1) = T.TABLE_ID,   JSON_TABLE(A.statistics, '$.cache_objs_info[*]' COLUMNS (       CACHE_TYPE INT PATH '$.cache_type',       MEMORY_USED INT UNSIGNED PATH '$.mem_used',       MEMORY_HOLD INT UNSIGNED PATH '$.mem_hold',       CACHE_INFO JSON PATH '$.cache_info'   )) JT   WHERE A.INDEX_TYPE IN (2,3,4); )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_ivf_index_info_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_INVALID_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_V_OB_IVF_INDEX_INFO_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(0);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_V_OB_IVF_INDEX_INFO_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     SVR_IP,     SVR_PORT,     TENANT_ID,     DATA_TABLE_ID,     TABLE_NAME,     INDEX_NAME,     INDEX_TABLE_ID,     INDEX_TABLET_ID,     INDEX_PARAM,     CACHE_TYPE,     IS_INIT,     MEMORY_USED,     MEMORY_HOLD,     CACHE_INFO FROM     OCEANBASE.GV$OB_IVF_INDEX_INFO WHERE         SVR_IP=HOST_IP()     AND         SVR_PORT=RPC_PORT() )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::all_ai_model_endpoint_idx_endpoint_name_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    ADD_COLUMN_SCHEMA_WITH_COLUMN_FLAGS("shadow_pk_2", //column_name
      column_id + 32770, //column_id
      4, //rowkey_id
      0, //index_id
      0, //part_key_pos
      ObVarcharType, //column_type
  ...
  table_schema.set_max_used_column_id(column_id + 32770);
  return ret;
}
...
int ObInnerTableSchema::all_tenant_location_idx_location_name_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_SYS_TABLEGROUP_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_ALL_TENANT_LOCATION_IDX_LOCATION_NAME_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(2);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_ALL_TENANT_LOCATION_IDX_LOCATION_NAME_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  table_schema.set_progressive_merge_round(1);
  table_schema.set_storage_format_version(3);
  table_schema.set_tablet_id(OB_ALL_TENANT_LOCATION_IDX_LOCATION_NAME_TID);
  table_schema.set_micro_index_clustered(false);
  ...
  table_schema.set_index_status(INDEX_STATUS_AVAILABLE);
  table_schema.set_index_type(INDEX_TYPE_NORMAL_LOCAL);
  table_schema.set_data_table_id(OB_ALL_TENANT_LOCATION_TID);

  table_schema.set_max_used_column_id(column_id + 3);
  return ret;
}

int ObInnerTableSchema::all_tenant_objauth_mysql_idx_objauth_mysql_user_id_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_SYS_TABLEGROUP_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_USER_ID_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(4);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_USER_ID_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  table_schema.set_progressive_merge_round(1);
  table_schema.set_storage_format_version(3);
  table_schema.set_tablet_id(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_USER_ID_TID);
  table_schema.set_micro_index_clustered(false);
  ...
  table_schema.set_index_status(INDEX_STATUS_AVAILABLE);
  table_schema.set_index_type(INDEX_TYPE_NORMAL_LOCAL);
  table_schema.set_data_table_id(OB_ALL_TENANT_OBJAUTH_MYSQL_TID);

  table_schema.set_max_used_column_id(column_id + 4);
  return ret;
}

int ObInnerTableSchema::all_tenant_objauth_mysql_idx_objauth_mysql_obj_name_schema(ObTableSchema &table_schema)
{
  int ret = OB_SUCCESS;
  uint64_t column_id = OB_APP_MIN_COLUMN_ID - 1;
  ...
  table_schema.set_tablegroup_id(OB_SYS_TABLEGROUP_ID);
  table_schema.set_database_id(OB_SYS_DATABASE_ID);
  table_schema.set_table_id(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_OBJ_NAME_TID);
  table_schema.set_rowkey_split_pos(0);
  table_schema.set_is_use_bloomfilter(false);
  table_schema.set_progressive_merge_num(0);
  table_schema.set_rowkey_column_num(4);
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_table_name(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_OBJ_NAME_TNAME))) {
      LOG_ERROR("fail to set table_name", K(ret));
    }
  }
  ...
  table_schema.set_progressive_merge_round(1);
  table_schema.set_storage_format_version(3);
  table_schema.set_tablet_id(OB_ALL_TENANT_OBJAUTH_MYSQL_IDX_OBJAUTH_MYSQL_OBJ_NAME_TID);
  table_schema.set_micro_index_clustered(false);
