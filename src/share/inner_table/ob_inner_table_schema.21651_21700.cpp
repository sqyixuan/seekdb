...
int ObInnerTableSchema::dba_ob_change_stream_refresh_stat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       CHANGE_STREAM_REFRESH_SCN,       CHANGE_STREAM_MIN_DEP_LSN,       CHANGE_STREAM_PENDING_TX_COUNT,       CHANGE_STREAM_FETCH_TX,       CHANGE_STREAM_FETCH_LSN,       CHANGE_STREAM_FETCH_SCN     FROM oceanbase.__all_virtual_change_stream_refresh_stat     WHERE TENANT_ID = EFFECTIVE_TENANT_ID()   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
