...
int ObInnerTableSchema::cdb_ob_dynamic_partition_tables_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     C.DATABASE_NAME AS DATABASE_NAME,     A.TABLE_NAME AS TABLE_NAME,     A.TABLE_ID AS TABLE_ID,     B.HIGH_BOUND_VAL AS MAX_HIGH_BOUND_VAL,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 1), '=', -1) AS ENABLE,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 2), '=', -1) AS TIME_UNIT,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 3), '=', -1) AS PRECREATE_TIME,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 4), '=', -1) AS EXPIRE_TIME,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 5), '=', -1) AS TIME_ZONE,     SUBSTRING_INDEX(SUBSTRING_INDEX(A.DYNAMIC_PARTITION_POLICY, ',', 6), '=', -1) AS BIGINT_PRECISION   FROM     oceanbase.__all_virtual_table A   JOIN     oceanbase.__all_virtual_database C   ON     A.DATABASE_ID = C.DATABASE_ID   JOIN   (     SELECT       TABLE_ID,       PART_IDX,       HIGH_BOUND_VAL,       ROW_NUMBER() OVER (         PARTITION BY TABLE_ID         ORDER BY PART_IDX DESC       ) AS rn     FROM       oceanbase.__all_virtual_part   ) B   ON     A.TABLE_ID = B.TABLE_ID   AND     B.rn = 1   WHERE     A.DYNAMIC_PARTITION_POLICY != ''     AND C.DATABASE_NAME != '__recyclebin'     AND C.IN_RECYCLEBIN = 0; )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_vector_memory_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     (VECTOR_MEM_HOLD + RAW_MALLOC_SIZE + INDEX_METADATA_SIZE) as VECTOR_MEM_HOLD,     (VECTOR_MEM_USED + RAW_MALLOC_SIZE + INDEX_METADATA_SIZE) as VECTOR_MEM_USED,     VECTOR_MEM_LIMIT FROM     oceanbase.__all_virtual_tenant_vector_mem_info )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_vector_memory_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT     VECTOR_MEM_HOLD,     VECTOR_MEM_USED,     VECTOR_MEM_LIMIT FROM     OCEANBASE.GV$OB_VECTOR_MEMORY )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_ai_model_endpoints_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       ENDPOINT_ID,       ENDPOINT_NAME,       AI_MODEL_NAME,       SCOPE,       URL,       ACCESS_KEY,       PROVIDER,       REQUEST_MODEL_NAME,       PARAMETERS,       REQUEST_TRANSFORM_FN,       RESPONSE_TRANSFORM_FN     FROM oceanbase.__all_virtual_ai_model_endpoint WHERE ENDPOINT_ID != -1;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_ai_models_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       MODEL_ID,       NAME,       case type         when 1 then 'DENSE_EMBEDDING'         when 2 then 'SPARSE_EMBEDDING'         when 3 then 'COMPLETION'         when 4 then 'RERANK'         else 'INVALID'       END AS TYPE,       MODEL_NAME     FROM oceanbase.__all_virtual_ai_model;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_change_stream_refresh_stat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT       REFRESH_SCN,       MIN_DEP_LSN,       PENDING_TX_COUNT,       FETCH_TX,       FETCH_LSN,       FETCH_SCN     FROM oceanbase.__all_virtual_change_stream_refresh_stat     WHERE TENANT_ID = EFFECTIVE_TENANT_ID()   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
