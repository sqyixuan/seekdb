...
int ObInnerTableSchema::v_ob_ls_log_restore_status_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT TENANT_ID,            SYNC_LSN,            SYNC_SCN,            SYNC_STATUS,            ERR_CODE,            COMMENT   FROM OCEANBASE.__ALL_VIRTUAL_LS_LOG_RESTORE_STATUS;   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::dba_ob_tenant_event_history_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     TENANT_ID,     gmt_create AS `TIMESTAMP`,     CAST(MODULE AS CHAR(256)) AS MODULE,     CAST(EVENT AS CHAR(256)) AS EVENT,     CAST(NAME1 AS CHAR(256)) AS NAME1,     CAST(VALUE1 AS CHAR(4096)) AS VALUE1,     CAST(NAME2 AS CHAR(256)) AS NAME2,     CAST(VALUE2 AS CHAR(4096)) AS VALUE2,     CAST(NAME3 AS CHAR(256)) AS NAME3,     CAST(VALUE3 AS CHAR(4096)) AS VALUE3,     CAST(NAME4 AS CHAR(256)) AS NAME4,     CAST(VALUE4 AS CHAR(4096)) AS VALUE4,     CAST(NAME5 AS CHAR(256)) AS NAME5,     CAST(VALUE5 AS CHAR(4096)) AS VALUE5,     CAST(NAME6 AS CHAR(256)) AS NAME6,     CAST(VALUE6 AS CHAR(4096)) AS VALUE6,     CAST(EXTRA_INFO AS CHAR(4096)) AS EXTRA_INFO,     CAST(SVR_IP AS CHAR(128)) AS SVR_IP,     SVR_PORT AS SVR_PORT,     CAST(TRACE_ID AS CHAR(128)) AS TRACE_ID,     COST_TIME AS COST_TIME,     RET_CODE AS RET_CODE,     CAST(ERROR_MSG AS CHAR(512)) AS ERROR_MSG   FROM OCEANBASE.__ALL_VIRTUAL_TENANT_EVENT_HISTORY   WHERE TENANT_ID = EFFECTIVE_TENANT_ID();   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
