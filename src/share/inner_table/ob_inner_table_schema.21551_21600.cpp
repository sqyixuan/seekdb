...
int ObInnerTableSchema::v_ob_tenant_resource_limit_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT  RESOURCE_NAME, CURRENT_UTILIZATION, MAX_UTILIZATION,     RESERVED_VALUE, LIMIT_VALUE, EFFECTIVE_LIMIT_TYPE FROM     oceanbase.GV$OB_TENANT_RESOURCE_LIMIT )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_tenant_resource_limit_detail_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT      resource_name AS RESOURCE_NAME,      limit_type AS LIMIT_TYPE,      limit_value AS LIMIT_VALUE FROM     oceanbase.__all_virtual_tenant_resource_limit_detail  )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_tenant_resource_limit_detail_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__( SELECT  RESOURCE_NAME, LIMIT_TYPE, LIMIT_VALUE FROM     oceanbase.GV$OB_TENANT_RESOURCE_LIMIT_DETAIL )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_nic_info_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     DEVNAME,     SPEED_MBPS   FROM oceanbase.GV$OB_NIC_INFO    )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_nic_info_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     DEVNAME,     SPEED_MBPS   FROM oceanbase.__all_virtual_nic_info   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_query_response_time_histogram_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     sql_type as SQL_TYPE,     cast ((response_time/1000000 ) as decimal(24,6)) as RESPONSE_TIME,     count as COUNT,     cast ((total/1000000)  as decimal(24,6))  as TOTAL   FROM oceanbase.__all_virtual_query_response_time )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_query_response_time_histogram_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     SQL_TYPE,     RESPONSE_TIME,     COUNT,     TOTAL   FROM     oceanbase.GV$OB_QUERY_RESPONSE_TIME_HISTOGRAM  )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_scheduler_job_run_details_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   (   SELECT                         CAST(NULL AS NUMBER) AS LOG_ID,                         CAST(NULL AS DATETIME) AS LOG_DATE,                         CAST(NULL AS CHAR(128)) AS OWNER,                         CAST(NULL AS CHAR(128)) AS JOB_NAME,                         CAST(NULL AS CHAR(128)) AS JOB_SUBNAME,                         CAST(NULL AS CHAR(128)) AS STATUS,                         CODE,                         CAST(NULL AS DATETIME) AS REQ_START_DATE,                         CAST(NULL AS DATETIME) AS ACTUAL_START_DATE,                         CAST(NULL AS NUMBER) AS RUN_DURATION,                         CAST(NULL AS CHAR(128)) AS INSTANCE_ID,                         CAST(NULL AS NUMBER) AS SESSION_ID,                         CAST(NULL AS CHAR(128)) AS SLAVE_PID,                         CAST(NULL AS NUMBER) AS CPU_USED,                         CAST(NULL AS CHAR(128)) AS CREDENTIAL_OWNER,                         CAST(NULL AS CHAR(128)) AS CREDENTIAL_NAME,                         CAST(NULL AS CHAR(128)) AS DESTINATION_OWNER,                         CAST(NULL AS CHAR(128)) AS DESTINATION,                         MESSAGE,                         JOB,                         TIME,                         JOB_CLASS,                         GMT_CREATE,                         GMT_MODIFIED                        FROM OCEANBASE.__ALL_VIRTUAL_TENANT_SCHEDULER_JOB_RUN_DETAIL ) UNION ALL ( SELECT                         LOG_ID,                         LOG_DATE,                         OWNER,                         JOB_NAME,                         JOB_SUBNAME,                         STATUS,                         CODE,                         REQ_START_DATE,                         ACTUAL_START_DATE,                         RUN_DURATION,                         INSTANCE_ID,                         SESSION_ID,                         SLAVE_PID,                         CPU_USED,                         CREDENTIAL_OWNER,                         CREDENTIAL_NAME,                         DESTINATION_OWNER,                         DESTINATION,                         MESSAGE,                         JOB,                         TIME,                         JOB_CLASS,                         GMT_CREATE,                         GMT_MODIFIED                        FROM OCEANBASE.__ALL_VIRTUAL_SCHEDULER_JOB_RUN_DETAIL_V2 ) )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::cdb_ob_table_space_usage_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     select       subquery.TABLE_ID AS TABLE_ID,       subquery.TENANT_NAME AS TENANT_NAME,       subquery.DATABASE_NAME AS DATABASE_NAME,       avt_name.TABLE_NAME AS TABLE_NAME,       subquery.OCCUPY_SIZE AS OCCUPY_SIZE,       subquery.REQUIRED_SIZE AS REQUIRED_SIZE     from     (select       CASE       	WHEN avt.table_type in (12, 13) THEN avt.data_table_id       	ELSE avt.table_id       END as TABLE_ID,       'SYS' AS TENANT_NAME,       ad.database_name as DATABASE_NAME,       sum(avtps.occupy_size) as OCCUPY_SIZE,       sum(avtps.required_size) as REQUIRED_SIZE     from     oceanbase.__all_virtual_tablet_pointer_status avtps     INNER JOIN oceanbase.__all_virtual_tablet_to_ls avttl       ON      avttl.tablet_id = avtps.tablet_id     INNER JOIN oceanbase.__all_virtual_table avt       ON      avt.table_id = avttl.table_id     INNER JOIN oceanbase.__all_virtual_database ad       ON      ad.database_id = avt.database_id     group by table_id     ) as subquery     INNER JOIN oceanbase.__all_virtual_table avt_name       ON    subquery.TABLE_ID = avt_name.table_id     order by table_id )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::v_ob_log_transport_dest_stat_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(     SELECT           CLIENT_IP,           CLIENT_PID,           CLIENT_TENANT_ID,           CLIENT_TYPE,           START_SERVE_TIME,           LAST_SERVE_TIME,           LAST_READ_SOURCE,           LAST_REQUEST_TYPE,           LAST_REQUEST_LOG_LSN,           LAST_REQUEST_LOG_SCN,           LAST_FAILED_REQUEST,           AVG_REQUEST_PROCESS_TIME,           AVG_REQUEST_QUEUE_TIME,           AVG_REQUEST_READ_LOG_TIME,           AVG_REQUEST_READ_LOG_SIZE,           AVG_LOG_TRANSPORT_BANDWIDTH     FROM OCEANBASE.GV$OB_LOG_TRANSPORT_DEST_STAT  )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
...
int ObInnerTableSchema::gv_ob_ss_local_cache_schema(ObTableSchema &table_schema)
{
  ...
  if (OB_SUCC(ret)) {
    if (OB_FAIL(table_schema.set_view_definition(R"__(   SELECT     CACHE_NAME,     PRIORITY,     HIT_RATIO,     TOTAL_HIT_CNT,     TOTAL_MISS_CNT,     HOLD_SIZE,     ALLOC_DISK_SIZE,     USED_DISK_SIZE,     USED_MEM_SIZE   FROM oceanbase.__all_virtual_ss_local_cache_info   )__"))) {
      LOG_ERROR("fail to set view_definition", K(ret));
    }
  }
  table_schema.set_index_using_type(USING_BTREE);
