...
def gen_sqlite_virtual_table_def(table_id, table_name, keywords):
  ...
  # Set partition columns (if svr_ip, svr_port exist)
  all_column_names = [col[0] for col in kw.get('normal_columns', [])]
  if 'svr_ip' in all_column_names and 'svr_port' in all_column_names:
    kw['partition_columns'] = ['svr_ip', 'svr_port']
    kw['vtable_route_policy'] = 'distributed'
  else:
    kw['partition_columns'] = []
    kw['vtable_route_policy'] = 'local'  # Tables without svr_ip/svr_port use local routing
