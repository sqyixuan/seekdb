...
int ObPluginVectorIndexAdaptor::check_delta_buffer_table_readnext_status(ObVectorQueryAdaptorResultContext *ctx,
                                                                         ...
                                                                         SCN query_scn)
{
  FLOG_INFO("check_delta_buffer_table_readnext_status");
  INIT_SUCC(ret);
  SCN min_delta_scn;
  bool can_skip = true;
                                                        ...
                                                        ObArray<uint64_t> &i_vids)
{
  ...
  FLOG_INFO("complete_index_mem_data", K(ret), K(i_vids));
                                                                   ...
                                                                   ObArray<uint64_t> &i_vids)
{
  ...
  LOG_INFO("complete_index_mem_data_incremental", K(ls_id));
  INIT_SUCC(ret);
  ObArray<uint64_t> d_vids;
  ...
  FLOG_INFO("complete_index_mem_data_incremental", K(ret), K(ls_id), K(last_row_scn), K(i_vids.count()), K(cost_us));
  return ret;
}
