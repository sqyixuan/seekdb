...
#endif
// macOS doesn't have libaio, provide stub functions (types are defined in ob_local_device.h)
static inline int io_setup(int maxevents, io_context_t *ctxp) {
  (void)maxevents;
  *ctxp = (io_context_t)malloc(sizeof(void*));
  return 0; // Success
}
...
static inline void io_prep_pwrite(struct iocb *iocb, int fd, void *buf, size_t count, int64_t offset) {
  (void)iocb; (void)fd; (void)buf; (void)count; (void)offset;
  // Stub: does nothing
}
...
int ObLocalDevice::resize_block_file(const int64_t new_size)
{
  ...
  if (delta_size < 0) {
    ...
    fstore_t store = {F_ALLOCATECONTIG, F_PEOFPOSMODE, block_file_size_, delta_size, 0};
