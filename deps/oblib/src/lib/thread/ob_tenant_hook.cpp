...
// macOS doesn't support epoll, provide stub implementation
int ob_epoll_wait(int __epfd, struct epoll_event *__events,
		              int __maxevents, int __timeout)
{
  (void)__epfd;
  ...
  (void)__maxevents;
  (void)__timeout;
  // Stub implementation: epoll is not available on macOS
  return -1;
}
#endif
...
#elif defined(__APPLE__)
// macOS doesn't support futex, provide stub implementation
int futex_hook(uint32_t *uaddr, int futex_op, uint32_t val, const struct timespec* timeout)
{
  (void)uaddr;
  (void)futex_op;
  (void)val;
  (void)timeout;
  // Stub implementation: futex is not available on macOS
  return 0;
}
#endif
