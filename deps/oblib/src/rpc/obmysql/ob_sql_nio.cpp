...
static inline int accept4(int sockfd, struct sockaddr *addr, socklen_t *addrlen, int flags) {
  ...
  if (flags & SOCK_NONBLOCK) { u_long m = 1; ioctlsocket(fd, FIONBIO, &m); }
  return (int)fd;
}
...
public:
  ...
  ReadBuffer(SocketReader& reader, ssl_state_st &ssl_st)
      : reader_(reader),
...
private:
  ...
  int do_read_fd(int64_t sz) {
    ...
    while(remain() < sz && OB_SUCCESS == ret && !reader_.has_EAGAIN()) {
      ...
      ret = reader_.read(data_end_, read_size, rbytes, ssl_st_);
      if (OB_SUCC(ret)) {
        data_end_ += rbytes;
      }
    }
...
{
public:
  PendingWriteTask(ssl_state_st &ssl_st): buf_(NULL), sz_(0), ssl_st_(ssl_st) {}
  ~PendingWriteTask() {}
  void reset() {
    buf_ = NULL;
    sz_ = 0;
...
public:
  ObSqlSock(ObSqlNioImpl *nio, int fd): dlink_(), all_list_link_(), write_task_link_(), nio_impl_(nio), fd_(fd),
            err_(0), reader_(fd), read_buffer_(reader_, ssl_st_), pending_write_task_(ssl_st_), need_epoll_trigger_write_(false),
            may_handling_(true), handler_close_flag_(false), need_shutdown_(false), last_decode_time_(0),
            sql_session_info_(NULL), tls_verion_option_(SSL_OP_NO_SSLv2|SSL_OP_NO_SSLv3), ssl_st_() {
  ...
  {
    int ret = OB_SUCCESS;
    ReadBuffer * buffer = OB_NEW(ReadBuffer, MEMORY_MODEL_NAME, reader_, ssl_st_);
    if (OB_ISNULL(buffer)) {
      ret = OB_ALLOCATE_MEMORY_FAILED;
    } else {
...
static int listen_create_unix(const char* unix_path, bool need_monopolize)
{
  int err = 0;
  int fd = 0;
  ...
  if ((fd = oceanbase::lib::ob_socket_cloexec_nonblock(AF_UNIX, SOCK_STREAM, 0)) < 0) {
    ...
    if (bind(fd, (sockaddr*)&addr, sizeof(addr)) < 0) {
      LOG_ERROR_RET(OB_ERR_SYS, "sql nio bind unix socket failed", K(errno), K(fd), K(unix_path));
      err = errno;
    } else if (listen(fd, 1024) < 0) {
...
private:
  void handle_epoll_event() {
    ...
    for(int i = 0; i < cnt; i++) {
      ...
      if (is_wrapped_fd(num64)) {
        ...
        if (lfd == lfd_) {
          do_accept_loop(lfd, false);
        } else if (lfd == unix_lfd_) {
          do_accept_loop(lfd, true /* is_unix_socket */);
        } else {
          // skip
        }
      } else {
  ...
  void do_accept_loop(int lfd, bool is_unix_socket = false) {
    while(1){
      int fd = -1;
  ...
  void do_accept_one(int fd, bool is_unix_socket) {
    int err = 0;
    ObSqlSock* s = NULL;
    int enable_tcp_nodelay = 1;
    uint32_t epflag = EPOLLIN | EPOLLOUT | EPOLLERR | EPOLLET | EPOLLRDHUP;
    if (is_unix_socket == 0
        && setsockopt(fd, IPPROTO_TCP, TCP_NODELAY, (const void *)&enable_tcp_nodelay, sizeof(enable_tcp_nodelay)) < 0) {
      err = errno;
    ...
    } else if (0 != (err = handler_.on_connect(s->sess_, fd, is_unix_socket))) {
