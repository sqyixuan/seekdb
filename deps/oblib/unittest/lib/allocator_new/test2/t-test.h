...
#endif /* ALLOCATOR_TEST > 0 */

template<class Allocator>
class BinAllocator
{
public:
	BinAllocator() : allocator_()
	{}
	template<class... Args>
	void init(const Args&... args) { allocator_.init(args...); }
	int64_t id() { return allocator_.id(); }
	void* malloc(unsigned long size) { return allocator_.alloc(size); }
	void free(void* ptr) { allocator_.free(ptr); }
	void* calloc(int num, unsigned long size) { return allocator_.alloc(num*size); }
	void* realloc(void* ptr, unsigned long size)
	{
		allocator_.free(ptr);
		return allocator_.alloc(size);
	}
	void* memalign(int alignment, unsigned long size)
	{
		unsigned long align_size = (size + alignment - 1) ^ (alignment - 1);
		return allocator_.alloc(align_size);
	}
/* Allocate a bin with malloc(), realloc() or memalign().  r must be a
   random number >= 1024. */
	void bin_alloc(struct bin *m, unsigned long size, int r)
	{
#if ALLOCATOR_TEST > 0
