    ...
    std::string dir_uri)
{
  ...
  if (info_base_.get_type() == OB_STORAGE_COS) {
    // test access to COS using the S3 protocol
    std::string tmp_cmd = cmd.replace(cmd.find(OB_COS_PREFIX), strlen(OB_COS_PREFIX), OB_S3_PREFIX);
    std::string uri_without_appid = remove_appid(uri);
    tmp_cmd = tmp_cmd.replace(cmd.find(uri), strlen(uri), uri_without_appid);
    tmp_cmd += "-e 'compliantRfc3986Encoding'";
    run_cmd(tmp_cmd.c_str());
  }
}
