  ...
  if (OB_UNLIKELY(!is_valid_type(type))) {
    ...
    switch (type) {
      case ObTestStorageType::TEST_STORAGE_S3:
        build_s3_cfg();
        break;
      case ObTestStorageType::TEST_STORAGE_OBS:
        build_obs_cfg();
        break;
      ...
      case ObTestStorageType::TEST_STORAGE_FS:
        build_fs_cfg();
        break;
      case ObTestStorageType::TEST_STORAGE_GCS:
        build_gcs_cfg();
        break;
      case ObTestStorageType::TEST_STORAGE_AZBLOB:
        build_azblob_cfg();
        break;
      default:
...
{
  if (!config_.is_valid()) {
    if (!need_skip_test(S3_BUCKET)
        && !need_skip_test(S3_REGION)
        && !need_skip_test(S3_ENDPOINT)
        && !need_skip_test(S3_AK)
        && !need_skip_test(S3_SK)) {
...
void ObTestStorageMeta::build_obs_cfg()
{
  if (!config_.is_valid()) {
    if (!need_skip_test(OBS_BUCKET)
        && !need_skip_test(OBS_ENDPOINT)
        && !need_skip_test(OBS_AK)
        && !need_skip_test(OBS_SK)) {
      config_.set_bucket(OBS_BUCKET);
      config_.set_endpoint(OBS_ENDPOINT);
      config_.set_ak(OBS_AK);
      config_.set_sk(OBS_SK);
      config_.is_valid_ = true;
    } else {
      config_.is_valid_ = false;
    }
...
{
  if (!config_.is_valid()) {
    if (!need_skip_test(OSS_BUCKET)
        && !need_skip_test(OSS_ENDPOINT)
        && !need_skip_test(OSS_AK)
        && !need_skip_test(OSS_SK)) {
...
{
  if (!config_.is_valid()) {
    if (!need_skip_test(COS_BUCKET)
        && !need_skip_test(COS_ENDPOINT)
        && !need_skip_test(COS_AK)
        && !need_skip_test(COS_SK)
        && !need_skip_test(COS_APPID)) {
...
void ObTestStorageMeta::build_gcs_cfg()
{
  if (!config_.is_valid()) {
    if (!need_skip_test(GCS_BUCKET)
        && !need_skip_test(GCS_ENDPOINT)
        && !need_skip_test(GCS_AK)
        && !need_skip_test(GCS_SK)) {
      config_.set_bucket(GCS_BUCKET);
      config_.set_endpoint(GCS_ENDPOINT);
      config_.set_ak(GCS_AK);
      config_.set_sk(GCS_SK);
      config_.is_valid_ = true;
    } else {
      config_.is_valid_ = false;
    }
...
void ObTestStorageMeta::build_azblob_cfg()
{
  if (!config_.is_valid()) {
    if (!need_skip_test(AZBLOB_BUCKET)
        && !need_skip_test(AZBLOB_ENDPOINT)
        && !need_skip_test(AZBLOB_AK)
        && !need_skip_test(AZBLOB_SK)) {
      config_.set_bucket(AZBLOB_BUCKET);
      config_.set_endpoint(AZBLOB_ENDPOINT);
      config_.set_ak(AZBLOB_AK);
      config_.set_sk(AZBLOB_SK);
      config_.is_valid_ = true;
    } else {
      config_.is_valid_ = false;
    }
...
public:
  ...
  const static int64_t part_size = 5 * 1024 * 1024;
...
public:
  ...
  static void SetUpTestCase()
  {
    ...
    ASSERT_EQ(OB_SUCCESS, init_obdal_env());
...
private:
  ...
  int tag_single_file(const common::ObString &uri);
...
{
  OB_LOG(INFO, "--------------BEGIN CHECK_BASIC_WRITER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char fragment_name[128] = { 0 };
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  ...
  if (FAILEDx(build_uri(uri_buf, OB_MAX_URI_LENGTH, "append_file1.bak/"))) {
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("append_file1.bak", OB_ADAPTIVELY_APPENDABLE_FORMAT_META,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret), K(OB_ADAPTIVELY_APPENDABLE_FORMAT_META));
  } else if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret), K(fragment_name));
  } else if (OB_FAIL(write_single_file(uri_buf, default_size))) {
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  }
  // write "/parent_path/append_file1.bak/@APD_PART@0-100"
  if (FAILEDx(construct_fragment_full_name("append_file1.bak", 0, 100,
                                           fragment_name, sizeof(fragment_name)))) {
  ...
  if (FAILEDx(construct_fragment_full_name("append_file1.bak", 100, 300,
                                           fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  ...
  if (FAILEDx(construct_fragment_full_name("append_file1.bak", 200, 500,
                                           fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  ...
  OB_LOG(INFO, "--------------END CHECK_BASIC_WRITER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_basic_appender()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_BASIC_APPENDER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  ...
  OB_LOG(INFO, "--------------END CHECK_BASIC_APPENDER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_basic_multipartwriter()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_BASIC_MULTIPARTWRITER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, default_multi_obj))) {
    ...
    if (OB_ISNULL(buf = static_cast<char *>(allocator.alloc(buf_len)))) {
      ...
      if (OB_FAIL(multiwriter.open(uri_buf, &cur_storage_info_))) {
        ...
        if (OB_FAIL(ret)) {
          // write or complete failed, clean parts
          if (OB_TMP_FAIL(multiwriter.abort())) {
            OB_LOG(WARN, "fail to abort multipartwriter", K(ret), K(tmp_ret), K(uri_buf));
          }
        }
  ...
  OB_LOG(INFO, "--------------END CHECK_BASIC_MULTIPARTWRITER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_basic_reader()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_BASIC_READER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char fragment_name[128] = { 0 };
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  ...
  if (FAILEDx(construct_fragment_full_name("append_file1.bak", OB_ADAPTIVELY_APPENDABLE_FORMAT_META,
                                           fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret), K(OB_ADAPTIVELY_APPENDABLE_FORMAT_META));
  } else if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  ...
  OB_LOG(INFO, "--------------END CHECK_BASIC_READER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_basic_util()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_BASIC_UTIL-----------------------------------------------");
  int ret = OB_SUCCESS;
  char fragment_name[128] = { 0 };
  /*
  ...
  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL dir-----------------------------------------------");
  ...
  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL write-----------------------------------------------");
  ...
  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL tag-----------------------------------------------");
  // write_single_file && tag_file && del_file
  if (FAILEDx(build_uri(uri_buf, OB_MAX_URI_LENGTH, "tmp_tag"))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(write_single_file(uri_buf, default_size))) {
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(tag_single_file(uri_buf))) {
    OB_LOG(WARN, "fail to tag single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(delete_single_file(uri_buf))) {
    OB_LOG(WARN, "fail to delete single file", K(ret), K(uri_buf));
  }

  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL list dir-----------------------------------------------");
  ...
  if (FAILEDx(build_uri(uri_buf, buf_len, "tmp_dir_1/"))) {
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, "tmp_dir_1/sub_dir1/file1"))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(write_single_file(uri_buf, default_size))) {
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, "tmp_dir_1/sub_dir2/file2"))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(write_single_file(uri_buf, default_size))) {
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  ...
  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL list files-----------------------------------------------");
  ...
  list_op.reset();
  if (FAILEDx(build_uri(uri_buf, buf_len, "basic_tmp_list/"))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(create_fs_dir(uri_buf, true/*drop_if_exist*/))) {
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  ...
  OB_LOG(INFO, "--------------CHECK_BASIC_UTIL obj meta-----------------------------------------------");
  ...
  if (FAILEDx(build_uri(uri_buf, buf_len, "tmp_dir_3/"))) {
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("tmp_dir_3/append_file2.bak",
                                                  OB_ADAPTIVELY_APPENDABLE_FORMAT_META,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret), K(OB_ADAPTIVELY_APPENDABLE_FORMAT_META));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(write_single_file(uri_buf, default_size))) {
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("tmp_dir_3/append_file2.bak", 0, 100,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
    ...
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("tmp_dir_3/append_file2.bak", 100, 400,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  ...
  OB_LOG(INFO, "--------------END CHECK_BASIC_UTIL-----------------------------------------------", K(ret));
  return ret;
}
...
int TestCommonStorage::abnormal_prepare()
{
  ...
  if (FAILEDx(build_uri(uri_buf, buf_len, "ab_tmp_append_file.bak"))) {
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("ab_tmp_append_file.bak", 0, 100,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
    ...
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("ab_tmp_append_file.bak", 100, 200,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  ...
  if (FAILEDx(build_uri(uri_buf, buf_len, "ab_tmp_append_file_nc.bak"))) {
    ...
    OB_LOG(WARN, "fail to create fs dir", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("ab_tmp_append_file_nc.bak",
                                                  OB_ADAPTIVELY_APPENDABLE_FORMAT_META,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret), K(OB_ADAPTIVELY_APPENDABLE_FORMAT_META));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    ...
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("ab_tmp_append_file_nc.bak", 0, 100,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
    ...
    OB_LOG(WARN, "fail to write single file", K(ret), K(uri_buf));
  } else if (OB_FAIL(construct_fragment_full_name("ab_tmp_append_file_nc.bak", 200, 300,
                                                  fragment_name, sizeof(fragment_name)))) {
    OB_LOG(WARN, "fail to construct fragment name", K(ret));
  } else if (OB_FAIL(build_uri(uri_buf, buf_len, fragment_name))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
...
{
  OB_LOG(INFO, "--------------BEGIN CHECK_ABNORMAL_WRITER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  ...
  OB_LOG(INFO, "--------------END CHECK_ABNORMAL_WRITER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_abnormal_appender()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_ABNORMAL_APPENDER-----------------------------------------------");
  int ret = OB_SUCCESS;
  char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
  // 1. when first append, the offset not start with 0
  // [Expect]: for nfs and s3/obs, succ; for oss and cos, fail
  if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, "ab_tmp_append_file_1.bak"))) {
    ...
    if (cur_type_ == ObTestStorageType::TEST_STORAGE_S3 || 
        ...
        cur_type_ == ObTestStorageType::TEST_STORAGE_OBS ||
        cur_type_ == ObTestStorageType::TEST_STORAGE_GCS ||
        (cur_type_ == ObTestStorageType::TEST_STORAGE_COS && is_use_obdal())) {
      // nfs can succ, but the file length is 200, cuz nfs open file with O_APPEND
      // s3/obs can succ, but there won't exist FORMAT_META
      if (tmp_ret != OB_SUCCESS) {
        ret = OB_ERR_UNEXPECTED;
        OB_LOG(WARN, "s3 and fs can append succ if offset not start with 0", K(ret), K_(cur_type));
      }
    } else {
      // oss and cos(don't use obdal) must append with start_offset=0
      if (tmp_ret == OB_SUCCESS) {
        ret = OB_ERR_UNEXPECTED;
  ...
  OB_LOG(INFO, "--------------END CHECK_ABNORMAL_APPENDER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_abnormal_multipartwriter()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_ABNORMAL_READER-----------------------------------------------");
  int ret = OB_SUCCESS;
  int tmp_ret = OB_SUCCESS;
  // use crc32 algorithm for dest OBS, call complete interface
  // [Expect]: OB_CHECKSUM_TYPE_NOT_SUPPORTED
  if (ObTestStorageType::TEST_STORAGE_OBS == cur_meta_->type_
      // && !is_use_obdal()) {
      ) {
    ObObjectStorageInfo info;
    ObStorageMultiPartWriter multiwriter;
    const char *buf = "abc";
    std::string checksum_algo = CHECKSUM_TYPE;
    checksum_algo += CHECKSUM_TYPE_CRC32;
    char uri_buf[OB_MAX_URI_LENGTH] = { 0 };
    
    if (OB_FAIL(TestCommonStorageUtil::build_object_storage_info(cur_meta_->config_.bucket_, 
        cur_meta_->config_.endpoint_, cur_meta_->config_.ak_, cur_meta_->config_.sk_,
        cur_meta_->config_.region_, cur_meta_->config_.appid_,
        checksum_algo.c_str(), info))) {
      OB_LOG(WARN, "fail to build object storage info", K(ret), KPC_(cur_meta));
    } else if (OB_FAIL(build_uri(uri_buf, OB_MAX_URI_LENGTH, "ab_multipart"))) {
      OB_LOG(WARN, "fail to build uri", K(ret));
    } else if (OB_FAIL(multiwriter.open(uri_buf, &info))) {
      OB_LOG(WARN, "fail to open multipartwriter", K(ret), K(uri_buf), K(info));
    } else {
      if (OB_FAIL(multiwriter.pwrite(buf, strlen(buf), 0))) {
        OB_LOG(WARN, "fail to pwrite", K(ret), K(buf), K(uri_buf));
      } else if (OB_FAIL(multiwriter.complete())) {
        ...
        ret = OB_ERR_UNEXPECTED;
        OB_LOG(WARN, "complete should not succeed", K(ret), K(uri_buf));
      }
  ...
  OB_LOG(INFO, "--------------END CHECK_ABNORMAL_READER-----------------------------------------------");
  return ret;
}
...
int TestCommonStorage::check_abnormal_reader()
{
  ...
  OB_LOG(INFO, "--------------END CHECK_ABNORMAL_READER-----------------------------------------------", K(ret));
  return ret;
}

int TestCommonStorage::check_abnormal_util()
{
  OB_LOG(INFO, "--------------BEGIN CHECK_ABNORMAL_UTIL-----------------------------------------------");
  int ret = OB_SUCCESS;
  ...
  // 4. delete not exist file
  // [Expect]: for nfs, return OB_FILE_NOT_EXIST; else return OB_SUCCESS
  if (FAILEDx(build_uri(uri_buf, buf_len, "ab_tmp_not_exist_file"))) {
    OB_LOG(WARN, "fail to build uri", K(ret));
  } else if (OB_FAIL(is_exist(uri_buf, false/*is_adaptive*/, exist))) {
    OB_LOG(WARN, "fail to check is_exist", K(ret), K(uri_buf));
  } else if (OB_UNLIKELY(exist)) {
    ret = OB_ERR_UNEXPECTED;
    OB_LOG(WARN, "this file should not be existed", K(ret), K(uri_buf));
  } else if (OB_FAIL(delete_single_file(uri_buf))) {
    if (cur_meta_->is_file_type() && OB_OBJECT_NOT_EXIST == ret) {
      ret = OB_SUCCESS;
    } else {
      OB_LOG(WARN, "fail to delete single file", K(ret), K(uri_buf));
    }
  }
  OB_LOG(INFO, "--------------END CHECK_ABNORMAL_UTIL-----------------------------------------------", K(ret));
  return ret;
}
...
int TestCommonStorage::tag_single_file(const common::ObString &uri)
{
  int ret = OB_SUCCESS;
  ObStorageUtil util;
  ObObjectStorageInfo info;
  // for nfs, tag operation always succeed
  // gcs doesn't support tag, so skip this case
  if (cur_meta_->type_ == ObTestStorageType::TEST_STORAGE_GCS) {
    // do nothing
  } else if (cur_meta_->is_obj_type()) {
    if (OB_FAIL(TestCommonStorageUtil::build_object_storage_info(cur_meta_->config_.bucket_, 
        cur_meta_->config_.endpoint_, cur_meta_->config_.ak_, cur_meta_->config_.sk_,
      ...
      OB_LOG(WARN, "fail to build object storage info", K(ret), KPC_(cur_meta));
    } else if (OB_FAIL(util.open(&info))) {
      OB_LOG(WARN, "fail to open storage_util", K(ret));
    } else if (OB_FAIL(util.del_file(uri))) {
      OB_LOG(WARN, "fail to delete file", K(ret), K(uri));
    }
    ...
    if (FAILEDx(util.is_exist(uri, false/*is_adaptive*/, is_file_exist))) {
      ...
      ret = OB_ERR_UNEXPECTED;
      OB_LOG(WARN, "fail to delete file cuz file still exist", K(ret), K(uri));
    } else if (OB_FAIL(util.is_tagging(uri, is_tagging))) {
      OB_LOG(WARN, "fail to check is_tagging", K(ret), K(uri));
    } else if (!is_tagging) {
      ret = OB_ERR_UNEXPECTED;
      OB_LOG(WARN, "fail to tag file cuz file has no tag", K(ret), K(uri));
    }
...
TEST_F(TestCommonStorage, obdal_base_situation_test)
{
...
TEST_F(TestCommonStorage, obdal_abnormal_situation_test)
{
  cluster_enable_obdal_config = &ObClusterEnableObdalConfigBase::get_instance();
  TestCommonStorage storage;
  ASSERT_EQ(OB_SUCCESS, storage.init());
