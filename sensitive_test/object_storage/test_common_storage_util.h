...
#ifndef OCEANBASE_UNITTEST_COMMON_STORAGE_UTIL_H_
#define OCEANBASE_UNITTEST_COMMON_STORAGE_UTIL_H_
    ...
    ObObjectStorageInfo &storage_info)
{
  ...
  if (OB_ISNULL(bucket) || OB_ISNULL(endpoint) ||
    ...
    OB_LOG(WARN, "invalid argument", K(ret), KP(bucket), KP(region), KP(endpoint), KP(ak));
  } else {
    ...
    if (OB_FAIL(get_storage_type_from_path(bucket, storage_type))) {
      ...
      if (OB_FAIL(databuff_printf(account, sizeof(account), pos,
        ...
        OB_LOG(WARN, "fail to databuff printf", K(ret), K(endpoint), K(ak));
      } else if (OB_NOT_NULL(extension) && OB_FAIL(databuff_printf(account, sizeof(account), pos,
                                                                   "&%s", extension))) {
        OB_LOG(WARN, "fail to databuff printf", K(ret), K(pos), K(extension));
