...
TEST_P(TestObjectStorage, test_invalid_bucket_name)
{
  if (enable_test_ && info_base_.get_type() != ObStorageType::OB_STORAGE_FILE) {
    ...
    if (info_base_.get_type() == ObStorageType::OB_STORAGE_OSS) {
      wrong_bucket = "oss://1";
    } else if (info_base_.get_type() == ObStorageType::OB_STORAGE_S3) {
      wrong_bucket = "s3://1";
    } else if (info_base_.get_type() == ObStorageType::OB_STORAGE_COS) {
      // COS bucket name does not limit the length, so use another invalid bucket name
      wrong_bucket = "cos://1_";
...
TEST_P(TestObjectStorage, test_retry)
{
  ...
  if (enable_test_
    ...
    if (info_base_.get_type() == ObStorageType::OB_STORAGE_OSS) {
      {
        ObStorageOSSRetryStrategy oss_retry_strategy(-1);
        ASSERT_EQ(OB_STORAGE_MAX_IO_TIMEOUT_US, oss_retry_strategy.timeout_us_);
      }
    }
...
TEST_P(TestObjectStorage, test_oss_wrong_endpoint)
{
  ...
  if (enable_test_
      && is_use_obdal() == false
      && info_base_.get_type() == ObStorageType::OB_STORAGE_OSS
      && GetParam().extension_ == nullptr) {
