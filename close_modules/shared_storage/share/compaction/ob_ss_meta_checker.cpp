...
int ObTenantSSMetaChecker::check_inner_table_replicas(ObTabletReplicaMap &replica_map)
{
  ...
  } else if (OB_FAIL(locality_cache.init(MTL_ID()))) {
    LOG_WARN("failed to init locality cache for ss report", K(ret));
  } else if (OB_FAIL(locality_cache.refresh_ls_locality(true/*force_refresh*/))) {
    LOG_WARN("failed to refresh ls locality cache", K(ret));
  }
  ...
  FOREACH_X(it, replica_map, OB_SUCC(ret)) {
    ...
    if (OB_TMP_FAIL(check_local_tablet_exist(ls_id, tablet_id, locality_cache, not_exist))) {
    ...
    bool &not_exist)
{
  ...
  if (OB_UNLIKELY(!ls_id.is_valid() || !tablet_id.is_valid())) {
    ...
    if (OB_LS_NOT_EXIST != ret) {
      ...
      // ls exist on other server, do nothing
    }
