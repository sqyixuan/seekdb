    ...
    TenantIDS &gc_tenant_ids) const
{
  ...
  ARRAY_FOREACH_X(tenant_ids, idx, cnt, true) {
    ...
    // may return OB_TENANT_NOT_EXIST if the tenant has been force-dropped
    if (OB_FAIL(ls_status_op.get_all_ls_status_by_order(tenant_id, 
        ls_array, *sql_proxy_)) && OB_TENANT_NOT_EXIST != ret) {
      CLOG_LOG(WARN, "get_all_ls_status_by_order failed", KR(ret), K(tenant_id));
    } else if ((OB_TENANT_NOT_EXIST == ret || 0 == ls_array.count()) &&
        OB_FAIL(gc_tenant_ids.push_back(tenant_id))) {
      CLOG_LOG(WARN, "push_back failed", KR(ret), K(gc_tenant_ids), K(tenant_id));
    }
  }
  return ret;
}
...
int ObSharedLogGarbageCollector::handle_tenant_block_gc_(const uint64_t tenant_id)
{
  ...
  } else if (OB_FAIL(ls_op.get_all_ls_by_order(ls_attr_array, only_existing_ls))) {
    CLOG_LOG(WARN, "get_all_ls_by_order failed", KR(ret), K(tenant_id));
  } else {
    ARRAY_FOREACH_X(ls_attr_array, idx, cnt, true) {
      int tmp_ret = OB_SUCCESS;
      const share::ObLSAttr &ls_attr = ls_attr_array.at(idx);
      if (OB_TMP_FAIL(handle_ls_block_gc_(tenant_id, ls_attr, retention_scn))) {
        CLOG_LOG(WARN, "handle_ls_block_gc_ failed", KR(tmp_ret), K(tenant_id),
            K(ls_attr), K(retention_scn));
      }
    }
  }
  return ret;
    ...
    const share::SCN &retention_scn)
{
  ...
  } else if (OB_FAIL(get_min_using_scn_(tenant_id, ls_attr, min_using_scn, min_using_upper_lsn))) {
    CLOG_LOG(WARN, "get_min_using_scn_ failed", KR(ret), K(tenant_id), K(ls_attr));
    ...
    palf::LSN &min_using_upper_lsn) const
{
  ...
  if (true == ls_attr.ls_is_dropped_create_abort()) {
    min_using_scn = share::SCN::max_scn();
    min_using_upper_lsn = palf::LSN(LOG_MAX_LSN_VAL);
  } else if (OB_FAIL(get_min_ckpt_from_all_replicas_(tenant_id, ls_id, ckpt))) {
    CLOG_LOG(WARN, "get_min_ckpt_from_all_replicas_ failed", K(ret),
        K(tenant_id), K(ls_attr), "ckpt_scn", ckpt.first, "ckpt_lsn", ckpt.second);
  } else {
  ...
  CLOG_LOG(INFO, "get_min_using_scn_ finish", K(ret), K(tenant_id),
      K(ls_attr), K(min_using_scn), K(min_using_upper_lsn),
    ...
    CKPTPair &ckpt) const
{
  ...
  if (OB_FAIL(location_cb_->nonblock_get_leader(tenant_id, ls_id.id(), leader))) {
    ...
    (void) location_cb_->nonblock_renew_leader(tenant_id, ls_id.id());
