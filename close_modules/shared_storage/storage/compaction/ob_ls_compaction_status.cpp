    ...
    ObCompactionObjBuffer &obj_buf)
{
  ...
  ObMemberListInfo member_list(MTL_ID());

  if (OB_FAIL(MTL_LS_LOCALITY_CACHE.get_ls_info(ls_id, ls_info))) {
    LOG_WARN("failed to get ls info", KR(ret), K(ls_id));
  } else if (OB_FAIL(ls_info.find_leader(replica))) {
    if (OB_ENTRY_NOT_EXIST == ret) {
      LOG_INFO("leader not exist", KR(ret), K(ls_id));
    } else {
      LOG_WARN("failed to find leader", KR(ret), K(ls_info));
    }
  } else if (OB_FAIL(member_list.build(*replica))) {
    LOG_WARN("failed to build member list", KR(ret), KPC(replica));
  } else if (OB_FAIL(MTL_SVR_ID_CACHE.get_svr_infos(svr_infos))) {
    LOG_WARN("failed to get svr infos", KR(ret));
  } else if (svr_infos.count() > 0 && member_list.member_list_array_.count() > 0) {
    const ObIArray<common::ObAddr> &member_addrs = member_list.member_list_array_;
    ObSvrLSCompactionStatusObj svr_array[member_addrs.count()];
    ...
    uint64_t svr_id = 0;
    for (int64_t idx = 0; OB_SUCC(ret) && idx < member_addrs.count(); ++idx) {
      if (OB_FAIL(ObSvrIDCache::get_svr_id_by_addr(svr_infos, member_addrs.at(idx), svr_id))) {
        if (OB_ENTRY_NOT_EXIST == ret) {
          LOG_TRACE("svr_id not cached member addr, need wait", KR(ret), K(ls_id));
        } else {
          LOG_WARN("failed to get svr id by addr", KR(ret), K(svr_infos), K(member_addrs.at(idx)));
        }
...
#endif
        if (OB_FAIL(svr_array[idx].init(svr_id, ls_id, OBJ_EXEC_MODE_READ_ONLY))) {
          LOG_WARN("failed to init svr finish tablets", KR(ret), K(idx), K(svr_id), K(ls_id));
        } else if (OB_FAIL(svr_array[idx].refresh(info, obj_buf))) {
          ...
          for (int64_t repl_idx = 0; OB_SUCC(ret) && repl_idx < ls_info.get_replicas_cnt(); ++repl_idx) {
            const ObLSReplica &replica = ls_info.get_replicas().at(repl_idx);
            if (replica.get_server() == member_addrs.at(idx)) {
              if (!replica.is_in_service()) {
                LOG_TRACE("ls replica is not in service, skip it", KR(ret), K(ls_id), K(svr_id), K(replica));
              } else if (OB_FAIL(svr_obj_array.push_back(&svr_array[idx]))) {
    ...
    LOG_INFO("empty member list or empty svr id cache", KR(ret), K(ls_info), K(svr_infos));
  }
