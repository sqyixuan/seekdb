    ...
    const int64_t broadcast_version)
{
  ...
  } else if (OB_FAIL(MTL_LS_LOCALITY_CACHE.refresh_ls_locality(true/*force_refresh*/))) {
...
int ObErrorTabletValidateScheduler::schedule_error_tablet_verify_ckm(const int64_t compaction_scn)
{
  ...
  if (OB_FAIL(ObTabletCkmErrorInfoOperator::get_ckm_error_tablets(MTL_ID(), compaction_scn, error_tablets))) {
    LOG_WARN("failed to get ckm error tablets", KR(ret), K(compaction_scn));
  } else if (error_tablets.count() > 0) { // return error_tablets is order by ls_id
    ObLSHandle ls_handle;
    ObLSID skip_ls_id;
    ObTabletHandle tablet_handle;
