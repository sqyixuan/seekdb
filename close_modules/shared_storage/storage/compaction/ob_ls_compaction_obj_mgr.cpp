    ...
    ObLSBroadcastInfo &info)
{
  ...
  } else if (OB_FAIL(MTL_LS_LOCALITY_CACHE.get_ls_info(ls_compaction_status_.get_ls_id(), ls_info))) {
    LOG_WARN("failed to get ls info", K(ret), "ls_id", ls_compaction_status_.get_ls_id());
  ...
  } else if (OB_FAIL(check_locality_info(svr_info.svr_addr_, ls_info, has_exec_svr))) {
    LOG_WARN("failed to check ls locality", K(ret), K(info), K(svr_info), K(ls_info));
  }

  ObSEArray<uint64_t, 8> candidates;
  if (OB_FAIL(ret) || has_exec_svr) {
  } else if (OB_FAIL(get_candidate_svrs(ls_info, candidates))) {
...
int ObLSObj::check_locality_info(
    ...
    bool &has_exec_svr)
{
  ...
  if (OB_UNLIKELY(!svr_addr.is_valid() || !ls_info.is_valid() || ls_info.get_replicas().empty())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("get invalid arguments", K(ret), K(svr_addr), K(ls_info));
  } else {
    const ObLSInfo::ReplicaArray &replicas = ls_info.get_replicas();
    has_exec_svr = false;
    for (int64_t i = 0; !has_exec_svr && i < replicas.count(); ++i) {
      if (svr_addr == replicas.at(i).get_server() && replicas.at(i).is_in_service()) {
        has_exec_svr = true;
      }
    }
  }
    ...
    ObIArray<uint64_t> &candidates)
{
  ...
  if (OB_UNLIKELY(!input_ls_info.is_valid())) {
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("get invalid argument", K(ret), K(input_ls_info));
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < svr_infos.count(); ++i) {
    ...
    bool is_exist = false;
    if (OB_FAIL(check_locality_info(cur_info.svr_addr_, input_ls_info, is_exist))) {
      LOG_WARN("failed to check locality info", K(ret), K(cur_info), K(input_ls_info));
    } else if (is_exist && OB_FAIL(candidates.push_back(cur_info.svr_id_))) {
      LOG_WARN("failed to add svr id to candidates", K(ret), K(cur_info), K(svr_infos), K(input_ls_info));
    }
  }
  return ret;
}
