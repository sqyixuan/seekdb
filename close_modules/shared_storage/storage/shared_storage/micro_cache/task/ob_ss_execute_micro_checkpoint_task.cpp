...
/*-----------------------------------------ObSSExecuteMicroCheckpointTask------------------------------------------*/
ObSSExecuteMicroCheckpointTask::ObSSExecuteMicroCheckpointTask(ObSSMicroCacheTaskRunner &runner)
  : ObSSMicroCacheTaskBase(), runner_(runner), ckpt_op_()
{}

void ObSSExecuteMicroCheckpointTask::destroy()
{
  ckpt_op_.destroy();
  ObSSMicroCacheTaskBase::destroy();
}

int ObSSExecuteMicroCheckpointTask::init(
...
void ObSSExecuteMicroCheckpointTask::runTimerTask()
{
  int ret = OB_SUCCESS;
  ...
  } else if (OB_FAIL(ckpt_op_.execute_checkpoint())) {
    LOG_WARN("fail to execute checkpoint", KR(ret));
  }

  if (OB_TMP_FAIL(runner_.schedule_do_micro_checkpoint_task(interval_us_))) {
    LOG_WARN("fail to reschedule do_micro_checkpoint task", KR(ret), K(tmp_ret), K_(interval_us));
  }
}
