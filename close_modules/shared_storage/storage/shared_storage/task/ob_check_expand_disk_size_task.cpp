...
int ObCheckExpandDiskTask::check_expand_disk_size()
{
  ...
  } else if (OB_FAIL(OB_SERVER_DISK_SPACE_MGR.check_expand_tenant_data_disk_size(tenant_id_, need_expand))) {
    LOG_WARN("fail to check expand data disk size", KR(ret));
  } else if (!need_expand) {
    // do nothing, cannot expand disk size
  } else if (OB_FAIL(OB_SERVER_DISK_SPACE_MGR.auto_expand_data_disk_size(tenant_id_,
             ObDiskSpaceManager::DEFAULT_TENANT_DISK_SIZE_EXPAND_STEP_LENGTH/*expand_size*/))) { // default step length is 1GB
    LOG_WARN("fail to auto expand data disk size", KR(ret));
  } else {
    LOG_INFO("succ to auto expand tenant data disk size", K_(tenant_id));
  }
  return ret;
}
