...
//
// Only supported in backup scenarios, no need to handle non-atomic archive write situations
...
// Temporarily only supports single fetch Need continuous fetch of large data to be controlled by upper layer
...
// To accelerate the location of the starting point file, rely on the LSN -> file_id rule
...
int LocationDataGenerator::fetch_log_from_location_(char *&buf, int64_t &buf_size)
{
  ...
  if (OB_FAIL(get_precise_file_and_offset_(dest_id, round_id, piece_id,
    ...
    // Not the first time reading the file, no need to parse the file header again
...
// Current start LSN is less than the maximum consumed LSN in piece_context, need to relocate piece
// Because the file_id is calculated based on LSN, this file may be located in the current piece, or in the previous one or multiple pieces, requiring a re-locate
