...
// When creating a network tenant, you need to wait for the tenant to become NORMAL status before returning success to the user.
// Tenant becomes Normal state requires system log stream to be readable, but during this waiting process, log stream synchronization
//Possible errors may occur, these errors will cause the log pull to stop, so when encountering these errors
// Directly return client failure, no need to wait for timeout
// These states are considered as can continue to wait for the tenant to become NORMAL
