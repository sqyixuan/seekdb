...
#include "close_modules/observer_lite/logservice/archiveservice/ob_archive_define.h"
#include "close_modules/observer_lite/logservice/archiveservice/ob_archive_file_utils.h"      // ObArchiveFileUtils
#include "close_modules/observer_lite/logservice/archiveservice/ob_archive_util.h"
...
// Here it is assumed that the round in stop state always includes the round end file
// Otherwise, only the rounds where round end file persistence is successful can be confirmed as stop state, and it is irrelevant to the archive cluster status
    ...
    bool &to_newest)
{
  ...
  while (OB_SUCC(ret) && ! done) {
    ...
    // Due to the complexity of the scene, to avoid missing scenes and thus unable to break out of the loop, each retry sleeps for 1ms
    if (! done) {
      ob_usleep(1000L);
    }
...
void ObLogArchivePieceContext::check_if_switch_round_(const share::SCN &scn, const palf::LSN &lsn, RoundOp &op)
{
  ...
  if (min_round_id_ == 0 || max_round_id_ == 0
      ...
      || is_max_round_done_(lsn) /* The maximum value of the current maximum round that has already STOP */) {
    op = RoundOp::LOAD_RANGE;
  } else if (! locate_round_) {
    op = RoundOp::LOCATE;
  } else if (need_backward_round_(lsn)/*Determine that all logs in the current round are greater than the logs to be consumed, and the current round is greater than the minimum round id*/) {
    op = RoundOp::BACKWARD;
  } else if (need_forward_round_(lsn)/*Determine that all logs in the current round are less than the logs to be consumed, and the current round is less than the maximum round id*/) {
    op = RoundOp::FORWARD;
  } else if (need_load_round_info_(scn, lsn)/*The maximum piece that can be accessed in the current round has STOPped, and the current round is still ACTIVE*/) {
    op = RoundOp::LOAD;
  }
...
// The maximum value of the current maximum round that has already STOPPED
...
// Can forward slice round, it must be piece already sliced to the minimum, and the minimum piece is FROZEN state
...
// Note: For an empty round, it is always discontinuous with the next round; therefore, when encountering an empty round, forward queries are supported by default, backward queries are not supported
// bad case round 1, 2, 3, current is 3, need to switch to round 1, middle round 2 is empty, relying only on round 2 information cannot determine the need to switch to round 1
//  encounter forward cut round, by caller to handle
bool ObLogArchivePieceContext::need_forward_round_(const palf::LSN &lsn) const
{
  ...
  if (max_round_id_ <= 0) {
    ...
    CLOG_LOG(INFO, "empty round, only forward round supported", K(round_context_));
      ...
      && inner_piece_context_.piece_id_ == round_context_.max_piece_id_
      && inner_piece_context_.is_valid()) {
    // The current piece is the latest piece under round, and round has already STOP
    if (inner_piece_context_.is_low_bound_()) {
      // Current piece status is LOW_BOUND, log stream has not yet been generated within the current piece
      bret = true;
    }
    else if ((inner_piece_context_.is_frozen_() || inner_piece_context_.is_empty_())
        && inner_piece_context_.max_lsn_in_piece_ <= lsn) {
      // At the same time, the current piece status is FROZEN/EMPTY
      // and the log LSN needs to be greater than the maximum value in the current piece
      bret = true;
    }
  }
...
// Precondition: The current round_context status is not STOP
// 1. The current round_context information is invalid
// 2. The maximum piece in the current round does not contain the logs that need to be consumed
...
// Since round id may not be consecutive, this function finds a round that can be consumed
int ObLogArchivePieceContext::forward_round_(const RoundContext &pre_round)
{
  ...
  if (OB_UNLIKELY(min_round_id_ <= 0
    ...
    // Since max_round_id must exist, the expected success can definitely be done
    if (OB_SUCC(ret) && done) {
      round_context_.reset();
      round_context_.round_id_ = round_id;
...
// Forward lookup is more complex, does not support direct forward query, handled by the caller
    ...
    PieceOp &op)
{
  ...
  // Current piece status is determined
  ...
  // Current piece status is LOW BOUND
  else if (inner_piece_context_.is_low_bound_()) {
    op = PieceOp::FORWARD;
  }
  // Current piece log stream GC
  ...
  // The current piece is still ACTIVE
  else {
...
// By scn and inner_piece_context with round_context jointly determine piece_id
// 1. inner_piece_context.round_id == round_context.round_id indicates that the current round is still being consumed. If the current piece_id is valid, and the piece is still active, then continue to refresh this piece
// 2. inner_piece_context.round_id == round_context.round_id and piece status is FROZEN or EMPTY, unexpected error
// 3. inner_piece_context.round_id != round_context.round_id, needs to be determined by scn and round piece range, to decide piece id
...
int ObLogArchivePieceContext::cal_load_piece_id_(const SCN &scn, int64_t &piece_id)
{
  ...
  if (inner_piece_context_.round_id_ == round_context_.round_id_) {
    // Probably been recycled, error handling, can be optimized later
...
int ObLogArchivePieceContext::get_piece_meta_info_(const int64_t piece_id)
{
  ...
  if (piece_id < round_context_.min_piece_id_) {
    ...
    // Different log stream archiving progress may vary, there might be multiple pieces not FROZEN, consuming piece needs to confirm if FROZEN
  ...
  if (OB_SUCC(ret)) {
    ...
    // If piece meta exists, then the piece is already FROZEN, the log stream may be in FROZEN, EMPTY, and LOW_BOUND state for this piece
    if (piece_meta_exist) {
      // ls exists within this piece, then the status must be FROZEN or EMPTY
      if (is_ls_in_piece) {
      ...
      // ls does not exist within this piece, and the piece is in FROZEN state, then ls must be within a larger piece
      // NOTE: restore does not understand the content of the archive file, it is very difficult to distinguish between GC or LOW_BOUND
      // Dependency submitted by zeyong, indicates that the log stream has been GC'd, fixed in physical recovery scenarios, can assume it is LOW_BOUND
      else {
    ...
    bool &to_newest)
{
  ...
  // Reached the maximum consumption but still failed to locate this LSN, and the current piece contains a log range less than this LSN, return OB_ITER_END
  if (OB_SUCC(ret) && ! done) {
    if (inner_piece_context_.is_valid()
        && inner_piece_context_.round_id_ == max_round_id_
        && inner_piece_context_.piece_id_ == round_context_.max_piece_id_
  ...
  // The log stream in this piece has been GC'd, and the maximum LSN is less than or equal to the LSN that needs to be retrieved, return OB_ITER_END
  if (OB_SUCC(ret) && ! done) {
    if (inner_piece_context_.is_valid()
  ...
  // Minimum round minimum piece containing the minimum lsn is still greater than specified LSN
  if (! done) {
    if (inner_piece_context_.is_valid()
        && round_context_.is_valid()
        && inner_piece_context_.min_lsn_in_piece_.is_valid()
