...
public:
  ...
  int get_piece(const share::SCN &pre_scn,
      ...
      palf::LSN &max_lsn,
      bool &to_newest);
  // For convenient continuous consumption of active files, update the consumed records to piece context
...
private:
  ...
  struct RoundContext
  {
    ...
    State state_;
    int64_t round_id_;
    share::SCN start_scn_;              // the minimum log SCN of this round
    share::SCN end_scn_;                // The maximum log SCN for this round, if not in STOP state, this time is SCN_MAX
    int64_t min_piece_id_;          // the minimum piece of this round, this value may increase with the recovery of archived data
    int64_t max_piece_id_;          // the maximum piece of this round, the value may increase for an ACTIVE round
  ...
  // Read a piece that is already in frozen state, and have finished reading the last file of this piece, then the file range information of this piece
  // as well as the maximum LSN of the last file have been determined, if reading is still needed, then switch piece
  struct InnerPieceContext
  {
    enum State : uint8_t
    {
      INVALID = 0,
      FROZEN = 1,       // Normal FROZEN state piece, containing log and meta files
      EMPTY = 2,        // the current piece is empty, it has only meta information without logs, its log range is determined, it is a special case of FROZEN state
      LOW_BOUND = 3,    // piece exists and is FROZEN, but the log stream has not yet been generated for this piece, which is another special case of the FROZEN state
      ACTIVE = 4,       // current piece is not frozen, still has log writes
      GC = 5,           // Log stream has been GC'd in this piece, no need to forward slice the piece again, also a special case of FROZEN state
    };

    State state_;                 // current piece state
    int64_t piece_id_;            // current piece id
    int64_t round_id_;            // the round this piece belongs to

    palf::LSN min_lsn_in_piece_;  // minimum LSN in piece
    palf::LSN max_lsn_in_piece_;  // maximum LSN in piece, this value is only valid for piece in FROZEN state
    int64_t min_file_id_;         // current piece maximum file id
    int64_t max_file_id_;         // current piece minimum file id

    int64_t file_id_;         // maximum file id of the current piece that has been read
    int64_t file_offset_;     // Maximum file offset read for the current piece
    palf::LSN max_lsn_;       // Current piece maximum file offset corresponding log LSN
...
private:
  int get_piece_(const share::SCN &scn,
      ...
      palf::LSN &max_lsn,
      bool &to_newest);
  // Get the basic metadata of the archive source
  ...
  // If round does not meet data requirements, support round switch
  ...
  // Get the metadata of the specified round
  virtual int load_round_(const int64_t round_id, RoundContext &round_context, bool &exist);
  // round may not be consecutive, check if round exists
  ...
  int backward_round_();
  // When piece does not match, need to slice piece
  ...
  // load current piece information, including file range and LSN range within the piece
...
private:
  ...
  int64_t file_offset_;     // current piece maximum file offset read
};
