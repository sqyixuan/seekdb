...
#include "close_modules/observer_lite/logservice/restoreservice/ob_log_archive_piece_mgr.h"
#include "close_modules/observer_lite/logservice/restoreservice/ob_log_restore_define.h"
...
struct RemoteDataBuffer
{
  ...
  int next(LogEntryType &entry, LSN &lsn, const char *&buf, int64_t &buf_size)
  {
    ...
    if (is_empty()) {
      ...
      // The current return entry corresponds to the buff and its length
...
{
public:
  // TODO Consider concurrent log consumption of physical recovery log streams, it should use 64M and integer multiples, for now do not consider
