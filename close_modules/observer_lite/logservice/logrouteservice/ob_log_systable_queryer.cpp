    ...
    ObUnitsRecordInfo &units_record_info)
{
  ...
  } else if (OB_FAIL(units_record.init(GCTX.self_addr(), GCTX.config_->zone.str(),
                                       ObZoneType::ZONE_TYPE_READWRITE, DEFAULT_REGION_NAME))) {
    LOG_WARN("units_record init failed", KR(ret));
    ...
    ObAllServerInfo &all_server_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), KP(GCTX.config_));
  } else if (OB_FAIL(server_record.init(OB_INIT_SERVER_ID, GCTX.self_addr(),
                     share::ObServerStatus::OB_SERVER_ACTIVE, GCTX.config_->zone))) {
    LOG_WARN("fail to init server record", KR(ret));
    ...
    ObAllZoneInfo &all_zone_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), KP(GCTX.config_));
  } else if (OB_FAIL(zone_record.init(GCTX.config_->zone.str(), DEFAULT_REGION_NAME))) {
    LOG_WARN("fail to init zone record", KR(ret));
    ...
    ObAllZoneTypeInfo &all_zone_type_info)
{
  ...
  if (IS_NOT_INIT) {
    ...
    ret = OB_INVALID_ARGUMENT;
    LOG_WARN("invalid argument", KR(ret), KP(GCTX.config_));
  } else if (OB_FAIL(zone_type_record.init(GCTX.config_->zone.str(), ObZoneType::ZONE_TYPE_READWRITE))) {
    LOG_WARN("fail to init zone type record", KR(ret));
