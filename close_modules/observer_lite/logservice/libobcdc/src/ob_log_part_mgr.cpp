    ...
    const char *&db_name)
{
  ...
  // 1. Due to the Schema implementation defect, if a tenant is deleted, when retrieving historical schema, the deleted tenant's
  //    "oceanbase" DB schema will not be built. But some system tables under the oceanbase DB,
  //    For example __all_dummy, etc., will be built, i.e., the table schema exists but the DB schema
  //    The situation does not exist. Since libobcdc does not need to synchronize system tables, the filtering of system tables is performed here through
  //    Method to avoid the situation where DB schema does not exist.
  //
  // 2. Ensure the global index table is not filtered out
  //
  // 3. Ensure that the unique index table is not filtered out, used for maintaining TableIDCache
  //    Note: is_unique_index interface and is_global_index_table have an intersection: global unique index
  //
  // 4. DDL table is default filtered
  //
  // 5. In backupm mode, specify tables not to be filtered
  //
  // 6. Temporary tables are not user tables/system tables/unique indexes/global indexes, so this function will also filter out temporary tables
