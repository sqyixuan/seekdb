  ...
  if (OB_ISNULL(tenant_name)) {
    ...
    for (int64_t idx = 0, cnt = ptns->count(); OB_SUCCESS == ret && !matched && idx < cnt; ++idx) {
      ...
      if (0 != (err = fnmatch(pattern.tenant_pattern_.ptr(),
        ...
        // Matched.
        matched = true;
        // Here it affects the maintenance of the tenant_id collection and the filtering logic of DDL statements
        // If it is a blacklist match:
        // 1. tt1.*.* format, indicating that all data for this tenant needs to be filtered
        // 2. tt1.*.*_t format, only filter table data ending with _t for this tenant, but tenant information will not be filtered
        //    Same tt1.db1.* format, only filter all data of tenant db1, but tenant information will not be filtered
        // 3. Configure shadow table: *.*.*_t|*.*.*_[0-9][a-z], at this time, when matching tenant blacklist, it will not filter
        if (is_black) {
          // Always ensure that pattern is a null-terminated string, see build_patterns_() implementation
          // Therefore build the same format, ObString::case_compare will compare length
          char tmp_str[2];
          tmp_str[0] = '*';
          tmp_str[1] = '\0';
          ...
          if ((0 == strcmp(pattern.database_pattern_.ptr(), tmp_str))
            ...
            // depend on fnmatch match result
          } else {
            // As long as there exists a db or table that is not *, it means this tenant cannot be filtered
            matched = false;
          }
        }
      }
