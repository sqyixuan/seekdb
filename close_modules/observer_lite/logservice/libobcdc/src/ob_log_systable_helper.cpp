    ...
    int64_t &pos)
{
  ...
  if (OB_ISNULL(sql_buf) || OB_UNLIKELY(mul_statement_buf_len <=0)) {
    ...
    LOG_ERROR("not support fetch timezone_version in tenant_sync_mode", KR(ret));
      ...
      "SELECT VALUE FROM %s WHERE NAME = 'CURRENT_TIMEZONE_VERSION'", OB_ALL_VIRTUAL_SYS_STAT_TNAME))) {
    ...
    int64_t &pos)
{
  ...
  if (OB_ISNULL(sql_buf) || OB_UNLIKELY(mul_statement_buf_len <=0)) {
    ...
    LOG_ERROR("invalid argument", KR(ret), K(sql_buf), K(mul_statement_buf_len));
          ...
          "SELECT 1 as LS_ID FROM %s", OB_CDB_OB_LS_TNAME))) {
    LOG_ERROR("build sql fail", KR(ret), K(pos), "buf_size", mul_statement_buf_len, KCSTRING(sql_buf));
  }
...
{
  int ret = OB_SUCCESS;
  TenantInfo tenant_info(OB_SYS_TENANT_ID/*tenant_id*/, "sys"/*tenant_name*/);
  if (OB_FAIL(tenant_info_list.push_back(tenant_info))) {
    LOG_WARN("fail to push back tenant info", KR(ret), K(tenant_info));
  }
  LOG_DEBUG("query all tenant info", KR(ret), K(tenant_info_list));
  return ret;
}
    ...
    common::ObIArray<common::ObAddr> &sql_server_list)
{
  ...
  } else if (OB_FAIL(sql_server_list.push_back(GCTX.self_addr()))) {
...
{
  int ret = OB_SUCCESS;
  if (OB_FAIL(tenant_id_list.push_back(OB_SYS_TENANT_ID))) {
    LOG_WARN("failt o push back sys tenant id", KR(ret));
  }
  LOG_DEBUG("query all tenant id", KR(ret), K(tenant_id_list));
  return ret;
}
