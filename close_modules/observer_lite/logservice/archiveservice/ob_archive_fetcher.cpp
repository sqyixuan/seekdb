...
void ObArchiveFetcher::check_capacity_enough_(const LSN &commit_lsn,
    ...
    bool &data_full)
{
  // Already large enough to compress the encryption unit or reach the end of the archive file (which is also the end of the ob log file)
...
// generate send buffer before there must be enough data
int ObArchiveFetcher::generate_send_buffer_(PalfGroupBufferIterator &iter, TmpMemoryHelper &helper)
{
  ...
  LogGroupEntry entry;
  LSN offset;
  bool piece_change = false;  // Encounter a larger piece, data within different pieces are separated, current piece has reached the end
  bool iter_end = false;    // Task has been processed to end_lsn
  SCN max_scn;
  ...
  while (OB_SUCC(ret) && ! iter_end && ! piece_change && ! has_set_stop()) {
    ...
    if (OB_FAIL(iter.next(max_scn))) {
      ...
      // Since the archive starts from the beginning LSN of the palf block, there are some logs whose scn is less than the archive round_start_scn,
      // For this part of the log, archive to the first piece
      ...
      if (OB_FAIL(fill_helper_piece_if_empty_(piece, helper))) {
        ...
        // Log belongs to a piece exceeding the current task piece, this log will not be archived under the task description piece
        piece_change = true;
      ...
      // If piece slicing is needed or there is already enough unitized processed data, proceed with processing
...
// Since the same LogFetchTask can contain multiple piece data, it needs to be archived into multiple directories, only when all directories are archived completely
// can consume the next LogFetchTask data
...
int ObArchiveFetcher::try_consume_fetch_log_(const ObLSID &id)
{
  ...
  GET_LS_TASK_CTX(ls_mgr_, id) {
    // To ensure that the fetch log on LS is not starved, if consumption rights are obtained, the task must be consumed completely
...
// Aggregate a sufficiently large processing unit, or reach the end of the archive file
