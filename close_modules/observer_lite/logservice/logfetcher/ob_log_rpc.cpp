...
#endif
#include "close_modules/observer_lite/logservice/data_dictionary/ob_data_dict_utils.h"
  ...
  if (enable_ssl_client_authentication) {
    ...
    if (OB_SUCC(ret)) {
      ...
      if (ssl_config.empty() && ! file_exist) {
        ...
        if (is_local_file_mode) {
          if (OB_FAIL(observer::ObSrvNetworkFrame::extract_expired_time(OB_CLIENT_SSL_CERT_FILE, ssl_key_expired_time))) {
