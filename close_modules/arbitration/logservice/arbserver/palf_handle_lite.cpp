  ...
  if (IS_NOT_INIT) {
    ...
    do {
      ...
      if (ObRole::LEADER != curr_role || ObReplicaState::ACTIVE != curr_state) {
        ...
        // Explanation that election has already switched to the new leader, PALF will eventually switch to <FOLLOWER, PENDING>, this situation is also considered as pending state,
        // Otherwise the caller sees PALF as <FOLLOWER, ACTIVE>, but proposal_id is still the value from LEADER,
        // may lead to unexpected behavior, for example role change service might execute the resignation operation prematurely.
        role = ObRole::FOLLOWER;
        is_pending_state = true;
      } else {
        // Return LEADER
...
int PalfHandleLite::receive_log(const common::ObAddr &server,
...
int PalfHandleLite::receive_batch_log(const common::ObAddr &server,
                                      const int64_t mgs_proposal_id,
                                      const int64_t prev_log_proposal_id,
                                      const palf::LSN &prev_lsn,
                                      const palf::LSN &curr_lsn,
                                      const char *buf,
                                      const int64_t buf_len)
{
  int ret = OB_NOT_SUPPORTED;
...
int PalfHandleLite::handle_notify_fetch_log_req(const common::ObAddr &server)
{
  int ret = OB_NOT_SUPPORTED;
  PALF_LOG(WARN, "arb replica not support this function", K(ret), KPC(this));
  return ret;
                                       ...
                                       const LogConfigMeta &meta)
{
  ...
  if (IS_NOT_INIT) {
    ...
    if (false == state_mgr_.can_receive_config_log(msg_proposal_id) ||
      ...
      // TODO Whether it should switch to a different mode meta retrieval method, relying on log pull to trigger mode meta retrieval is not very feasible.
